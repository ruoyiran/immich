...
- 保持 ML 为独立 HTTP inference service。当前 `../../photo-classifier/` runtime 使用其配置的外部模型服务，不依赖本目录。
- 在 Python schema 中定义 request/response 变更；只有外部服务器明确采用本服务时，才在 `../../photo-classifier/` 同步 integration。
...
- `/predict` contract 变更需要检查本模块的 request/response 与调用方；不要假设 `../../photo-classifier/` 正在消费它。
...
在完整 checklist 前，先针对 endpoint/cache/provider 行为运行 focused pytest。若 `../../photo-classifier/` 已接入对应 endpoint，还要在该仓库运行调用方测试。
