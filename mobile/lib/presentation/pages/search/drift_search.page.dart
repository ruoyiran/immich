...
class DriftSearchPage extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    void datePicked(DateFilterInputModel? selectedDate) {
      ...
      search(
        filter.value.copyWith(
          date: SearchDateFilter(takenAfter: date.start, takenBefore: date.end),
        ),
      );
    }

    Future<void> showDateRangeSelector() async {
    ...
    Future<void> showCalendarPeriodPicker(CalendarPeriod period) async {
      final date = await showDatePicker(
        context: context,
        firstDate: DateTime(1900),
        lastDate: DateTime.now(),
        initialDate: dateInputFilter.value?.asDateTimeRange().start ?? DateTime.now(),
        helpText: 'search_filter_date_title'.t(context: context),
        cancelText: 'cancel'.t(context: context),
        confirmText: 'select'.t(context: context),
      );
      if (date != null) {
        datePicked(CalendarPeriodDateFilter(date, period));
      }
    }
          ...
          child: FilterBottomSheetScaffold(
            ...
            child: QuickDatePicker(
              ...
              onRequestPicker: () {
                ...
                unawaited(showDateRangeSelector());
              },
              onRequestPeriodPicker: (period) {
                ContextHelper(context).pop();
                unawaited(showCalendarPeriodPicker(period));
              },
              onSelect: (date) {
                ContextHelper(context).pop();
                datePicked(date);
