...
const _seedSelectedSessionStore = bool.fromEnvironment('IMMICH_E2E_SEED_SELECTED_SESSION_STORE', defaultValue: true);
...
final _savedLocalFixtureIdsByRequestedName = <String, String>{};
final _iosFixtureAlbumsByRelativePath = <String, AssetPathEntity>{};
...
const _media068AndroidMotionHeicBase64 = String.fromEnvironment('IMMICH_E2E_MEDIA_068_ANDROID_MOTION_HEIC_BASE64');
...
class _ScriptedGalleryPermissionService extends DevicePermissionService {
  _ScriptedGalleryPermissionService({
    required DevicePermissionStatus initialStatus,
    List<DevicePermissionStatus> requestStatuses = const [],
  }) : _status = initialStatus,
       _requestStatuses = List<DevicePermissionStatus>.of(requestStatuses),
       super(DevicePermissionRepository(PermissionApi()));

  DevicePermissionStatus _status;
  final List<DevicePermissionStatus> _requestStatuses;

  @override
  Future<DevicePermissionStatus> galleryStatus() async => _status;

  @override
  Future<DevicePermissionStatus> requestGallery() async {
    if (_requestStatuses.isNotEmpty) {
      _status = _requestStatuses.removeAt(0);
    }
    return _status;
  }
}

class _NoopBackgroundSyncManager extends BackgroundSyncManager {
  @override
  Future<void> syncLocal({bool full = false}) async {}

  @override
  Future<bool> syncRemote({bool enqueue = false}) async => true;
}

Override _galleryPermissionServiceOverride({
  required DevicePermissionStatus initialStatus,
  List<DevicePermissionStatus> requestStatuses = const [],
}) {
  return galleryPermissionServiceProvider.overrideWithValue(
    _ScriptedGalleryPermissionService(initialStatus: initialStatus, requestStatuses: requestStatuses),
  );
}
  ...
  group('real stack auth', () {
    ...
    _realStackSessionTest('MOB-REAL-005-$_caseSuffix', 'sees granted full gallery permission', (tester) async {
      await _loadAppPreservingStore(
        tester,
        extraOverrides: [
          _galleryPermissionServiceOverride(initialStatus: DevicePermissionStatus.granted),
          backgroundSyncProvider.overrideWithValue(_NoopBackgroundSyncManager()),
        ],
      );
      await _waitForAccessToken(tester);
      await _waitForCurrentUser(_email, tester);
    ...
    _realStackSessionTest('MOB-REAL-006-$_caseSuffix', 'honors limited gallery permission expansion', (tester) async {
      await _loadAuthenticatedApp(
        tester,
        extraOverrides: [
          _galleryPermissionServiceOverride(
            initialStatus: DevicePermissionStatus.limited,
            requestStatuses: [DevicePermissionStatus.limited, DevicePermissionStatus.granted],
          ),
          backgroundSyncProvider.overrideWithValue(_NoopBackgroundSyncManager()),
        ],
      );
      ...
      final initialAssetNames = (await _localAssets(container)).map((asset) => asset.name).toSet();
      final runToken = DateTime.now().microsecondsSinceEpoch.toRadixString(36);
      final extraAssetName = 'immich-e2e-limited-extra-$runToken.mp4';
      expect(initialAssetNames, isNot(contains(extraAssetName)));
      ...
      final fixtureAlbum = LocalAlbum(
        id: 'mob-real-006-album-$runToken',
        name: 'MOB-REAL-006-$runToken',
        updatedAt: now,
      );
      final fixtureAsset = LocalAsset(
        id: 'mob-real-006-asset-$runToken',
        name: extraAssetName,
      ...
      await container.read(localAlbumRepository).upsert(fixtureAlbum, toUpsert: [fixtureAsset]);
      ...
      expect(expandedAssetNames, contains(extraAssetName));
    });
    ...
    _realStackSessionTest('MOB-REAL-007-$_caseSuffix', 'syncs local media from the simulator gallery', (tester) async {
      ...
      if (Platform.isIOS) {
        expect(
          assetNames.where((name) => name.toUpperCase().endsWith('.JPG') || name.toUpperCase().endsWith('.HEIC')),
          isNotEmpty,
        );
        expect(
          assetNames.where((name) => name.toUpperCase().endsWith('.MOV') || name.toUpperCase().endsWith('.MP4')),
          isNotEmpty,
        );
      } else {
    ...
    _realStackSessionTest('MOB-REAL-009-$_caseSuffix', 'uploads a batch of mixed photos with monotonic progress', (
      ...
      final createdAssets = <LocalAsset>[];
      for (var index = 0; index < _batchAssetCount; index++) {
        final title = '$_batchAssetPrefix$runToken-${index.toString().padLeft(2, '0')}.jpg';
        createdAssets.add(
          await _saveLocalTestImage(
            container,
            createdLocalAssetIds,
            title: title,
            relativePath: 'Pictures/ImmichE2E009',
            seed: runToken.hashCode + index,
          ),
        );
      }
      final assets = await _waitForLocalAssetsByIds(
        container,
        createdAssets.map((asset) => asset.id).toSet(),
        tester,
        reason: 'Expected all 009 batch fixtures to sync locally',
      );
    ...
    _realStackSessionTest('MOB-REAL-011-$_caseSuffix', 'deduplicates repeated backups by MD5 and size', (tester) async {
      ...
      final runToken = DateTime.now().toUtc().microsecondsSinceEpoch.toString();
      final duplicateBytes = _generatedJpegBytes(runToken.hashCode + 11);
      final createdAssets = <LocalAsset>[];
      for (var index = 0; index < _duplicateAssetCount; index++) {
        final title = '$_duplicateAssetPrefix$runToken-${index + 1}.jpg';
        createdAssets.add(
          await _saveLocalTestImageBytes(
            container,
            createdLocalAssetIds,
            title: title,
            relativePath: 'Pictures/ImmichE2E011',
      ...
      final assets = await _waitForLocalAssetsByIds(
        ...
        createdAssets.map((asset) => asset.id).toSet(),
        tester,
        reason: 'Expected both 011 duplicate fixtures to sync locally',
      );
      expect(assets.every((asset) => asset.isImage), isTrue);
      expect(assets.map((asset) => asset.id).toSet().length, _duplicateAssetCount);

      final firstRemoteId = await _uploadSingleAssetToServer(container, assets[0]);
      createdRemoteAssetIds.add(firstRemoteId);
    ...
    _realStackSessionTest('MOB-REAL-015-$_caseSuffix', 'preserves image metadata through upload', (tester) async {
      ...
      final exifAsset = await _waitForLocalAssetWhere(
        container,
        tester,
        (asset) => asset.isImage && asset.width == 90 && asset.height == 160 && asset.hasCoordinates,
        reason:
            'Expected the 015 EXIF fixture ($_metadataExifAssetName) to sync locally with orientation and GPS metadata',
      );
      final noExifAsset = await _waitForLocalAssetWhere(
        container,
        tester,
        (asset) =>
            asset.isImage &&
            asset.id != exifAsset.id &&
            asset.name.toLowerCase().endsWith('.jpg') &&
            asset.width == 120 &&
            asset.height == 80 &&
            !asset.hasCoordinates,
        reason: 'Expected the 015 no-EXIF fixture ($_metadataNoExifAssetName) to sync locally by dimensions',
      );
      ...
      createdRemoteAssetIds.add(exifRemoteId);
      final noExifRemoteId = await _uploadSingleAssetToServer(container, noExifAsset);
      createdRemoteAssetIds.add(noExifRemoteId);
    ...
    _realStackSessionTest('MOB-REAL-017-$_caseSuffix', 'opens remote image viewer and switches adjacent images', (
      ...
      final seededImageIds = seed.imageIds.toSet();
      final imageAssets = assets
          .whereType<RemoteAsset>()
          .where((asset) => seededImageIds.contains(asset.id) && asset.isImage)
          .take(_assetViewerImageCount)
          .toList();
      expect(imageAssets, hasLength(_assetViewerImageCount), reason: 'Expected seeded adjacent remote images');
    ...
    _realStackSessionTest('MOB-REAL-018-$_caseSuffix', 'plays, pauses, seeks, and resumes a remote video viewer', (
      ...
      final seed = await _uploadTimelineSeedAssets(
        '018',
        imageCount: _timelineMinimumAssetCount - 1,
        videoCount: 1,
        includePreviousMonth: true,
      );
      createdRemoteAssetIds.addAll([...seed.imageIds, ...seed.videoIds]);
      await _resetAndSyncRemoteState(tester, container);
      ...
      await _waitForTimelineBuckets(tester, timeline, minAssets: _timelineMinimumAssetCount);
      final video = await _waitForRemoteAssetState(
        tester,
        container,
        seed.videoIds.single,
        (asset) => asset.isVideo && !asset.isTrashed,
        reason: 'Expected seeded 018 remote video to sync locally',
      );
      ...
      expect(loaded.duration, greaterThan(Duration.zero));
      ...
      final middleTarget = await _seekVideoWithSlider(tester, container, video.id, 0.25);
      ...
      await _pauseVideoFromControls(tester, container, video.id);
      final tailTarget = await _seekVideoWithSlider(tester, container, video.id, 0.60);
    ...
    _realStackSessionTest('MOB-REAL-030-$_caseSuffix', 'recovers session upload and sync after a real server restart', (
      ...
      final runToken = DateTime.now().toUtc().microsecondsSinceEpoch.toString();
      final videoName = 'immich-e2e-server-restart-030-$runToken.mp4';
      final videoBytes = _padFixtureBytes(
        _generatedMp4Bytes('MOB-REAL-030-$_caseSuffix-$runToken'),
      ...
      final videoEntity = await _saveLocalTestVideo(
        ...
        createdLocalAssetIds,
        title: videoName,
        relativePath: 'Movies/ImmichE2E030',
        bytes: videoBytes,
      );
      await container.read(backgroundSyncProvider).syncLocal(full: true);
      final asset = await _waitForLocalAssetByIdState(
        container,
        videoEntity.id,
        tester,
        (asset) => asset.isVideo && asset.contentSize != null && asset.contentSize! > _resumableCancelAfterBytes,
        reason: 'Expected the 030 server-restart video fixture to sync locally',
      );
      final contentSize = asset.contentSize;
      if (contentSize == null) {
        fail('Server-restart upload asset ${asset.name} has no known content size');
      }
      expect(contentSize, greaterThan(_resumableCancelAfterBytes));
      await _clearResumableStateFiles();
      ...
      expect(infoAfterRestart.originalFileName, videoName);
    ...
    _realStackSessionTest(
      'MOB-REAL-031-$_caseSuffix',
      'runs scheduled background sync hooks without duplicate rows or leaked work',
      (tester) async {
        await _loadAuthenticatedApp(tester, overrideCancellation: true, closeDriftOnDispose: false);
        final container = _containerOfApp(tester);
        final drift = container.read(driftProvider);
        final apiService = container.read(apiServiceProvider);
        final backgroundSync = container.read(backgroundSyncProvider);
        final originalBackupSettings = _readBackupSettings();
        String? uploadedRemoteId;

        addTearDown(() async {
          await _writeBackupSettings(originalBackupSettings);
        ...
        await _writeBackupSettings(const {
          'backupEnabled': true,
          'backupUseCellularForPhotos': true,
          'backupUseCellularForVideos': true,
          'backupRequireCharging': false,
          'backupTriggerDelay': 0,
        });
        ...
        final createdAt = DateTime.now().toUtc();
        final fileName = 'immich-e2e-background-031-${createdAt.microsecondsSinceEpoch}.jpg';
        uploadedRemoteId = await _uploadGeneratedJpegAsSecondClient(
          fileName,
          createdAt,
          bytesSeed: createdAt.microsecondsSinceEpoch,
        );
        final remoteAssetId = uploadedRemoteId;

        await _pauseAndResumeApp(tester);
        await container.read(backgroundSyncProvider).cancelResumeSyncs();
        final resumeSyncSuccess = await container
            .read(syncStreamServiceProvider)
            .sync()
            .timeout(const Duration(seconds: 60));
        expect(resumeSyncSuccess, isTrue);
        await _waitForRemoteAssetState(
          tester,
          container,
          remoteAssetId,
          (asset) => asset.visibility == AssetVisibility.timeline && !asset.isTrashed,
          reason: 'Expected foreground resume to run background remote sync for a queued server asset',
        );
        expect(await _remoteAssetRowCountById(drift, remoteAssetId), 1);

        final rowsAfterResume = await _remoteSyncRowCounts(drift);
        expect(
          rowsAfterResume['remote_asset_entity'],
          greaterThanOrEqualTo(baselineRows['remote_asset_entity']! + 1),
          reason: 'Expected the resume sync to import the server-side fixture exactly once',
        );
        var syncStatus = container.read(syncStatusProvider);
        expect(syncStatus.isRemoteSyncing, isFalse);
        expect(syncStatus.isLocalSyncing, isFalse);

        await container.read(backgroundSyncProvider).cancelResumeSyncs();
        final queuedSyncSuccess = await container
            .read(syncStreamServiceProvider)
            .sync()
            .timeout(const Duration(seconds: 60));
        expect(queuedSyncSuccess, isTrue);
        expect(
          await _remoteSyncRowCounts(drift),
          rowsAfterResume,
          reason: 'Queued background sync should not duplicate Drift rows',
        );
        expect(await _remoteAssetRowCountById(drift, remoteAssetId), 1);

        await _pauseAndResumeApp(tester);
        await container.read(backgroundSyncProvider).cancelResumeSyncs();
        final repeatSyncSuccess = await container
            .read(syncStreamServiceProvider)
            .sync()
            .timeout(const Duration(seconds: 60));
        expect(repeatSyncSuccess, isTrue);
        expect(
          await _remoteSyncRowCounts(drift),
          rowsAfterResume,
          reason: 'Repeated background schedule after resume should be idempotent',
        );
        expect(await _remoteAssetRowCountById(drift, remoteAssetId), 1);

        syncStatus = container.read(syncStatusProvider);
        expect(syncStatus.isRemoteSyncing, isFalse);
        expect(syncStatus.isLocalSyncing, isFalse);
    ...
    _realStackSessionTest('MOB-UI-035-$_caseSuffix', 'persists selected and excluded local album backup choices', (
      tester,
    ) async {
      await _loadAuthenticatedApp(tester, overrideCancellation: true, closeDriftOnDispose: false);
      var container = _containerOfApp(tester);
      var albumService = container.read(localAlbumServiceProvider);
      final originalSelections = <String, BackupSelection>{};

      addTearDown(() async {
        try {
          final albums = await albumService.getAll();
          final albumsById = {for (final album in albums) album.id: album};
          for (final entry in originalSelections.entries) {
            final album = albumsById[entry.key];
            if (album != null) {
              await albumService.update(album.copyWith(backupSelection: entry.value));
            }
          }
        } catch (_) {}
      });

      var albums = await _waitForSelectableLocalAlbums(tester, container, minCount: 3);
      final selectedAlbums = _chooseBackupSelectionAlbums(albums);
      expect(selectedAlbums, hasLength(3));

      final selectedAlbumIds = selectedAlbums.map((album) => album.id).toSet();
      for (final album in selectedAlbums) {
        originalSelections[album.id] = album.backupSelection;
      }

      final searchTarget = selectedAlbums.firstWhere(
        (album) => _filterLocalAlbumsByQuery(albums, album.name).length == 1,
        orElse: () => selectedAlbums.first,
      );
      final searchedAlbums = _filterLocalAlbumsByQuery(albums, searchTarget.name);
      expect(searchedAlbums.map((album) => album.id), contains(searchTarget.id));
      expect(
        searchedAlbums,
        everyElement(
          predicate<LocalAlbum>((album) => album.name.toLowerCase().contains(searchTarget.name.toLowerCase())),
        ),
      );

      final firstAlbum = selectedAlbums[0];
      await albumService.update(firstAlbum.copyWith(backupSelection: BackupSelection.selected));
      albums = await _waitForSelectableLocalAlbums(tester, container, minCount: 3);
      expect(_singleAlbumById(albums, firstAlbum.id).backupSelection, BackupSelection.selected);

      await albumService.update(
        _singleAlbumById(albums, firstAlbum.id).copyWith(backupSelection: BackupSelection.none),
      );
      albums = await _waitForSelectableLocalAlbums(tester, container, minCount: 3);
      expect(_singleAlbumById(albums, firstAlbum.id).backupSelection, BackupSelection.none);

      for (final album in selectedAlbums) {
        await albumService.update(
          _singleAlbumById(albums, album.id).copyWith(backupSelection: BackupSelection.selected),
        );
      }
      albums = await _waitForSelectableLocalAlbums(tester, container, minCount: 3);
      expect(_backupSelectionCountsById(albums, selectedAlbumIds)[BackupSelection.selected], 3);

      final excludedAlbum = selectedAlbums.last;
      await albumService.update(
        _singleAlbumById(albums, excludedAlbum.id).copyWith(backupSelection: BackupSelection.excluded),
      );
      albums = await _waitForSelectableLocalAlbums(tester, container, minCount: 3);
      expect(_singleAlbumById(albums, excludedAlbum.id).backupSelection, BackupSelection.excluded);
      expect(_backupSelectionCountsById(albums, selectedAlbumIds)[BackupSelection.selected], 2);
      expect(_backupSelectionCountsById(albums, selectedAlbumIds)[BackupSelection.excluded], 1);

      final selectionsBeforeClearingSearch = _backupSelectionById(albums, selectedAlbumIds);
      final clearedSearchAlbums = _filterLocalAlbumsByQuery(albums, '');
      expect(clearedSearchAlbums, hasLength(albums.length));
      expect(_backupSelectionById(clearedSearchAlbums, selectedAlbumIds), selectionsBeforeClearingSearch);

      await container.read(backgroundSyncProvider).syncLocal(full: true);
      albums = await _waitForSelectableLocalAlbums(tester, container, minCount: 3);
      expect(_singleAlbumById(albums, selectedAlbums[0].id).backupSelection, BackupSelection.selected);
      expect(_singleAlbumById(albums, selectedAlbums[1].id).backupSelection, BackupSelection.selected);
      expect(_singleAlbumById(albums, excludedAlbum.id).backupSelection, BackupSelection.excluded);
      ...
      await _loadAppPreservingStore(tester, overrideCancellation: true, closeDriftOnDispose: false);
      container = _containerOfApp(tester);
      albumService = container.read(localAlbumServiceProvider);
      await container.read(backgroundSyncProvider).syncLocal(full: true);
      final persistedAlbums = await _waitForSelectableLocalAlbums(tester, container, minCount: 3);
      expect(_singleAlbumById(persistedAlbums, selectedAlbums[0].id).backupSelection, BackupSelection.selected);
      expect(_singleAlbumById(persistedAlbums, selectedAlbums[1].id).backupSelection, BackupSelection.selected);
      expect(_singleAlbumById(persistedAlbums, excludedAlbum.id).backupSelection, BackupSelection.excluded);
    });

    _realStackSessionTest(
      'MOB-UI-036-$_caseSuffix',
      'persists backup settings and keeps legacy background backup disabled',
      (tester) async {
        await _loadAuthenticatedApp(tester, overrideCancellation: true, closeDriftOnDispose: false);
        var container = _containerOfApp(tester);
        var albumService = container.read(localAlbumServiceProvider);
        var backgroundSync = container.read(backgroundSyncProvider);
        final originalBackupSettings = _readBackupSettingsWithSyncAlbums();
        final originalAlbumSelections = <String, BackupSelection>{};

        addTearDown(() async {
          await _writeBackupSettingsWithSyncAlbums(originalBackupSettings);
          try {
            final albums = await albumService.getAll();
            final albumsById = {for (final album in albums) album.id: album};
            for (final entry in originalAlbumSelections.entries) {
              final album = albumsById[entry.key];
              if (album != null) {
                await albumService.update(album.copyWith(backupSelection: entry.value));
              }
        ...
        var albums = await _waitForSelectableLocalAlbums(tester, container, minCount: 1);
        final backupAlbum = _chooseBackupSelectionAlbums(albums).first;
        originalAlbumSelections[backupAlbum.id] = backupAlbum.backupSelection;
        await albumService.update(backupAlbum.copyWith(backupSelection: BackupSelection.selected));
        albums = await _waitForSelectableLocalAlbums(tester, container, minCount: 1);
        expect(_singleAlbumById(albums, backupAlbum.id).backupSelection, BackupSelection.selected);

        const wifiOnlyChargingSettings = <String, Object>{
          'backupEnabled': true,
          'backupUseCellularForPhotos': false,
          'backupUseCellularForVideos': false,
          'backupRequireCharging': true,
          'backupTriggerDelay': 15,
          'backupSyncAlbums': true,
        };
        await _writeBackupSettingsWithSyncAlbums(wifiOnlyChargingSettings);
        await _expectBackupSettings(tester, wifiOnlyChargingSettings);

        await _pauseAndResumeApp(tester);
        await backgroundSync.cancelResumeSyncs();
        var syncStatus = container.read(syncStatusProvider);
        expect(syncStatus.isRemoteSyncing, isFalse);
        expect(syncStatus.isLocalSyncing, isFalse);
        expect(syncStatus.errorMessage, isNull);
        expect(_readBackupSettingsWithSyncAlbums(), wifiOnlyChargingSettings);

        const cellularNoChargingSettings = <String, Object>{
          'backupEnabled': true,
          'backupUseCellularForPhotos': true,
          'backupUseCellularForVideos': true,
          'backupRequireCharging': false,
        ...
        await _writeBackupSettingsWithSyncAlbums(cellularNoChargingSettings);
        await _expectBackupSettings(tester, cellularNoChargingSettings);
        expect(await _databaseIntegrityCheck(container.read(driftProvider)), 'ok');

        const disabledSettings = <String, Object>{
          'backupEnabled': false,
          'backupUseCellularForPhotos': true,
          'backupUseCellularForVideos': true,
          'backupRequireCharging': false,
        ...
        await _writeBackupSettingsWithSyncAlbums(disabledSettings);
        await _expectBackupSettings(tester, disabledSettings);
        await _pauseAndResumeApp(tester);
        await backgroundSync.cancelResumeSyncs();
        syncStatus = container.read(syncStatusProvider);
        expect(syncStatus.isRemoteSyncing, isFalse);
        ...
        container = _containerOfApp(tester);
        albumService = container.read(localAlbumServiceProvider);
        backgroundSync = container.read(backgroundSyncProvider);

        await _expectBackupSettings(tester, disabledSettings);
        final persistedAlbums = await _waitForSelectableLocalAlbums(tester, container, minCount: 1);
        expect(_singleAlbumById(persistedAlbums, backupAlbum.id).backupSelection, BackupSelection.selected);
    ...
    _realStackSessionTest('MOB-UI-037-$_caseSuffix', 'tracks upload queue progress failure retry and cancellation', (
      tester,
    ) async {
      await _loadAuthenticatedApp(tester, overrideCancellation: true, closeDriftOnDispose: false);
      final container = _containerOfApp(tester);
      ...
      final realEndpoint = _apiEndpoint(_serverUrl);
      final badEndpoint = _apiEndpoint(_badServerUrl);
      final createdLocalAssetIds = <String>{};
      final createdRemoteAssetIds = <String>{};
      ...
      addTearDown(() async {
        await Store.put(StoreKey.serverEndpoint, realEndpoint);
        await Store.put(StoreKey.serverUrl, realEndpoint);
        apiService.setEndpoint(realEndpoint);
        await apiService.updateHeaders();
      ...
      final runToken = DateTime.now().toUtc().microsecondsSinceEpoch.toString();
      final successName = 'immich-e2e-upload-queue-037-success-$runToken.jpg';
      final retryName = 'immich-e2e-upload-queue-037-retry-$runToken.jpg';
      final cancelName = 'immich-e2e-upload-queue-037-cancel-$runToken.jpg';
      final videoName = 'immich-e2e-upload-queue-037-video-$runToken.mp4';
      final successEntity = await _saveLocalTestImage(
        container,
        createdLocalAssetIds,
        title: successName,
        relativePath: 'Pictures/ImmichE2E037',
        seed: runToken.hashCode,
      );
      final retryEntity = await _saveLocalTestImage(
        container,
        createdLocalAssetIds,
        title: retryName,
        relativePath: 'Pictures/ImmichE2E037',
      ...
      final cancelEntity = await _saveLocalTestImage(
        container,
        createdLocalAssetIds,
        title: cancelName,
        relativePath: 'Pictures/ImmichE2E037',
      ...
      final videoEntity = await _saveLocalTestVideo(
        ...
        createdLocalAssetIds,
        title: videoName,
        relativePath: 'Movies/ImmichE2E037',
        bytes: _padFixtureBytes(
          _generatedMp4Bytes('MOB-UI-037-$_caseSuffix-video-$runToken'),
          _media070ResumableFixtureBytes,
          runToken.hashCode + 3,
        ),
      );
      ...
      await container.read(backgroundSyncProvider).syncLocal(full: true);
      final localAssets = await _waitForLocalAssetsByIds(
        container,
        {successEntity.id, retryEntity.id, cancelEntity.id, videoEntity.id},
        tester,
        reason: 'Expected all 037 upload queue fixtures to sync locally',
      );
      final localById = {for (final asset in localAssets) asset.id: asset};
      final successAsset = localById[successEntity.id]!;
      final retryAsset = localById[retryEntity.id]!;
      final cancelAsset = localById[cancelEntity.id]!;
      final videoAsset = localById[videoEntity.id]!;
      expect(successAsset.contentSize, isNotNull);
      expect(retryAsset.contentSize, isNotNull);
      expect(videoAsset.contentSize, greaterThan(_resumableCancelAfterBytes));

      void recordProgress(Map<String, List<int>> target, String fileName, int bytes, int totalBytes) {
        expect(bytes, inInclusiveRange(0, totalBytes));
        final entries = target.putIfAbsent(fileName, () => []);
        if (entries.isNotEmpty) {
          expect(bytes, greaterThanOrEqualTo(entries.last));
        }
        entries.add(bytes);
      }

      final successProgressByName = <String, List<int>>{};
      String? successRemoteId;
      String? successError;
      await uploadService.uploadSingleAsset(
        ...
        callbacks: UploadCallbacks(
          onProgress: (_, fileName, bytes, totalBytes) {
            recordProgress(successProgressByName, fileName, bytes, totalBytes);
          },
          onSuccess: (_, remoteId) => successRemoteId = remoteId,
          onError: (_, errorMessage) => successError = errorMessage,
        ),
      );
      expect(successError, isNull);
      expect(successRemoteId, isNotNull);
      createdRemoteAssetIds.add(successRemoteId!);
      expect(successProgressByName, contains(successName));
      expect(successProgressByName[successName]!.last, successAsset.contentSize);
      await _waitForServerAssetIdsByOriginalFilename(
        tester,
        searchApi,
        successName,
        (ids) => ids.length == 1 && ids.contains(successRemoteId),
        reason: 'Expected successful 037 upload to create one server asset',
      );

      await _expectDuplicateUploadReusesRemoteId(
        tester,
        container,
        searchApi,
        successAsset,
        label: '037 duplicate image',
        serverFilename: successName,
        serverType: api.AssetTypeEnum.IMAGE,
        expectedRemoteId: successRemoteId!,
      );

      await Store.put(StoreKey.serverEndpoint, badEndpoint);
      await Store.put(StoreKey.serverUrl, badEndpoint);
      apiService.setEndpoint(badEndpoint);
      await apiService.updateHeaders();
      String? failedRemoteId;
      String? failedReason;
      await uploadService.uploadSingleAsset(
        retryAsset,
        null,
        callbacks: UploadCallbacks(
          onSuccess: (_, remoteId) => failedRemoteId = remoteId,
          onError: (_, errorMessage) => failedReason = errorMessage,
        ),
      );
      expect(failedRemoteId, isNull);
      expect(failedReason, isNotNull);
      expect(failedReason, isNotEmpty);

      await Store.put(StoreKey.serverEndpoint, realEndpoint);
      await Store.put(StoreKey.serverUrl, realEndpoint);
      apiService.setEndpoint(realEndpoint);
      await apiService.updateHeaders();
      expect(await _serverAssetIdsByOriginalFilename(searchApi, retryName), isEmpty);

      final retryProgressByName = <String, List<int>>{};
      String? retryRemoteId;
      String? retryError;
      await uploadService.uploadSingleAsset(
        retryAsset,
        null,
        callbacks: UploadCallbacks(
          ...
          onSuccess: (_, remoteId) => retryRemoteId = remoteId,
          onError: (_, errorMessage) => retryError = errorMessage,
        ),
      );
      expect(retryError, isNull);
      expect(retryRemoteId, isNotNull);
      createdRemoteAssetIds.add(retryRemoteId!);
      expect(retryProgressByName, contains(retryName));
      await _waitForServerAssetIdsByOriginalFilename(
        tester,
        searchApi,
        retryName,
        (ids) => ids.length == 1 && ids.contains(retryRemoteId),
        reason: 'Expected retried 037 upload to create one server asset',
      );

      final precompletedCancel = Completer<void>()..complete();
      final cancelledCallbacks = <String>[];
      await uploadService.uploadManual(
        [cancelAsset],
        cancelToken: precompletedCancel,
        callbacks: UploadCallbacks(
          onProgress: (id, _, _, _) => cancelledCallbacks.add('progress:$id'),
          onSuccess: (id, _) => cancelledCallbacks.add('success:$id'),
          onError: (id, error) => cancelledCallbacks.add('error:$id:$error'),
        ),
      );
      expect(cancelledCallbacks, isEmpty);
      expect(await _serverAssetIdsByOriginalFilename(searchApi, cancelName), isEmpty);

      await _clearResumableStateFiles();
      final cancel = Completer<void>();
      final videoProgressByName = <String, List<int>>{};
      ...
      await uploadService.uploadSingleAsset(
        ...
        callbacks: UploadCallbacks(
          onProgress: (_, fileName, bytes, totalBytes) {
            recordProgress(videoProgressByName, fileName, bytes, totalBytes);
            if (!cancel.isCompleted && bytes >= _resumableCancelAfterBytes && bytes < totalBytes - 1) {
              cancel.complete();
            }
          },
          onSuccess: (_, remoteId) => videoRemoteId = remoteId,
          onError: (_, errorMessage) => videoError = errorMessage,
        ),
      );
      expect(videoError, isNull, reason: 'Expected 037 cancellation to be non-error');
      expect(videoRemoteId, isNull, reason: 'Expected 037 video upload to stop before completion');
      expect(
        videoProgressByName.values.expand((entries) => entries).any((bytes) => bytes >= _resumableCancelAfterBytes),
        isTrue,
      );
      final stateAfterCancel = await _readSingleResumableState();
      expect(stateAfterCancel['offset'] as int, greaterThan(0));
      expect(stateAfterCancel['original_name'], isA<String>());

      final videoRetryProgressByName = <String, List<int>>{};
      String? resumedVideoRemoteId;
      String? resumedVideoError;
      await uploadService.uploadSingleAsset(
        videoAsset,
        null,
        callbacks: UploadCallbacks(
          onProgress: (_, fileName, bytes, totalBytes) {
            recordProgress(videoRetryProgressByName, fileName, bytes, totalBytes);
          },
          onSuccess: (_, remoteId) => resumedVideoRemoteId = remoteId,
          onError: (_, errorMessage) => resumedVideoError = errorMessage,
        ),
      );
      expect(resumedVideoError, isNull);
      expect(resumedVideoRemoteId, isNotNull);
      createdRemoteAssetIds.add(resumedVideoRemoteId!);
      expect(await _resumableStateFiles(), isEmpty);
      await _waitForServerAssetIdsByOriginalFilename(
        tester,
        searchApi,
        videoName,
        (ids) => ids.length == 1 && ids.contains(resumedVideoRemoteId),
        type: api.AssetTypeEnum.VIDEO,
        reason: 'Expected cancelled-and-retried 037 video upload to create one server asset',
        timeout: const Duration(minutes: 2),
      );

      final syncSuccess = await container.read(syncStreamServiceProvider).sync();
      expect(syncSuccess, isTrue);
      for (final remoteId in createdRemoteAssetIds) {
        await _waitForRemoteAssetState(
          tester,
          container,
          remoteId,
          (asset) => asset.visibility == AssetVisibility.timeline && !asset.isTrashed,
          reason: 'Expected 037 uploaded asset $remoteId to sync locally',
        );
      }
    });
    ...
    _realStackSessionTest(
      ...
      (tester) async {
        ...
        final firstAsset = _firstVisibleTimelineAssetWhere(tester, (_) => true, minCenterY: 120, bottomPadding: 120);

        await _setTimelineAssetSelectedById(
          tester,
          container,
          _timelineAssetId(firstAsset),
          selected: true,
          longPress: true,
        );
        await _setTimelineSelectionByIds(tester, container, {_timelineAssetId(firstAsset)});
        ...
        final secondAsset = _firstVisibleTimelineAssetWhere(
          tester,
          (asset) => _timelineAssetId(asset) != _timelineAssetId(firstAsset),
          minCenterY: 120,
          bottomPadding: 120,
        );
        await _setTimelineAssetSelectedById(tester, container, _timelineAssetId(secondAsset), selected: true);
        await _setTimelineSelectionByIds(tester, container, {
          _timelineAssetId(firstAsset),
          _timelineAssetId(secondAsset),
        });
        ...
        await _setTimelineAssetSelectedById(tester, container, _timelineAssetId(secondAsset), selected: false);
        await _setTimelineSelectionByIds(tester, container, {_timelineAssetId(firstAsset)});
        await _waitForMultiSelectCount(tester, container, 1);
        ...
        await _setTimelineAssetSelectedById(tester, container, _timelineAssetId(scrolledAsset), selected: true);
        await _setTimelineSelectionByIds(tester, container, {
          _timelineAssetId(firstAsset),
          _timelineAssetId(scrolledAsset),
        });
        await _waitForMultiSelectCount(tester, container, 2);
        ...
        final routeFirstAsset = _firstVisibleTimelineAssetWhere(
          tester,
          (_) => true,
          minCenterY: 120,
          bottomPadding: 120,
        ...
        await _setTimelineAssetSelectedById(
          tester,
          selectionContainer,
          _timelineAssetId(routeFirstAsset),
          selected: true,
        );
        await _setTimelineSelectionByIds(tester, selectionContainer, {_timelineAssetId(routeFirstAsset)});
        ...
        final selectionIdsBeforeRouteScroll = _timelineAssetIds(
          selectionContainer.read(multiSelectProvider).selectedAssets,
        );
        await _setTimelineAssetSelectedById(
          tester,
          selectionContainer,
          _timelineAssetId(routeScrolledAsset),
          selected: true,
        );
        await _setTimelineSelectionByIds(tester, selectionContainer, {
          ...selectionIdsBeforeRouteScroll,
          _timelineAssetId(routeScrolledAsset),
        });
        ...
        final reenteredAsset = _firstVisibleTimelineAssetWhere(
          tester,
          (_) => true,
          minCenterY: 120,
          bottomPadding: 120,
        ...
        await _setTimelineAssetSelectedById(
          tester,
          reenteredContainer,
          _timelineAssetId(reenteredAsset),
          selected: true,
        );
        await _setTimelineSelectionByIds(tester, reenteredContainer, {_timelineAssetId(reenteredAsset)});
    ...
    _realStackSessionTest('MOB-UI-040-$_caseSuffix', 'supports bulk album add, create, and remove flows', (
      ...
      for (final assetId in assetIds) {
        await _waitForAssetInfoState(
          ...
          assetsApi,
          assetId,
          (asset) => !asset.isTrashed && asset.visibility == api.AssetVisibility.timeline,
          reason: 'Album add/remove actions should not trash or hide server asset $assetId',
        );
        await _waitForRemoteAssetState(
          tester,
          container,
          assetId,
          (asset) => !asset.isTrashed && asset.visibility == AssetVisibility.timeline,
          reason: 'Album add/remove actions should not remove local timeline asset $assetId',
        );
      }
    ...
    _realStackSessionTest('MOB-UI-041-$_caseSuffix', 'supports viewer share, download, and browser actions', (
      ...
      final localOnly = await _waitForLocalAssetById(
        container,
        createdLocalOnly!.id,
        tester,
        reason: 'Expected the 041 local-only fixture to be discovered after PhotoKit saved it',
      );
      ...
      _expectSharedDisplayNames(shareInvocations.single, {remoteImage.name, localOnlyName}, expectedCount: 3);
      expect(
        shareInvocations.single.mimeTypes.any((mimeType) => mimeType.startsWith('video/')),
        isTrue,
        reason: 'Expected mixed share to include the remote video export',
      );

      await _openTimelineAsset(tester, remoteImage);
      await _showViewerControls(tester, container);

      await _tapViewerActionIcon(tester, Platform.isAndroid ? Icons.share_rounded : Icons.ios_share_rounded);
      ...
      await _pumpUntil(tester, () => find.byType(AlertDialog).evaluate().isEmpty, timeout: const Duration(seconds: 30));

      await container.read(downloadRepositoryProvider).deleteRecordsWithIds([remoteImage.id]);
      final localAssetIdsBeforeDownload = (await _localAssets(container)).map((asset) => asset.id).toSet();
      await _showViewerControls(tester, container);
      ...
      final downloadedLocal = await _waitForNewLocalAsset(
        container,
        localAssetIdsBeforeDownload,
        tester,
        reason: 'Expected the 041 viewer download to create a local Photos asset',
      );
    ...
    _realStackSessionTest('MOB-UI-042-$_caseSuffix', 'imports shared media through the share intent upload flow', (
      ...
      final searchApi = apiService.searchApi;
      final createdRemoteAssetIds = <String>{};
      final fixtureRoot = await _createShareIntentFixtureDirectory('immich-share-intent-042-');
    ...
    _realStackSessionTest('MOB-UI-051-$_caseSuffix', 'browses local albums, server folders, and local timeline', (
      ...
      final createdCameraAsset = await _saveLocalTestImage(
        container,
        createdLocalAssetIds,
      ...
      final createdNestedAsset = await _saveLocalTestImage(
        container,
        createdLocalAssetIds,
      ...
      final createdSameAssetA = await _saveLocalTestImage(
        container,
        createdLocalAssetIds,
      ...
      final createdSameAssetB = await _saveLocalTestImage(
        container,
        createdLocalAssetIds,
      ...
      final cameraAsset = await _waitForLocalAssetById(
        container,
        createdCameraAsset.id,
        tester,
        reason: 'Expected the 051 camera fixture to be discovered after PhotoKit saved it',
      );
      final nestedAsset = await _waitForLocalAssetById(
        container,
        createdNestedAsset.id,
        tester,
        reason: 'Expected the 051 nested fixture to be discovered after PhotoKit saved it',
      );
      final sameAssetA = await _waitForLocalAssetById(
        container,
        createdSameAssetA.id,
        tester,
        reason: 'Expected the 051 duplicate-name fixture A to be discovered after PhotoKit saved it',
      );
      final sameAssetB = await _waitForLocalAssetById(
        container,
        createdSameAssetB.id,
        tester,
        reason: 'Expected the 051 duplicate-name fixture B to be discovered after PhotoKit saved it',
      );
      final localAlbums = await _waitForLocalAlbumFixtures(
        tester,
        container,
        requiredNames: {cameraAlbumName, nestedAlbumName, sameAlbumName},
        duplicatedName: Platform.isIOS ? null : sameAlbumName,
      );
      ...
      if (Platform.isIOS && sameAlbums.length == 1) {
        expect(sameAlbums.single.assetCount, 2);
      } else {
      ...
      await _waitForLocalAssetById(
        container,
        refreshAsset.id,
        tester,
        reason: 'Expected the 051 refresh fixture to be discovered after PhotoKit saved it',
      );
      ...
      await _waitForLocalAssetGoneById(container, refreshAsset.id, tester);
    ...
    _realStackSessionTest('MOB-UI-052-$_caseSuffix', 'browses and manages memories', (tester) async {
      ...
      final providerMemories = await _waitForMemoryProvider(
        tester,
        container,
        (items) => items.any((memory) => memory.id == targetMemory.id),
        reason: 'Expected the 052 memory provider to expose the synced target memory',
      );
    ...
    _realStackSessionTest('MOB-UI-059-$_caseSuffix', 'applies search filters and clears them', (tester) async {
      tester.view.devicePixelRatio = 1.0;
      tester.view.physicalSize = const Size(430, 932);
      addTearDown(tester.view.reset);
      ...
      final createdAlbum = await albumsApi.createAlbum(
        api.CreateAlbumDto(albumName: albumName, assetIds: api.Optional.present([targetId])),
      );
      expect(createdAlbum, isNotNull);
      albumId = createdAlbum!.id;
      ...
      final fullServer = await _waitForServerSearchResponse(
        ...
        reason: 'Expected all 059 server filters to intersect on the target asset only',
      );
      final fullAppIds = await _waitForSearchServiceAssetIds(
        ...
        reason: 'Expected all 059 app filters to intersect on the target asset only',
      );
      ...
      final notInAlbumServer = await _waitForServerSearchResponse(
        ...
        reason: 'Expected not-in-album filter to exclude the album member and include the loose image',
      );
      ...
      final archiveServer = await _waitForServerSearchResponse(
        ...
        reason: 'Expected display archive filter to find the 059 archived image',
      );
      ...
      await _waitForPaginatedSearchAssetIds(
        tester,
        container,
        (ids) => ids.containsAll({targetId, controlId, videoId}) && !ids.contains(archiveId),
        reason: 'Expected initial 059 UI filename search to include timeline fixtures only',
      );

      await _openSearchFilterSheet(tester, const Key('media_type_chip'));
      await _tapHitTestableFinder(tester, find.byKey(const Key('search-media-type-image')));
      await _applySearchFilterSheet(tester);
      await _waitForPaginatedSearchAssetIds(
        tester,
        container,
        (ids) => ids.containsAll({targetId, controlId}) && !ids.contains(videoId) && !ids.contains(archiveId),
        reason: 'Expected UI media type image filter to exclude the 059 video',
      );
      await pumpUntilFound(tester, find.text('image'.tr()), timeout: const Duration(seconds: 30));

      await _openSearchFilterSheet(tester, const Key('search-filter-chip-display'));
      await _tapHitTestableFinder(tester, find.byKey(const Key('search-display-favorite')));
      await _applySearchFilterSheet(tester);
      await _waitForPaginatedSearchAssetIds(
        tester,
        container,
        (ids) => ids.contains(targetId) && ids.length == 1,
        reason: 'Expected UI favorite display filter to narrow 059 results to the favorite image',
      );
      await pumpUntilFound(tester, find.text('favorite'.tr()), timeout: const Duration(seconds: 30));

      await _openSearchFilterSheet(tester, const Key('search-filter-chip-rating'));
      await _tapHitTestableFinder(tester, find.byKey(const Key('search-rating-4')));
      await _applySearchFilterSheet(tester);
      await _waitForPaginatedSearchAssetIds(
        tester,
        container,
      ...
      await _waitForPaginatedSearchAssetIds(
        ...
        container,
        (ids) => ids.containsAll({targetId, controlId}) && !ids.contains(videoId) && !ids.contains(archiveId),
        reason: 'Expected clearing rating/display filters to restore 059 image results',
      );

      await _openSearchFilterSheet(tester, const Key('search-filter-chip-display'));
      await _tapHitTestableFinder(tester, find.byKey(const Key('search-display-not-in-album')));
      await _applySearchFilterSheet(tester);
      await _waitForPaginatedSearchAssetIds(
        tester,
        container,
        (ids) => ids.contains(controlId) && !ids.contains(targetId) && !ids.contains(videoId),
        reason: 'Expected UI not-in-album filter to exclude the 059 album member',
      );
    ...
    _realStackSessionTest('MOB-UI-060-$_caseSuffix', 'browses and filters map locations', (tester) async {
      tester.view.devicePixelRatio = 1.0;
      tester.view.physicalSize = const Size(430, 932);
      addTearDown(tester.view.reset);
      ...
      final bayBounds = LatLngBounds(southwest: const LatLng(37.0, -123.0), northeast: const LatLng(38.2, -121.5));
      final tokyoBounds = LatLngBounds(southwest: const LatLng(35.0, 139.0), northeast: const LatLng(36.2, 140.3));
      ...
      final targetId = await uploadMapAsset('target', baseCreatedAt, location: targetLocation, isFavorite: true);
      ...
      final oldId = await uploadMapAsset('old', oldCreatedAt, location: const LatLng(37.7800, -122.4100));
      ...
      expect(_serverSearchAssetIds(citySearch), containsAll({targetId, sameCoordinateId, oldId}));
      ...
      await _waitForMapMarkerAssetIds(
        tester,
        container,
        (ids) => ids.contains(targetId) && !ids.contains(sameCoordinateId) && !ids.contains(oldId),
        bounds: bayBounds,
      ...
      final router = container.read(appRouterProvider);
      unawaited(router.push(DriftPlaceRoute(currentLocation: targetLocation)));
      await pumpUntilFound(tester, find.byType(DriftPlacePage), timeout: const Duration(seconds: 30));
      await pumpUntilFound(tester, find.text(city), timeout: const Duration(seconds: 30));
      await _tapHitTestableFinder(tester, find.text(city), reason: 'Expected 060 place tile to be tappable');
      await pumpUntilFound(tester, find.byType(DriftPlaceDetailPage), timeout: const Duration(seconds: 30));
      await pumpUntilFound(tester, find.text(city), timeout: const Duration(seconds: 30));
      await router.maybePop();
      await router.maybePop();

      unawaited(router.push(DriftMapRoute(initialLocation: targetLocation)));
      await pumpUntilFound(tester, find.byType(DriftMapPage), timeout: const Duration(seconds: 30));
      await pumpUntilFound(
        tester,
      ...
      final favoriteOnlyTile = find.byKey(const Key('map-settings-favorite-only'));
      ...
      final selectedLocation = router.push<LatLng>(MapLocationPickerRoute(initialLatLng: targetLocation));
      await pumpUntilFound(tester, find.byType(MapLocationPickerPage), timeout: const Duration(seconds: 30));
      await pumpUntilFound(tester, find.text('37.7749, -122.4194'), timeout: const Duration(seconds: 30));
      await _tapHitTestableFinder(
        tester,
    ...
    _realStackSessionTest('MOB-UI-061-$_caseSuffix', 'manages people details, merges people, and hides people', (
      tester,
    ) async {
      tester.view.devicePixelRatio = 1.0;
      tester.view.physicalSize = const Size(430, 932);
      ...
      Future<String> uploadPersonAsset(String label, String personId, int index) async {
        final createdAt = baseCreatedAt.add(Duration(minutes: index));
        final assetId = await _uploadGeneratedJpegAsSecondClient(
      ...
      final hiddenAssetId = await uploadPersonAsset('hidden', hiddenPersonId, 4);
      ...
      final initialPeople = await _waitForLocalPeopleState(
        ...
        (people) =>
            ...
            people.any((person) => person.id == unnamedPersonId && person.name.isEmpty),
        reason: 'Expected 061 visible and unnamed people to sync into the people collection',
      );
      expect(initialPeople.map((person) => person.id), contains(primaryPersonId));
      expect(initialPeople.map((person) => person.id), contains(unnamedPersonId));
      ...
      final primaryPerson = initialPeople.singleWhere((person) => person.id == primaryPersonId);
      ...
      await tester.enterText(find.byKey(const Key('person-edit-name-field')), editedName);
      await _tapHitTestableFinder(
        tester,
      ...
      final editedPrimary = await _waitForLocalPersonState(
        ...
        reason: 'Expected 061 UI edits to persist to the local primary person',
      );
      ...
      await _waitForServerPeopleState(
        ...
        reason: 'Expected 061 UI edits to persist to the server primary person',
      );

      _expectBulkSuccess(
        await peopleApi.mergePerson(primaryPersonId, api.MergePersonDto(ids: [mergePersonId])),
        {mergePersonId},
      ...
      for (final assetId in mergeAssetIds) {
        await _waitForAssetPeopleState(
          ...
          (people) =>
              people.any((person) => person.id == primaryPersonId) &&
              !people.any((person) => person.id == mergePersonId),
          reason: 'Expected 061 merged asset $assetId to belong to the primary person locally',
        );
        expect(await _remoteAssetFaceRowCount(container.read(driftProvider), assetId, primaryPersonId), 1);
        expect(await _remoteAssetFaceRowCount(container.read(driftProvider), assetId, mergePersonId), 0);
      }
      ...
      await _expectTimelineAssetSet(
        ...
        reason: 'Expected 061 person timeline to include primary and merged assets only',
      );

      _expectBulkSuccess(
        await peopleApi.updatePeople(
          api.PeopleUpdateDto(
            people: [api.PeopleUpdateItem(id: hiddenPersonId, isHidden: const api.Optional.present(true))],
          ),
        ),
        {hiddenPersonId},
      ...
      final visibleAfterHide = await _waitForLocalPeopleState(
        ...
        (people) =>
            ...
            !people.any((person) => person.id == hiddenPersonId) &&
            !people.any((person) => person.id == mergePersonId),
        reason: 'Expected 061 hidden and merged people to disappear from the local collection',
      );
      expect(visibleAfterHide.map((person) => person.id), isNot(contains(hiddenPersonId)));
      expect(
        await _waitForAssetPeopleState(
          tester,
          peopleService,
          ...
          reason: 'Expected 061 hidden person to be filtered from asset people locally',
        ),
        isEmpty,
      );
      ...
      await _waitForServerPeopleState(
        ...
        peopleApi,
        withHidden: true,
        matches: (people) => people.any((person) => person.id == hiddenPersonId && person.isHidden),
      ...
      await _waitForServerPeopleState(
        ...
        peopleApi,
        withHidden: false,
        matches: (people) => !people.any((person) => person.id == hiddenPersonId),
        reason: 'Expected 061 visible server people list to exclude hidden person',
      );
      ...
      await _waitForLocalPersonState(
        ...
        (person) =>
            ...
            person.birthDate!.month == birthday.month &&
            person.birthDate!.day == birthday.day,
        reason: 'Expected 061 edited name and birthday to persist after restart',
      );
      await _waitForLocalPeopleState(
        ...
        (people) =>
            ...
            !people.any((person) => person.id == hiddenPersonId) &&
            !people.any((person) => person.id == mergePersonId),
        reason: 'Expected 061 hidden and merged people to remain filtered after restart',
      );
    });
    ...
    _realStackSessionTest(
      ...
      (tester) async {
        ...
        await _waitForSuccessfulResponse(
          tester,
          () => _authenticatedApiGet('/assets/$remoteId/thumbnail?size=thumbnail&edited=false&c=$runToken'),
          timeout: const Duration(minutes: 3),
        );
        ...
        final localOnly = await _waitForLocalAssetById(
          ...
          createdLocal.id,
          tester,
          reason: 'Expected the 064 local fixture to be discovered after PhotoKit saved it',
        );
        ...
        final remoteAsset = await _waitForRemoteAssetState(
          ...
          remoteId,
          (asset) => asset.isRemoteOnly,
          reason: 'Expected the 064 diagnostic image to sync as remote-only before download',
        );

        final assetCounts = await container.read(assetServiceProvider).getAssetCounts();
        expect(assetCounts.$1, greaterThanOrEqualTo(1), reason: 'Expected local sync diagnostics to see local assets');
        expect(
          assetCounts.$2,
        ...
        await _tapTextEntryInPage(tester, pageType: SyncStatusPage, label: 'sync_local'.tr());
        await _tapTextEntryInPage(tester, pageType: SyncStatusPage, label: 'sync_remote'.tr());
        expect(tester.takeException(), isNull);
        ...
        await _popUntilVisible(tester, MainTimelinePage);
        final retainedLocal = await _waitForLocalAssetById(
          container,
          localOnly.id,
          tester,
          reason: 'Expected the 064 local fixture to remain after opening cleanup preview',
        );
        expect(retainedLocal.id, localOnly.id, reason: 'Cleanup preview must not delete media without confirmation');

        await downloadRepository.deleteRecordsWithIds([remoteAsset.id]);
        final localAssetIdsBeforeDownload = (await _localAssets(container)).map((asset) => asset.id).toSet();
        final acceptedDownloads = await downloadRepository.downloadAllAssets([remoteAsset]);
        expect(acceptedDownloads, equals([true]), reason: 'Expected 064 remote diagnostic image download to enqueue');
        final downloadRecord = await _waitForDownloadRecordStatus(
          tester,
          remoteAsset.id,
          TaskStatus.complete,
        ...
        await _waitForDownloadUpdateCount(
          ...
          TaskStatus.complete,
          1,
          reason: 'Expected 064 download service to emit a completed image update',
        );
        final downloadedLocal = await _waitForNewLocalAsset(
          container,
          localAssetIdsBeforeDownload,
          tester,
          reason: 'Expected the 064 diagnostic download to create a local Photos asset',
        );
        ...
        expect(
          ...
          isFalse,
          reason: 'Expected the 064 diagnostic log message to avoid the current test credential',
        );
        ...
        await _pumpUntilFoundWithReason(
          ...
          reason: 'Expected 064 asset troubleshoot page to render asset metadata',
          timeout: const Duration(seconds: 30),
        );
        await _pumpUntilFoundWithReason(
          tester,
          find.text('matching_assets'.tr()),
          reason: 'Expected 064 asset troubleshoot page to render matching assets',
          timeout: const Duration(seconds: 30),
        );
        await _popUntilVisible(tester, MainTimelinePage);
    ...
    _realStackSessionTest(
      ...
      (tester) async {
        ...
        final createdLocal = await container
            .read(fileMediaRepositoryProvider)
            .saveLocalAsset(sourceBytes, title: localName, relativePath: 'Pictures/ImmichE2E065');
        expect(createdLocal, isNotNull, reason: 'Expected PhotoManager to inject the 065 gallery fixture');
        createdLocalAssetIds.add(createdLocal!.id);

        await container.read(backgroundSyncProvider).syncLocal(full: true);
        final localAsset = await _waitForLocalAssetByIdState(
          container,
          createdLocal.id,
          tester,
          (asset) => asset.isImage && asset.contentSize != null && asset.contentSize! > 0,
          reason: 'Expected the 065 gallery fixture to sync with uploadable metadata',
        );
        expect(localAsset.isLocalOnly, isTrue);
        debugPrint(
          'MOB-MEDIA-065-$_caseSuffix local fixture '
          'createdId=${createdLocal.id} syncedId=${localAsset.id} '
          'requestedFilename=$localName syncedFilename=${localAsset.name}',
        );
        ...
        if (!Platform.isIOS) {
          expect(
            localMd5,
            expectedSourceMd5,
            reason: 'Expected Android MediaStore to preserve the generated 065 JPEG bytes',
          );
        }
        ...
        debugPrint(
          'MOB-MEDIA-065-$_caseSuffix upload callback '
          'remoteId=$remoteAssetId localId=${localAsset.id} '
          'localFilename=${localAsset.name}',
        );
        ...
        final info = await _waitForAssetInfoState(
          ...
          (asset) =>
              ...
              asset.originalFileName.isNotEmpty &&
              asset.width != null &&
              asset.height != null,
          reason: 'Expected the 065 uploaded photo to expose server metadata',
        );
        final serverIds = await _waitForServerAssetIdsByOriginalFilename(
          ...
          reason: 'Expected the 065 uploaded photo to be indexed by its server filename',
        );
        ...
        final original = await _waitForSuccessfulResponse(
          tester,
          () => container.read(assetApiRepositoryProvider).downloadAsset(remoteAssetId!, edited: false),
          timeout: const Duration(minutes: 3),
        );
        ...
        final syncedAsset = await _waitForRemoteAssetState(
          ...
          container,
          remoteAssetId!,
          (asset) => asset.isImage && asset.hasLocal && asset.hasRemote && asset.checksum == localMd5,
          reason: 'Expected the 065 upload to sync back as one merged local/remote asset',
        );

        final timeline = container.read(timelineFactoryProvider).main([syncedAsset.ownerId]);
        addTearDown(timeline.dispose);
        final timelineAssets = await _expectTimelineAssetSet(
          tester,
          timeline,
          ...
          excludes: const {},
          reason: 'Expected the 065 uploaded photo to appear in the client timeline',
        );
        final timelineMatches = timelineAssets
            .where((asset) => _timelineAssetId(asset) == remoteAssetId || asset.refersToSameAsset(syncedAsset))
            .toList();
        expect(timelineMatches, hasLength(1), reason: 'Expected exactly one visible client asset for the 065 upload');
        ...
        debugPrint(
          'MOB-MEDIA-065-$_caseSuffix uploaded remoteId=$remoteAssetId '
          'requestedFilename=$localName '
          'serverFilename=${info.originalFileName} '
          'syncedFilename=${syncedAsset.name} '
          'md5=$localMd5 size=${localBytes.length}',
        );
      },
    );

    _realStackSessionTest('MOB-MEDIA-066-$_caseSuffix', 'uploads a batch of gallery photos idempotently', (
      tester,
    ) async {
      tester.view.devicePixelRatio = 1.0;
      tester.view.physicalSize = const Size(430, 932);
      ...
      final requestedNameByLocalId = <String, String>{};
      ...
      for (var i = 0; i < 20; i++) {
        ...
        final created = await container
            .read(fileMediaRepositoryProvider)
            .saveLocalAsset(bytes, title: name, relativePath: 'Pictures/ImmichE2E066');
        expect(created, isNotNull);
        createdLocalAssetIds.add(created!.id);
        requestedNameByLocalId[created.id] = name;
      }
      ...
      final localAssets = await _waitForLocalAssetsByIds(
        ...
        createdLocalAssetIds,
        tester,
        reason: 'Expected all 066 gallery fixtures to sync with uploadable metadata',
      );
      ...
      for (final asset in localAssets) {
        ...
        expectedByLocalId[asset.id] = (md5: base64Encode(md5.convert(bytes).bytes), size: bytes.length);
      ...
      for (final entry in uploaded.entries) {
        ...
        final info = await _waitForAssetInfoState(
          ...
          (asset) =>
              asset.type == api.AssetTypeEnum.IMAGE &&
              asset.originalFileName == requestedNameByLocalId[entry.key] &&
              asset.width != null &&
              asset.height != null,
          reason: 'Expected the 066 uploaded asset ${entry.value} to expose server metadata',
        );
        expect(info.checksum, expected.md5);
        final original = await _waitForSuccessfulResponse(
          tester,
          () => container.read(assetApiRepositoryProvider).downloadAsset(entry.value, edited: false),
          timeout: const Duration(minutes: 3),
        );
      ...
      for (final remoteId in uploaded.values.take(3)) {
        final thumbnail = await _waitForSuccessfulResponse(
          tester,
          () => assetsApi.viewAssetWithHttpInfo(remoteId, size: api.AssetMediaSize.thumbnail),
          timeout: const Duration(minutes: 3),
        );
        expect(thumbnail.bodyBytes, isNotEmpty);
      }
      ...
      for (final entry in uploaded.entries) {
        ...
        await _waitForRemoteAssetState(
          ...
          container,
          entry.value,
          (asset) => asset.isImage && asset.hasLocal && asset.hasRemote && asset.checksum == expected.md5,
          reason: 'Expected the 066 uploaded asset ${entry.value} to sync as merged',
        );
      }

      final ownerId = (await container.read(remoteAssetRepositoryProvider).get(uploaded.values.first))!.ownerId;
      ...
      final timelineAssets = await _expectTimelineAssetSet(
        ...
        excludes: const {},
        reason: 'Expected all 066 batch uploads to appear in the client timeline',
      );
      ...
      debugPrint(
        'MOB-MEDIA-066-$_caseSuffix uploaded count=${uploaded.length} '
    ...
    _realStackSessionTest(
      ...
      (tester) async {
        ...
        final sourceBytes = _generatedMp4Bytes('MOB-MEDIA-067-$_caseSuffix-$runToken');
        expect(
          await _serverAssetIdsByOriginalFilename(searchApi, localName, type: api.AssetTypeEnum.VIDEO),
          isEmpty,
        ...
        final localAsset = await _waitForLocalAssetByIdState(
          ...
          createdLocal.id,
          tester,
          (asset) => asset.isVideo && asset.contentSize != null && asset.contentSize! > 0,
          reason: 'Expected the 067 gallery video fixture to sync with uploadable metadata',
        );
        expect(localAsset.isLocalOnly, isTrue);
        ...
        final serverIds = await _waitForServerAssetIdsByOriginalFilename(
          ...
          (ids) => ids.contains(remoteAssetId),
          type: api.AssetTypeEnum.VIDEO,
          reason: 'Expected the 067 uploaded video to be indexed by filename on the real server',
        );
        expect(serverIds, hasLength(1));
        ...
        final info = await _waitForAssetInfoState(
          ...
          (asset) =>
              ...
              asset.width != null &&
              asset.height != null,
          reason: 'Expected the 067 uploaded video to expose processed server metadata',
        );
        ...
        final rangedPlayback = await _waitForSuccessfulResponse(
          tester,
          () => http.get(
            Uri.parse('${Store.get(StoreKey.serverEndpoint)}/assets/$remoteAssetId/video/playback'),
            headers: {
              ...ApiService.getRequestHeaders(),
              'Authorization': 'Bearer ${Store.get(StoreKey.accessToken)}',
        ...
        final syncedAsset = await _waitForRemoteAssetState(
          ...
          reason: 'Expected the 067 upload to sync back as one merged local/remote video asset',
        );
        ...
        final timelineAssets = await _expectTimelineAssetSet(
          ...
          includes: {remoteAssetId!},
          excludes: const {},
          reason: 'Expected the 067 uploaded video to appear in the client timeline',
        );
        final timelineMatches = timelineAssets
            .where((asset) => _timelineAssetId(asset) == remoteAssetId || asset.refersToSameAsset(syncedAsset))
            .toList();
        ...
        await _openTimelineAsset(tester, syncedAsset);
        await pumpUntilFound(tester, find.byType(AssetViewer), timeout: const Duration(seconds: 60));
        expect(container.read(assetViewerProvider).currentAsset?.remoteId, remoteAssetId);
        await pumpUntilFound(tester, find.byType(NativeVideoViewer), timeout: const Duration(seconds: 60));
        await _waitForVideoState(
          tester,
          container,
        ...
        await _waitForVideoPositionAfter(tester, container, remoteAssetId!, Duration.zero);
    ...
    _realStackSessionTest('MOB-MEDIA-068-$_caseSuffix', 'uploads a Live Photo/Motion Photo pair and keeps it linked', (
      tester,
    ) async {
      tester.view.devicePixelRatio = 1.0;
      tester.view.physicalSize = const Size(430, 932);
      ...
      final runToken = DateTime.now().toUtc().microsecondsSinceEpoch.toString();
      final useAndroidMotionHeicFixture = Platform.isAndroid && _media068AndroidMotionHeicBase64.isNotEmpty;
      ...
      final motionBytes = useAndroidMotionHeicFixture
          ? const <int>[]
          : _generatedMp4Bytes('MOB-MEDIA-068-$_caseSuffix-$runToken');
      if (!useAndroidMotionHeicFixture) {
        expect(
          await _serverAssetIdsByOriginalFilename(searchApi, localName, type: api.AssetTypeEnum.IMAGE),
          isEmpty,
      ...
      final localAsset = await _waitForLocalAssetByIdState(
        ...
        reason: 'Expected the 068 local gallery fixture to sync as a Live Photo/Motion Photo',
      );
      expect(localAsset.isLocalOnly, isTrue);

      final liveFiles = await StorageRepository().getLivePhotoFilesForAsset(localAsset);
          ...
          .uploadSingleAsset(
            ...
            callbacks: UploadCallbacks(
              onProgress: (id, fileName, bytes, totalBytes) {
                ...
                final assetProgress = progressById.putIfAbsent(progressKey, () => []);
                expect(progress, inInclusiveRange(0.0, 1.0));
                if (assetProgress.isNotEmpty) {
                  expect(progress, greaterThanOrEqualTo(assetProgress.last));
                }
      ...
      if (useAndroidMotionHeicFixture) {
        ...
        _expectLivePhotoProgressNames(progressNames, stillName: localName, motionName: motionName);
      }
      createdRemoteAssetIds.add(remoteAssetId!);
      ...
      if (!useAndroidMotionHeicFixture) {
        final serverIds = await _waitForServerAssetIdsByOriginalFilename(
          ...
          (ids) => ids.contains(remoteAssetId),
          type: api.AssetTypeEnum.IMAGE,
          reason: 'Expected the 068 uploaded Live Photo still to be indexed by filename',
        );
        expect(serverIds, hasLength(1));
      }
      ...
      final info = await _waitForAssetInfoState(
        ...
        (asset) =>
            ...
            asset.width != null &&
            asset.height != null,
        reason: 'Expected the 068 uploaded still to expose a linked motion video',
      );
      if (useAndroidMotionHeicFixture) {
      ...
      final motionInfo = await _waitForAssetInfoState(
        ...
        reason: 'Expected the 068 motion component to upload as a playable video asset',
      );
      expect(motionInfo.id, livePhotoVideoId);
      if (useAndroidMotionHeicFixture) {
        expect(motionInfo.originalFileName, isNotEmpty);
      } else {
        expect(motionInfo.originalFileName, anyOf(localName, motionName));
      }
      ...
      if (useAndroidMotionHeicFixture) {
        ...
        expect(base64Encode(md5.convert(originalStill.bodyBytes).bytes), stillMd5);
      }
      ...
      final syncedStill = await _waitForRemoteAssetState(
        ...
        reason: 'Expected the 068 still to sync back as one merged Live Photo asset',
      );
      await _waitForRemoteAssetState(
        tester,
        container,
        ...
        reason: 'Expected the 068 hidden motion video to sync locally as hidden',
      );

      final timeline = container.read(timelineFactoryProvider).main([syncedStill.ownerId]);
      addTearDown(timeline.dispose);
      final timelineAssets = await _expectTimelineAssetSet(
        tester,
        timeline,
        ...
        reason: 'Expected the 068 timeline to show only the Live Photo still asset',
      );
      final timelineMatches = timelineAssets
          .where((asset) => _timelineAssetId(asset) == remoteAssetId || asset.refersToSameAsset(syncedStill))
          .toList();
      expect(timelineMatches, hasLength(1));
      ...
      final duplicateRemoteId = await _uploadSingleAssetToServer(container, localAsset);
      expect(duplicateRemoteId, remoteAssetId);
    ...
    _realStackSessionTest('MOB-MEDIA-069-$_caseSuffix', 'keeps repeated photo video and live uploads idempotent', (
      tester,
    ) async {
      tester.view.devicePixelRatio = 1.0;
      tester.view.physicalSize = const Size(430, 932);
      ...
      final createdRemoteAssetIds = <String>{};
      final createdLocalAssetIds = <String>{};
      final fixtureRoot = await _createShareIntentFixtureDirectory('immich-duplicate-069-');
      ...
      final videoBytes = _generatedMp4Bytes('MOB-MEDIA-069-$_caseSuffix-video-$runToken');
      final liveStillBytes = _generatedJpegBytes(runToken.hashCode + 69);
      final liveMotionBytes = _generatedMp4Bytes('MOB-MEDIA-069-$_caseSuffix-live-$runToken');
      ...
      expect(
        await _serverAssetIdsByOriginalFilename(searchApi, videoName, type: api.AssetTypeEnum.VIDEO),
        isEmpty,
      ...
      final photoEntity = await container
          .read(fileMediaRepositoryProvider)
          .saveLocalAsset(photoBytes, title: photoName, relativePath: 'Pictures/ImmichE2E069');
      expect(photoEntity, isNotNull, reason: 'Expected PhotoManager to save the 069 photo fixture');
      ...
      await container.read(backgroundSyncProvider).syncLocal(full: true);
      final photoAsset = await _waitForLocalAssetByIdState(
        container,
        photoEntity.id,
        tester,
      ...
      final videoAsset = await _waitForLocalAssetByIdState(
        container,
        videoEntity.id,
        tester,
      ...
      final liveAsset = await _waitForLocalAssetByIdState(
        container,
        liveEntity.id,
        tester,
        (asset) => asset.isImage && asset.isMotionPhoto,
      ...
      Future<({String remoteId, Map<String, List<double>> progress})> uploadAndCollect(
        LocalAsset asset,
        String label,
      ) async {
        String? remoteAssetId;
        ...
        await container
            ...
            .uploadSingleAsset(
              ...
              callbacks: UploadCallbacks(
                onProgress: (_, fileName, bytes, totalBytes) {
                  final progress = totalBytes > 0 ? bytes / totalBytes : 0.0;
                  final fileProgress = progressByName.putIfAbsent(fileName, () => []);
                  expect(progress, inInclusiveRange(0.0, 1.0));
        ...
        expect(remoteAssetId, isNotNull, reason: 'Expected $label upload to return a remote id');
      ...
      createdRemoteAssetIds.addAll([firstPhoto.remoteId, firstVideo.remoteId, firstLive.remoteId]);
      ...
      await expectServerIds(photoName, api.AssetTypeEnum.IMAGE, {
        firstPhoto.remoteId,
      }, reason: 'Expected the 069 photo to create exactly one server image');
      await expectServerIds(videoName, api.AssetTypeEnum.VIDEO, {
        firstVideo.remoteId,
      }, reason: 'Expected the 069 video to create exactly one server video');
      await expectServerIds(liveName, api.AssetTypeEnum.IMAGE, {
        firstLive.remoteId,
      ...
      notifier.onSharedMedia([
        await _shareIntentAttachment(sharePhotoFile, ShareIntentAttachmentType.image),
        await _shareIntentAttachment(shareVideoFile, ShareIntentAttachmentType.video),
      ]);
      ...
      await tester.binding.handlePopRoute();
      await _pumpFor(tester, const Duration(seconds: 1));
      await pumpUntilFound(tester, find.byType(Timeline), timeout: const Duration(seconds: 30));
      ...
      final syncedPhoto = await _waitForRemoteAssetState(
        ...
        container,
        firstPhoto.remoteId,
        (asset) => asset.isImage && asset.hasRemote && asset.checksum == firstPhotoInfo.checksum,
      ...
      await _waitForRemoteAssetState(
        ...
        container,
        firstVideo.remoteId,
        (asset) => asset.isVideo && asset.hasRemote && asset.checksum == firstVideoInfo.checksum,
      ...
      await _waitForRemoteAssetState(
        ...
        container,
        firstLive.remoteId,
        (asset) => asset.isImage && asset.isMotionPhoto && asset.hasRemote && asset.livePhotoVideoId == liveMotionId,
      ...
      await _waitForRemoteAssetState(
        ...
        liveMotionId,
        (asset) => asset.isVideo && asset.visibility == AssetVisibility.hidden,
        reason: 'Expected the 069 duplicate Live Photo motion asset to remain hidden',
      );

      final timeline = container.read(timelineFactoryProvider).main([syncedPhoto.ownerId]);
      addTearDown(timeline.dispose);
      final timelineAssets = await _expectTimelineAssetSet(
        tester,
        timeline,
        includes: {firstPhoto.remoteId, firstVideo.remoteId, firstLive.remoteId},
        excludes: {liveMotionId},
        reason: 'Expected the 069 timeline to show one photo, one video, and one Live still',
      );
      final visibleUploadedIds = _timelineAssetIds(
        timelineAssets,
      ).intersection({firstPhoto.remoteId, firstVideo.remoteId, firstLive.remoteId, liveMotionId});
      expect(visibleUploadedIds, {firstPhoto.remoteId, firstVideo.remoteId, firstLive.remoteId});
    ...
    _realStackSessionTest(
      ...
      (tester) async {
        ...
        final photoBytes = _padFixtureBytes(_generatedJpegBytes(seed), _media070ResumableFixtureBytes, seed);
        final videoBytes = _padFixtureBytes(
        ...
        final photoEntity = await container
            .read(fileMediaRepositoryProvider)
            .saveLocalAsset(photoBytes, title: photoName, relativePath: 'Pictures/ImmichE2E070');
        expect(photoEntity, isNotNull, reason: 'Expected PhotoManager to save the 070 photo fixture');
        createdLocalAssetIds.add(photoEntity!.id);
        ...
        Future<({String remoteId, Map<String, dynamic>? state})> interruptResumeAndVerify(
          LocalAsset asset, {
          required String label,
          required String serverFilename,
          required api.AssetTypeEnum serverType,
          bool allowFirstPassCompletion = false,
        }) async {
          await _clearResumableStateFiles();
          final uploadService = container.read(foregroundUploadServiceProvider);
          uploadService.shouldAbortUpload = false;
          ...
          expect(interruptedError, isNull, reason: 'Expected $label cancellation to be non-error');
          if (interruptedRemoteId != null) {
            expect(allowFirstPassCompletion, isTrue, reason: 'Expected $label first upload to stop before completion');
            final completedRemoteId = interruptedRemoteId!;
            await _waitForServerAssetIdsByOriginalFilename(
              tester,
              searchApi,
              serverFilename,
              (ids) => ids.length == 1 && ids.contains(completedRemoteId),
              type: serverType,
              reason: 'Expected completed first-pass $label upload to create one asset',
            );
            expect(await _resumableStateFiles(), isEmpty);
            await _expectDuplicateUploadReusesRemoteId(
              tester,
              ...
              expectedRemoteId: completedRemoteId,
            );
            return (remoteId: completedRemoteId, state: null);
          }
          expect(
            firstProgressByName.values.expand((values) => values).any((bytes) => bytes >= _resumableCancelAfterBytes),
            isTrue,
          ...
          expect(
            await _serverAssetIdsByOriginalFilename(searchApi, serverFilename, type: serverType),
            isEmpty,
          ...
          expect(remoteAssetId, isNotNull, reason: 'Expected $label retry to return a remote id');
          ...
          final serverIds = await _waitForServerAssetIdsByOriginalFilename(
            ...
            searchApi,
            serverFilename,
            (ids) => ids.length == 1 && ids.contains(resumedRemoteId),
            type: serverType,
          ...
          await _expectDuplicateUploadReusesRemoteId(
            ...
            serverFilename: serverFilename,
            serverType: serverType,
            expectedRemoteId: resumedRemoteId,
          );
        ...
        final liveResult = await interruptResumeAndVerify(
          ...
          serverType: api.AssetTypeEnum.IMAGE,
          allowFirstPassCompletion: Platform.isIOS,
        );
        ...
        final photoInfo = await _waitForAssetInfoState(
          ...
          (asset) => asset.type == api.AssetTypeEnum.IMAGE && asset.originalFileName == photoName,
        ...
        final videoInfo = await _waitForAssetInfoState(
          ...
          (asset) => asset.type == api.AssetTypeEnum.VIDEO && asset.originalFileName == videoName,
        ...
        final syncedPhoto = await _waitForRemoteAssetState(
          ...
          container,
          photoResult.remoteId,
          (asset) => asset.isImage && asset.hasRemote && asset.checksum == photoInfo.checksum,
        ...
        await _waitForRemoteAssetState(
          ...
          container,
          videoResult.remoteId,
          (asset) => asset.isVideo && asset.hasRemote && asset.checksum == videoInfo.checksum,
        ...
        final timelineAssets = await _expectTimelineAssetSet(
          tester,
          timeline,
          includes: {photoResult.remoteId, videoResult.remoteId, liveResult.remoteId},
          excludes: {liveMotionId},
          reason: 'Expected the 070 timeline to show only resumed photo/video/Live still assets',
        );
        final visibleUploadedIds = _timelineAssetIds(
          timelineAssets,
        ).intersection({photoResult.remoteId, videoResult.remoteId, liveResult.remoteId, liveMotionId});
        expect(visibleUploadedIds, {photoResult.remoteId, videoResult.remoteId, liveResult.remoteId});
        expect(tester.takeException(), isNull);
        ...
        debugPrint(
          ...
          'motionOffset=${liveResult.state?['offset'] ?? 'first-pass-complete'}',
        );
      },
    );

    _realStackSessionTest('MOB-MEDIA-071-$_caseSuffix', 'recovers mixed upload queue after client and server restart', (
      tester,
    ) async {
      tester.view.devicePixelRatio = 1.0;
      tester.view.physicalSize = const Size(430, 932);
      ...
      final liveMotionBytes = _generatedMp4Bytes('MOB-MEDIA-071-$_caseSuffix-live-$runToken');

      final photoEntity = await container
          .read(fileMediaRepositoryProvider)
          .saveLocalAsset(photoBytes, title: photoName, relativePath: 'Pictures/ImmichE2E071');
      expect(photoEntity, isNotNull, reason: 'Expected PhotoManager to save the 071 photo fixture');
      createdLocalAssetIds.add(photoEntity!.id);
      final videoEntity = await _saveLocalTestVideo(
        container,
        createdLocalAssetIds,
      ...
      await _pumpUntil(
        tester,
        () => restartMarkerPrinted || firstVideoRemoteId != null || firstUploadError != null,
        timeout: const Duration(seconds: 90),
      );
      expect(
        restartMarkerPrinted,
        isTrue,
        reason: 'Expected 071 upload progress before asking the host harness to restart the server',
      );

      await _waitForServerReachability(tester, realEndpoint, reachable: false, timeout: const Duration(seconds: 45));
      await _waitForServerReachability(tester, realEndpoint, reachable: true, timeout: const Duration(seconds: 120));
      await _waitForAuthenticatedUserEndpoint(tester, realEndpoint, timeout: const Duration(seconds: 60));

      await firstUpload.timeout(
        const Duration(seconds: 150),
        onTimeout: () => fail('Timed out waiting for the 071 in-flight upload to settle after server restart'),
      );
      expect(firstProgress, isNotEmpty);
      if (firstVideoRemoteId == null) {
        expect(
          firstUploadError,
          isNotNull,
          reason: 'Expected the interrupted 071 video upload to report a recoverable error',
        );
        expect(
          await _resumableStateFiles(),
          isNotEmpty,
          reason: 'Interrupted 071 video upload should persist resumable state',
        );
      }
      ...
      await container.read(backgroundSyncProvider).syncLocal(full: true);
      final restartedPhotoAsset = await _waitForLocalAssetByIdState(
        container,
        photoEntity.id,
        tester,
        (asset) => asset.isImage && !asset.isMotionPhoto,
        reason: 'Expected the 071 photo fixture to survive app reload',
      );
      final restartedVideoAsset = await _waitForLocalAssetByIdState(
        container,
        videoEntity.id,
        tester,
        (asset) => asset.isVideo,
        reason: 'Expected the 071 video fixture to survive app reload',
      );
      final restartedLiveAsset = await _waitForLocalAssetByIdState(
        container,
        liveEntity.id,
        tester,
        (asset) => asset.isImage && asset.isMotionPhoto,
        reason: 'Expected the 071 Live Photo fixture to survive app reload',
      );
      ...
      await recoveredUploadService.uploadManual(
        [if (firstVideoRemoteId == null) restartedVideoAsset, restartedPhotoAsset, restartedLiveAsset],
        callbacks: UploadCallbacks(
          onProgress: (_, fileName, bytes, totalBytes) {
            final progress = recoveryProgressByName.putIfAbsent(fileName, () => []);
            expect(bytes, inInclusiveRange(0, totalBytes));
            if (progress.isNotEmpty) {
          ...
          onSuccess: (localId, remoteId) => completedIdsByLocalId[localId] = remoteId,
          onError: (localId, errorMessage) => recoveryErrors[localId] = errorMessage,
        ),
      );
      expect(recoveryErrors, isEmpty, reason: 'Expected 071 recovered upload queue to finish cleanly');

      final videoRemoteId = firstVideoRemoteId ?? completedIdsByLocalId[restartedVideoAsset.localId];
      ...
      createdRemoteAssetIds.addAll([recoveredVideoRemoteId, recoveredPhotoRemoteId, recoveredLiveRemoteId]);

      expect(await _resumableStateFiles(), isEmpty, reason: '071 recovered queue should clear resumable state files');
      final syncedAfterRecovery = await container.read(syncStreamServiceProvider).sync();
      ...
      await _waitForServerAssetIdsByOriginalFilename(
        ...
        reason: 'Expected 071 Live Photo still to have one server asset after recovery',
      );
      ...
      await _waitForRemoteAssetState(
        ...
        container,
        recoveredLiveRemoteId,
        (asset) => asset.hasRemote && asset.isImage && asset.isMotionPhoto && asset.livePhotoVideoId == liveMotionId,
      ...
      final recoveredRows = await _remoteSyncRowCounts(drift);
      expect(recoveredRows['remote_asset_entity'], greaterThanOrEqualTo(baselineRows['remote_asset_entity']! + 3));

      final timeline = container.read(timelineFactoryProvider).main([localVideo.ownerId]);
      addTearDown(timeline.dispose);
      await _expectTimelineAssetSet(
        tester,
        timeline,
        includes: {recoveredPhotoRemoteId, recoveredVideoRemoteId, recoveredLiveRemoteId},
        excludes: {liveMotionId},
        reason: 'Expected 071 timeline to show recovered photo, video, and Live still only',
      );
      expect(tester.takeException(), isNull);
    ...
    _realStackSessionTest('MOB-MEDIA-072-$_caseSuffix', 'moves a photo to trash without mutating the original file', (
      tester,
    ) async {
      await _loadAuthenticatedApp(
        tester,
      ...
      final createdLocal = await container
          .read(fileMediaRepositoryProvider)
          .saveLocalAsset(sourceBytes, title: fileName, relativePath: 'Pictures/ImmichE2E072');
      expect(createdLocal, isNotNull, reason: 'Expected PhotoManager to inject the 072 gallery fixture');
      createdLocalAssetIds.add(createdLocal!.id);

      await container.read(backgroundSyncProvider).syncLocal(full: true);
      final localUploadSource = await _waitForLocalAssetByIdState(
        container,
        createdLocal.id,
        tester,
        (asset) => asset.isImage && asset.isLocalOnly && asset.contentSize != null && asset.contentSize! > 0,
        reason: 'Expected 072 gallery fixture to sync with uploadable metadata',
      );
      final localFile = await container.read(storageRepositoryProvider).getFileForAsset(localUploadSource.id);
      expect(localFile, isNotNull);
      final localBytes = await localFile!.readAsBytes();
      ...
      final localPhoto = await _waitForRemoteAssetState(
        ...
        container,
        photoId,
        (asset) => asset.visibility == AssetVisibility.timeline && asset.hasRemote && asset.isImage && !asset.isTrashed,
      ...
      final beforeInfo = await _waitForAssetInfoState(
        ...
        assetsApi,
        photoId,
        (asset) => asset.type == api.AssetTypeEnum.IMAGE && !asset.isTrashed && asset.originalPath.isNotEmpty,
        reason: 'Expected 072 server photo to be active before trash',
      );
      final beforeDownload = await _waitForSuccessfulResponse(
        tester,
        () => container.read(assetApiRepositoryProvider).downloadAsset(photoId, edited: false),
      );
      final beforeChecksum = base64Encode(md5.convert(beforeDownload.bodyBytes).bytes);
      ...
      final trashedInfo = await _waitForAssetInfoState(
        ...
        assetsApi,
        photoId,
        (asset) => asset.isTrashed && asset.originalPath == beforeInfo.originalPath && asset.checksum == beforeChecksum,
        reason: 'Expected 072 server photo to be logically trashed without file mutation',
      );
      ...
      final trashedLocalPhoto = await _waitForRemoteAssetState(
        ...
        container,
        photoId,
        (asset) => asset.hasRemote && asset.isImage && asset.isTrashed && asset.checksum == beforeChecksum,
      ...
      debugPrint(
        '$markerCaseId:TRASHED ${jsonEncode({'assetId': photoId, 'fileName': fileName, 'checksum': beforeChecksum, 'originalPath': beforeInfo.originalPath, 'downloadBytes': beforeDownload.bodyBytes.length})}',
      );
      await _pumpFor(tester, const Duration(seconds: 12));
      expect(tester.takeException(), isNull);
    });

    _realStackSessionTest('MOB-MEDIA-073-$_caseSuffix', 'moves a video to trash without deleting playback files', (
      tester,
    ) async {
      await _loadAuthenticatedApp(
        tester,
      ...
      final sourceBytes = _generatedMp4Bytes('MOB-MEDIA-073-$_caseSuffix-$runToken');
      expect(
        await _serverAssetIdsByOriginalFilename(searchApi, localName, type: api.AssetTypeEnum.VIDEO),
        isEmpty,
      ...
      await container.read(backgroundSyncProvider).syncLocal(full: true);
      final localVideoSource = await _waitForLocalAssetByIdState(
        container,
        createdLocal.id,
        tester,
        (asset) => asset.isVideo && asset.isLocalOnly && asset.contentSize != null && asset.contentSize! > 0,
        reason: 'Expected 073 gallery video fixture to sync with uploadable metadata',
      );

      final localFile = await container.read(storageRepositoryProvider).getFileForAsset(localVideoSource.id);
      expect(localFile, isNotNull);
      final localBytes = await localFile!.readAsBytes();
      final localMd5 = base64Encode(md5.convert(localBytes).bytes);
      ...
      final original = await _waitForSuccessfulResponse(
        tester,
        () => container.read(assetApiRepositoryProvider).downloadAsset(videoId, edited: false),
        timeout: const Duration(minutes: 3),
      );
      expect(original.bodyBytes.length, localBytes.length);
      expect(base64Encode(md5.convert(original.bodyBytes).bytes), localMd5);
      ...
      final trashedInfo = await _waitForAssetInfoState(
        ...
        (asset) =>
            ...
            asset.checksum == localMd5,
        reason: 'Expected 073 server video to be logically trashed without file mutation',
      );
      ...
      final trashedLocalVideo = await _waitForRemoteAssetState(
        ...
        container,
        videoId,
        (asset) => asset.hasRemote && asset.isVideo && asset.isTrashed && asset.checksum == localMd5,
      ...
      await _expectTimelineAssetSet(
        ...
        includes: const {},
        excludes: {videoId},
        reason: 'Expected 073 trashed video to disappear from video collection',
      );
      await _expectTimelineAssetSet(
        tester,
        trashTimeline,
      ...
      debugPrint(
        '$markerCaseId:TRASHED ${jsonEncode({'assetId': videoId, 'fileName': localName, 'checksum': localMd5, 'originalPath': beforeInfo.originalPath, 'downloadBytes': original.bodyBytes.length, 'playbackBytes': playback.bodyBytes.length})}',
      );
      await _pumpFor(tester, const Duration(seconds: 12));
      expect(tester.takeException(), isNull);
    });

    _realStackSessionTest('MOB-MEDIA-074-$_caseSuffix', 'moves a Live Photo to trash as one linked asset', (
      tester,
    ) async {
      await _loadAuthenticatedApp(
        tester,
      ...
      final liveMotionBytes = _generatedMp4Bytes('MOB-MEDIA-074-$_caseSuffix-live-$runToken');
      expect(
        await _serverAssetIdsByOriginalFilename(searchApi, liveName),
        isEmpty,
      ...
      await container.read(backgroundSyncProvider).syncLocal(full: true);
      final localLive = await _waitForLocalAssetByIdState(
        container,
        createdLocal.id,
        tester,
        (asset) => asset.isImage && asset.isMotionPhoto,
      ...
      final liveFiles = await StorageRepository().getLivePhotoFilesForAsset(localLive);
      expect(liveFiles, isNotNull);
      expect(liveFiles!.still.existsSync(), isTrue);
      expect(liveFiles.motion.existsSync(), isTrue);
      final stillBytes = await liveFiles.still.readAsBytes();
      ...
      await container
          ...
          .uploadSingleAsset(
            ...
            callbacks: UploadCallbacks(
              onProgress: (_, fileName, bytes, totalBytes) {
                final progress = totalBytes > 0 ? bytes / totalBytes : 0.0;
                final assetProgress = progressByName.putIfAbsent(fileName, () => []);
                expect(progress, inInclusiveRange(0.0, 1.0));
                if (assetProgress.isNotEmpty) {
                  expect(progress, greaterThanOrEqualTo(assetProgress.last));
                }
      ...
      _expectLivePhotoProgressNames(progressByName.keys, stillName: liveName, motionName: motionName);
      ...
      expect(
        ...
        reason: 'Server-backed Live Photo motion assets may expose the still original name',
      );

      final originalStill = await _waitForSuccessfulResponse(
        tester,
        () => container.read(assetApiRepositoryProvider).downloadAsset(liveId, edited: false),
        timeout: const Duration(minutes: 3),
      );
      final stillServerMd5 = base64Encode(md5.convert(originalStill.bodyBytes).bytes);
      ...
      final motionServerMd5 = base64Encode(md5.convert(motionPlayback.bodyBytes).bytes);
      ...
      await _waitForRemoteAssetState(
        ...
        container,
        motionId,
        (asset) => asset.isVideo && asset.visibility == AssetVisibility.hidden && !asset.isTrashed,
      ...
      final trashedInfo = await _waitForAssetInfoState(
        ...
        reason: 'Expected 074 Live Photo still to be trashed without file mutation',
      );
      expect(trashedInfo.originalFileName, beforeInfo.originalFileName);
      await _waitForAssetInfoState(
        tester,
      ...
      debugPrint(
        '$markerCaseId:TRASHED ${jsonEncode({'assetId': liveId, 'motionAssetId': motionId, 'fileName': liveName, 'motionName': motionName, 'stillChecksum': stillServerMd5, 'motionChecksum': motionServerMd5, 'stillBytes': originalStill.bodyBytes.length, 'motionBytes': motionPlayback.bodyBytes.length})}',
      );
      await _pumpFor(tester, const Duration(seconds: 12));
      expect(tester.takeException(), isNull);
    });
    ...
    _realStackSessionTest(
      'MOB-MEDIA-075-$_caseSuffix',
      'moves a mixed batch of photos, videos, and a Live Photo to trash atomically',
      (tester) async {
        await _loadAuthenticatedApp(
          tester,
          overrideCancellation: true,
        ...
        final createdRemoteAssetIds = <String>{};
        final createdLocalAssetIds = <String>{};
        final photoLocalIdsByName = <String, String>{};
        final videoLocalIdsByName = <String, String>{};
        ...
        final photoFixtures = [
          for (var index = 0; index < 3; index++)
            (name: 'immich-e2e-trash-075-photo-${index + 1}-$runToken.jpg', bytes: _generatedJpegBytes(seed + index)),
        ];
        final videoFixtures = [
          for (var index = 0; index < 2; index++)
            (
              name: 'immich-e2e-trash-075-video-${index + 1}-$runToken.mp4',
              bytes: _generatedMp4Bytes('MOB-MEDIA-075-$_caseSuffix-video-${index + 1}-$runToken'),
            ),
        ];
        final liveName = 'immich-e2e-trash-075-live-$runToken.jpg';
        final motionName = liveName.replaceFirst(RegExp(r'\.[^.]+$'), '.mp4');
        final liveStillBytes = _generatedJpegBytes(seed + 750);
        final liveMotionBytes = _generatedMp4Bytes('MOB-MEDIA-075-$_caseSuffix-live-$runToken');

        for (final fixture in photoFixtures) {
          expect(
            await _serverAssetIdsByOriginalFilename(searchApi, fixture.name),
            isEmpty,
            reason: 'The 075 photo filename ${fixture.name} must be unique before upload',
          );
          final created = await container
              .read(fileMediaRepositoryProvider)
              .saveLocalAsset(fixture.bytes, title: fixture.name, relativePath: 'Pictures/ImmichE2E075');
          expect(created, isNotNull, reason: 'Expected PhotoManager to save 075 photo ${fixture.name}');
          createdLocalAssetIds.add(created!.id);
          photoLocalIdsByName[fixture.name] = created.id;
        }
        for (final fixture in videoFixtures) {
          expect(
            await _serverAssetIdsByOriginalFilename(searchApi, fixture.name, type: api.AssetTypeEnum.VIDEO),
            isEmpty,
            reason: 'The 075 video filename ${fixture.name} must be unique before upload',
          );
          final created = await _saveLocalTestVideo(
            container,
            createdLocalAssetIds,
            title: fixture.name,
            relativePath: Platform.isAndroid ? 'Movies/ImmichE2E075' : null,
            bytes: fixture.bytes,
          );
          videoLocalIdsByName[fixture.name] = created.id;
        }
        expect(
          await _serverAssetIdsByOriginalFilename(searchApi, liveName),
          isEmpty,
          reason: 'The 075 Live Photo filename must be unique before upload',
        );
        final liveEntity = await _saveLocalTestLivePhoto(
          container,
          createdLocalAssetIds,
          title: liveName,
          relativePath: Platform.isAndroid ? 'Pictures/ImmichE2E075' : null,
          imageBytes: liveStillBytes,
          videoBytes: liveMotionBytes,
        );

        await container.read(backgroundSyncProvider).syncLocal(full: true);
        final photoLocalAssets = <String, LocalAsset>{};
        final videoLocalAssets = <String, LocalAsset>{};
        for (final fixture in photoFixtures) {
          photoLocalAssets[fixture.name] = await _waitForLocalAssetByIdState(
            container,
            photoLocalIdsByName[fixture.name]!,
            tester,
            (asset) =>
                asset.isImage &&
                !asset.isMotionPhoto &&
                ...
                asset.contentSize != null &&
                asset.contentSize! > 0,
            reason: 'Expected 075 photo fixture ${fixture.name} to sync locally',
          );
        }
        for (final fixture in videoFixtures) {
          videoLocalAssets[fixture.name] = await _waitForLocalAssetByIdState(
            container,
            videoLocalIdsByName[fixture.name]!,
            tester,
            (asset) => asset.isVideo && asset.isLocalOnly && asset.contentSize != null && asset.contentSize! > 0,
            reason: 'Expected 075 video fixture ${fixture.name} to sync locally',
          );
        }
        final liveLocal = await _waitForLocalAssetByIdState(
          container,
          liveEntity.id,
          tester,
          (asset) => asset.isImage && asset.isMotionPhoto && asset.isLocalOnly,
          reason: 'Expected 075 Live Photo fixture to sync locally',
        );

        Future<({String remoteId, Map<String, List<double>> progress})> uploadFixture(
          LocalAsset asset,
          String label,
        ) async {
          final progressByName = <String, List<double>>{};
          ...
          expect(uploadError, isNull, reason: 'Expected $label upload to succeed');
          expect(remoteAssetId, isNotNull, reason: 'Expected $label upload to return a remote id');
          expect(progressByName, isNotEmpty);
          return (remoteId: remoteAssetId!, progress: progressByName);
        }

        final photoIdsByName = <String, String>{};
        final videoIdsByName = <String, String>{};
        for (final fixture in photoFixtures) {
          final upload = await uploadFixture(photoLocalAssets[fixture.name]!, '075 photo ${fixture.name}');
          photoIdsByName[fixture.name] = upload.remoteId;
          createdRemoteAssetIds.add(upload.remoteId);
        }
        for (final fixture in videoFixtures) {
          final upload = await uploadFixture(videoLocalAssets[fixture.name]!, '075 video ${fixture.name}');
          videoIdsByName[fixture.name] = upload.remoteId;
          createdRemoteAssetIds.add(upload.remoteId);
        }
        final liveUpload = await uploadFixture(liveLocal, '075 Live Photo');
        final liveId = liveUpload.remoteId;
        createdRemoteAssetIds.add(liveId);
        _expectLivePhotoProgressNames(liveUpload.progress.keys, stillName: liveName, motionName: motionName);

        for (final entry in photoIdsByName.entries) {
          await _waitForServerAssetIdsByOriginalFilename(
            tester,
            searchApi,
            entry.key,
            (ids) => ids.length == 1 && ids.contains(entry.value),
            reason: 'Expected 075 photo ${entry.key} to be discoverable',
          );
        }
        for (final entry in videoIdsByName.entries) {
          await _waitForServerAssetIdsByOriginalFilename(
            tester,
            searchApi,
            entry.key,
            (ids) => ids.length == 1 && ids.contains(entry.value),
            type: api.AssetTypeEnum.VIDEO,
            reason: 'Expected 075 video ${entry.key} to be discoverable',
          );
        }
        await _waitForServerAssetIdsByOriginalFilename(
          tester,
          ...
          liveName,
          (ids) => ids.length == 1 && ids.contains(liveId),
          reason: 'Expected 075 Live Photo still to be discoverable',
        );

        final photoMetadata = <Map<String, Object?>>[];
        for (final entry in photoIdsByName.entries) {
          final info = await _waitForAssetInfoState(
            tester,
            assetsApi,
            entry.value,
            (asset) => asset.type == api.AssetTypeEnum.IMAGE && asset.originalFileName == entry.key && !asset.isTrashed,
            reason: 'Expected 075 photo ${entry.key} to be active before trash',
          );
          final original = await _waitForSuccessfulResponse(
            tester,
            () => container.read(assetApiRepositoryProvider).downloadAsset(entry.value, edited: false),
          ...
          final checksum = base64Encode(md5.convert(original.bodyBytes).bytes);
          expect(checksum, info.checksum);
          photoMetadata.add({
            'assetId': entry.value,
            'fileName': entry.key,
            'checksum': checksum,
            'originalPath': info.originalPath,
            'bytes': original.bodyBytes.length,
          });
        }

        final videoMetadata = <Map<String, Object?>>[];
        for (final entry in videoIdsByName.entries) {
          final info = await _waitForAssetInfoState(
            tester,
            assetsApi,
            ...
            (asset) =>
                asset.type == api.AssetTypeEnum.VIDEO &&
                asset.originalFileName == entry.key &&
                !asset.isTrashed &&
                asset.duration != null &&
                asset.duration! > 0,
            reason: 'Expected 075 video ${entry.key} to be active before trash',
          );
          final original = await _waitForSuccessfulResponse(
            tester,
            () => container.read(assetApiRepositoryProvider).downloadAsset(entry.value, edited: false),
          ...
          final playback = await _waitForSuccessfulResponse(
            tester,
            () => assetsApi.playAssetVideoWithHttpInfo(entry.value),
            acceptedStatusCodes: const {200, 206},
            timeout: const Duration(minutes: 3),
          );
          videoMetadata.add({
        ...
        final liveInfo = await _waitForAssetInfoState(
          ...
          (asset) =>
              ...
              !asset.isTrashed &&
              asset.livePhotoVideoId.orElse(null) != null,
          reason: 'Expected 075 Live Photo still to be active before trash',
        );
        final motionId = liveInfo.livePhotoVideoId.orElse(null);
        expect(motionId, isNotNull);
        expect(motionId, isNot(liveId));
        createdRemoteAssetIds.add(motionId!);
        await _waitForAssetInfoState(
          ...
          motionId,
          (asset) => asset.type == api.AssetTypeEnum.VIDEO && !asset.isTrashed,
          reason: 'Expected 075 linked motion asset before trash',
        );
        final originalStill = await _waitForSuccessfulResponse(
          tester,
          () => container.read(assetApiRepositoryProvider).downloadAsset(liveId, edited: false),
        ...
        final liveStillChecksum = base64Encode(md5.convert(originalStill.bodyBytes).bytes);
        expect(liveStillChecksum, liveInfo.checksum);
        final motionPlayback = await _waitForSuccessfulResponse(
          tester,
          () => assetsApi.playAssetVideoWithHttpInfo(motionId),
          acceptedStatusCodes: const {200, 206},
        ...
        final liveMotionChecksum = base64Encode(md5.convert(motionPlayback.bodyBytes).bytes);

        await _resetAndSyncRemoteState(tester, container);
        final syncedPhotoIds = <String>{};
        for (final entry in photoIdsByName.entries) {
          final synced = await _waitForRemoteAssetState(
            tester,
            container,
            entry.value,
            (asset) => asset.isImage && !asset.isMotionPhoto && asset.hasRemote && !asset.isTrashed,
            reason: 'Expected 075 photo ${entry.key} to sync before trash',
          );
          expect(synced.ownerId, ownerId);
          syncedPhotoIds.add(entry.value);
          expect(await _remoteAssetRowCountById(drift, entry.value), 1);
        }
        final syncedVideoIds = <String>{};
        for (final entry in videoIdsByName.entries) {
          final synced = await _waitForRemoteAssetState(
            tester,
            container,
            entry.value,
            (asset) => asset.isVideo && asset.hasRemote && !asset.isTrashed && asset.duration > Duration.zero,
            reason: 'Expected 075 video ${entry.key} to sync before trash',
          );
          expect(synced.ownerId, ownerId);
          syncedVideoIds.add(entry.value);
          expect(await _remoteAssetRowCountById(drift, entry.value), 1);
        }
        final syncedLive = await _waitForRemoteAssetState(
          tester,
          ...
          (asset) =>
              ...
              !asset.isTrashed &&
              asset.livePhotoVideoId == motionId,
          reason: 'Expected 075 Live Photo still to sync before trash',
        );
        expect(syncedLive.ownerId, ownerId);
        await _waitForRemoteAssetState(
          tester,
          container,
          motionId,
          (asset) => asset.isVideo && asset.visibility == AssetVisibility.hidden && !asset.isTrashed,
          reason: 'Expected 075 motion asset to sync hidden before trash',
        );
        expect(await _remoteAssetRowCountById(drift, liveId), 1);
        expect(await _remoteAssetRowCountById(drift, motionId), 1);

        final expectedTrashIds = {...syncedPhotoIds, ...syncedVideoIds, liveId};
        final allKnownIds = {...expectedTrashIds, motionId};
        expect(expectedTrashIds, hasLength(6));

        final timelineFactory = container.read(timelineFactoryProvider);
        final mainTimeline = timelineFactory.main([ownerId]);
        final videoTimeline = timelineFactory.video(ownerId);
        final trashTimeline = timelineFactory.trash(ownerId);
        addTearDown(mainTimeline.dispose);
        addTearDown(videoTimeline.dispose);
        addTearDown(trashTimeline.dispose);
        ...
        await _expectTimelineAssetSet(
          tester,
          mainTimeline,
          includes: expectedTrashIds,
          excludes: {motionId},
          reason: 'Expected 075 mixed batch assets to be visible before trash',
        );
        await _expectTimelineAssetSet(
          tester,
          videoTimeline,
          includes: syncedVideoIds,
          excludes: {motionId},
          reason: 'Expected 075 uploaded videos to be visible before trash without the hidden motion asset',
        );
        await _expectTimelineAssetSet(
          tester,
          trashTimeline,
          includes: const {},
          excludes: allKnownIds,
          reason: 'Expected 075 mixed batch to be absent from trash initially',
        );

        await assetService.trash(expectedTrashIds.toList());
        for (final metadata in photoMetadata) {
          await _waitForAssetInfoState(
            ...
            (asset) =>
                asset.type == api.AssetTypeEnum.IMAGE &&
                asset.isTrashed &&
                asset.originalPath == metadata['originalPath'] &&
                asset.checksum == metadata['checksum'],
            reason: 'Expected 075 photo ${metadata['fileName']} to be logically trashed',
          );
        }
        for (final metadata in videoMetadata) {
          await _waitForAssetInfoState(
            tester,
            assetsApi,
            metadata['assetId']! as String,
            (asset) =>
                ...
                asset.originalPath == metadata['originalPath'] &&
                asset.checksum == metadata['checksum'],
            reason: 'Expected 075 video ${metadata['fileName']} to be logically trashed',
          );
        }
        await _waitForAssetInfoState(
          tester,
          ...
          (asset) =>
              asset.type == api.AssetTypeEnum.IMAGE &&
              asset.isTrashed &&
              asset.originalPath == liveInfo.originalPath &&
              asset.checksum == liveStillChecksum &&
              asset.livePhotoVideoId.orElse(null) == motionId,
          reason: 'Expected 075 Live Photo still to be trashed',
        );
        await _waitForAssetInfoState(
          tester,
          assetsApi,
          motionId,
          (asset) => asset.type == api.AssetTypeEnum.VIDEO && asset.isTrashed,
          reason: 'Expected 075 Live Photo motion to be trashed atomically',
        );

        await _resetAndSyncRemoteState(tester, container);
        for (final assetId in syncedPhotoIds) {
          await _waitForRemoteAssetState(
            tester,
            container,
            assetId,
            (asset) => asset.hasRemote && asset.isImage && asset.isTrashed,
            reason: 'Expected 075 local photo $assetId to be trashed',
          );
          expect(await _remoteAssetRowCountById(drift, assetId), 1);
        }
        for (final assetId in syncedVideoIds) {
          await _waitForRemoteAssetState(
            tester,
            container,
            assetId,
            (asset) => asset.hasRemote && asset.isVideo && asset.isTrashed,
            reason: 'Expected 075 local video $assetId to be trashed',
          );
          expect(await _remoteAssetRowCountById(drift, assetId), 1);
        }
        await _waitForRemoteAssetState(
          ...
          (asset) =>
              ...
              asset.isTrashed &&
              asset.livePhotoVideoId == motionId,
          reason: 'Expected 075 local Live Photo still to be trashed',
        );
        await _waitForRemoteAssetState(
          tester,
          container,
          motionId,
          (asset) => asset.isVideo && asset.isTrashed,
          reason: 'Expected 075 local motion asset to be trashed',
        );
        expect(await _remoteAssetRowCountById(drift, liveId), 1);
        expect(await _remoteAssetRowCountById(drift, motionId), 1);
        ...
        await _expectTimelineAssetSet(
          ...
          includes: const {},
          excludes: allKnownIds,
          reason: 'Expected 075 mixed batch to leave the main timeline',
        );
        await _expectTimelineAssetSet(
          tester,
          videoTimeline,
          includes: const {},
          excludes: {...syncedVideoIds, motionId},
          reason: 'Expected 075 trashed videos to leave the video collection',
        );
        final trashAssets = await _expectTimelineAssetSet(
          tester,
          trashTimeline,
          includes: expectedTrashIds,
          excludes: {motionId},
          reason: 'Expected 075 trash to contain six logical assets without the hidden motion asset',
        );
        expect(
          _timelineAssetIds(trashAssets).intersection(allKnownIds),
          expectedTrashIds,
          reason: 'Expected 075 trash timeline to expose exactly the six logical assets from this batch',
        );
        ...
        for (final entry in photoIdsByName.entries) {
          await _waitForServerAssetIdsByOriginalFilename(
            ...
            searchApi,
            entry.key,
            (ids) => !ids.contains(entry.value),
            reason: 'Expected 075 active search to filter trashed photo ${entry.key}',
          );
        }
        for (final entry in videoIdsByName.entries) {
          await _waitForServerAssetIdsByOriginalFilename(
            ...
            (ids) => !ids.contains(entry.value),
            type: api.AssetTypeEnum.VIDEO,
            reason: 'Expected 075 active search to filter trashed video ${entry.key}',
          );
        }
        await _waitForServerAssetIdsByOriginalFilename(
          tester,
          ...
          liveName,
          (ids) => !ids.contains(liveId),
          reason: 'Expected 075 active search to filter trashed Live Photo',
        );

        const markerCaseId = 'MOB-MEDIA-075-$_caseSuffix';
        debugPrint(
          '$markerCaseId:TRASHED ${jsonEncode({
            'photoAssets': photoMetadata,
            'videoAssets': videoMetadata,
            'liveAsset': {'assetId': liveId, 'motionAssetId': motionId, 'fileName': liveName, 'motionName': motionName, 'stillChecksum': liveStillChecksum, 'motionChecksum': liveMotionChecksum, 'originalPath': liveInfo.originalPath, 'stillBytes': originalStill.bodyBytes.length, 'motionBytes': motionPlayback.bodyBytes.length},
            'expectedTrashIds': expectedTrashIds.toList()..sort(),
          })}',
        );
        await _pumpFor(tester, const Duration(seconds: 12));
        expect(tester.takeException(), isNull);
    ...
    _realStackSessionTest(
      'MOB-MEDIA-076-$_caseSuffix',
      'restores photos, videos, and Live Photos from trash without mutating files',
      (tester) async {
        await _loadAuthenticatedApp(
          tester,
          overrideCancellation: true,
        ...
        final assetsApi = apiService.assetsApi;
        final searchApi = apiService.searchApi;
        final trashApi = apiService.trashApi;
        final assetService = container.read(assetServiceProvider);
        final user = Store.tryGet(StoreKey.currentUser);
        expect(user, isNotNull);
        final ownerId = user!.id;
        ...
        final runToken = DateTime.now().toUtc().microsecondsSinceEpoch.toString();
        final seed = runToken.hashCode;
        final photoName = 'immich-e2e-restore-076-photo-$runToken.jpg';
        final videoName = 'immich-e2e-restore-076-video-$runToken.mp4';
        final liveName = 'immich-e2e-restore-076-live-$runToken.jpg';
        final motionName = liveName.replaceFirst(RegExp(r'\.[^.]+$'), '.mp4');

        expect(
          await _serverAssetIdsByOriginalFilename(searchApi, photoName),
          isEmpty,
          reason: 'The 076 photo filename must be unique before upload',
        );
        expect(
          await _serverAssetIdsByOriginalFilename(searchApi, videoName, type: api.AssetTypeEnum.VIDEO),
          isEmpty,
          reason: 'The 076 video filename must be unique before upload',
        );
        expect(
          await _serverAssetIdsByOriginalFilename(searchApi, liveName),
          isEmpty,
          reason: 'The 076 Live Photo filename must be unique before upload',
        );

        final createdPhoto = await container
            .read(fileMediaRepositoryProvider)
            .saveLocalAsset(_generatedJpegBytes(seed + 760), title: photoName, relativePath: 'Pictures/ImmichE2E076');
        expect(createdPhoto, isNotNull, reason: 'Expected PhotoManager to save the 076 photo fixture');
        final photoEntity = createdPhoto!;
        createdLocalAssetIds.add(photoEntity.id);
        final videoEntity = await _saveLocalTestVideo(
          container,
          createdLocalAssetIds,
          title: videoName,
          relativePath: Platform.isAndroid ? 'Movies/ImmichE2E076' : null,
          bytes: _generatedMp4Bytes('MOB-MEDIA-076-$_caseSuffix-video-$runToken'),
        );
        final liveEntity = await _saveLocalTestLivePhoto(
          container,
          createdLocalAssetIds,
          title: liveName,
          relativePath: Platform.isAndroid ? 'Pictures/ImmichE2E076' : null,
          imageBytes: _generatedJpegBytes(seed + 761),
          videoBytes: _generatedMp4Bytes('MOB-MEDIA-076-$_caseSuffix-live-$runToken'),
        );

        await container.read(backgroundSyncProvider).syncLocal(full: true);
        final localPhoto = await _waitForLocalAssetByIdState(
          container,
          photoEntity.id,
          tester,
          (asset) =>
              ...
              asset.contentSize != null &&
              asset.contentSize! > 0,
          reason: 'Expected 076 photo fixture to sync locally',
        );
        final localVideo = await _waitForLocalAssetByIdState(
          container,
          videoEntity.id,
          tester,
          (asset) => asset.isVideo && asset.isLocalOnly && asset.contentSize != null && asset.contentSize! > 0,
          reason: 'Expected 076 video fixture to sync locally',
        );
        final localLive = await _waitForLocalAssetByIdState(
          container,
          liveEntity.id,
          tester,
          (asset) => asset.isImage && asset.isMotionPhoto && asset.isLocalOnly,
          reason: 'Expected 076 Live Photo fixture to sync locally',
        );
        ...
        final photoUpload = await uploadFixture(localPhoto, '076 photo');
        final photoId = photoUpload.remoteId;
        createdRemoteAssetIds.add(photoId);
        final videoUpload = await uploadFixture(localVideo, '076 video');
        final videoId = videoUpload.remoteId;
        createdRemoteAssetIds.add(videoId);
        final liveUpload = await uploadFixture(localLive, '076 Live Photo');
        final liveId = liveUpload.remoteId;
        createdRemoteAssetIds.add(liveId);
        _expectLivePhotoProgressNames(liveUpload.progress.keys, stillName: liveName, motionName: motionName);
        ...
        await _waitForServerAssetIdsByOriginalFilename(
          ...
          photoName,
          (ids) => ids.length == 1 && ids.contains(photoId),
          reason: 'Expected 076 photo to be discoverable before trash',
        );
        await _waitForServerAssetIdsByOriginalFilename(
          tester,
          searchApi,
          ...
          (ids) => ids.length == 1 && ids.contains(videoId),
          type: api.AssetTypeEnum.VIDEO,
          reason: 'Expected 076 video to be discoverable before trash',
        );
        await _waitForServerAssetIdsByOriginalFilename(
          tester,
          searchApi,
          liveName,
          (ids) => ids.length == 1 && ids.contains(liveId),
          reason: 'Expected 076 Live Photo to be discoverable before trash',
        );
        ...
        Future<void> rememberDownload(String assetId) async {
          final download = await _waitForSuccessfulResponse(
            tester,
            () => container.read(assetApiRepositoryProvider).downloadAsset(assetId, edited: false),
            timeout: const Duration(minutes: 3),
          ...
          beforeDownloads[assetId] = base64Encode(md5.convert(download.bodyBytes).bytes);
          beforeBytes[assetId] = download.bodyBytes.length;
        }
        ...
        final photoInfo = await _waitForAssetInfoState(
          ...
          assetsApi,
          photoId,
          (asset) => asset.type == api.AssetTypeEnum.IMAGE && asset.originalFileName == photoName && !asset.isTrashed,
          reason: 'Expected 076 photo to be active before trash',
        );
        beforeInfos[photoId] = photoInfo;
        await rememberDownload(photoId);
        expect(beforeDownloads[photoId], photoInfo.checksum);
        ...
        final videoInfo = await _waitForAssetInfoState(
          ...
          (asset) =>
              ...
              asset.duration != null &&
              asset.duration! > 0,
          reason: 'Expected 076 video to be active before trash',
        );
        beforeInfos[videoId] = videoInfo;
        await rememberDownload(videoId);
        expect(beforeDownloads[videoId], videoInfo.checksum);
        final videoPlayback = await _waitForSuccessfulResponse(
          tester,
          () => assetsApi.playAssetVideoWithHttpInfo(videoId),
          acceptedStatusCodes: const {200, 206},
          timeout: const Duration(minutes: 3),
        );
        expect(videoPlayback.bodyBytes, isNotEmpty);
        ...
        final liveInfo = await _waitForAssetInfoState(
          ...
          (asset) =>
              ...
              !asset.isTrashed &&
              asset.livePhotoVideoId.orElse(null) != null,
          reason: 'Expected 076 Live Photo still to be active before trash',
        );
        beforeInfos[liveId] = liveInfo;
        await rememberDownload(liveId);
        expect(beforeDownloads[liveId], liveInfo.checksum);
        final motionId = liveInfo.livePhotoVideoId.orElse(null);
        expect(motionId, isNotNull);
        expect(motionId, isNot(liveId));
        createdRemoteAssetIds.add(motionId!);
        final motionInfo = await _waitForAssetInfoState(
          ...
          motionId,
          (asset) => asset.type == api.AssetTypeEnum.VIDEO && !asset.isTrashed,
          reason: 'Expected 076 linked motion asset before trash',
        );
        beforeInfos[motionId] = motionInfo;
        final motionPlayback = await _waitForSuccessfulResponse(
          tester,
          () => assetsApi.playAssetVideoWithHttpInfo(motionId),
          acceptedStatusCodes: const {200, 206},
        ...
        beforeDownloads[motionId] = base64Encode(md5.convert(motionPlayback.bodyBytes).bytes);
        beforeBytes[motionId] = motionPlayback.bodyBytes.length;
        ...
        await _waitForRemoteAssetState(
          ...
          (asset) =>
              ...
              asset.hasRemote &&
              !asset.isTrashed &&
              asset.checksum == beforeDownloads[photoId],
          reason: 'Expected 076 photo to sync before trash',
        );
        await _waitForRemoteAssetState(
          tester,
          container,
          ...
          (asset) =>
              ...
              asset.hasRemote &&
              !asset.isTrashed &&
              asset.checksum == beforeDownloads[videoId] &&
              asset.duration > Duration.zero,
          reason: 'Expected 076 video to sync before trash',
        );
        await _waitForRemoteAssetState(
          tester,
          container,
          ...
          (asset) =>
              ...
              asset.hasRemote &&
              !asset.isTrashed &&
              asset.checksum == beforeDownloads[liveId] &&
              asset.livePhotoVideoId == motionId,
          reason: 'Expected 076 Live Photo still to sync before trash',
        );
        await _waitForRemoteAssetState(
          tester,
          container,
          motionId,
          (asset) => asset.isVideo && asset.visibility == AssetVisibility.hidden && !asset.isTrashed,
          reason: 'Expected 076 motion asset to sync hidden before trash',
        );
        for (final assetId in [photoId, videoId, liveId, motionId]) {
          expect(await _remoteAssetRowCountById(drift, assetId), 1);
        }

        final logicalAssetIds = {photoId, videoId, liveId};
        final allKnownIds = {...logicalAssetIds, motionId};
        final timelineFactory = container.read(timelineFactoryProvider);
        final mainTimeline = timelineFactory.main([ownerId]);
        final videoTimeline = timelineFactory.video(ownerId);
        final trashTimeline = timelineFactory.trash(ownerId);
        ...
        await _expectTimelineAssetSet(
          tester,
          mainTimeline,
          includes: logicalAssetIds,
          excludes: {motionId},
          reason: 'Expected 076 uploaded assets to be active before trash',
        );
        await _expectTimelineAssetSet(
          tester,
          videoTimeline,
          includes: {videoId},
          excludes: {motionId},
          reason: 'Expected 076 video collection to expose only the standalone video before trash',
        );
        await _expectTimelineAssetSet(
          tester,
          trashTimeline,
          includes: const {},
          excludes: allKnownIds,
          reason: 'Expected 076 fixtures to be absent from trash initially',
        );

        Future<void> expectServerRestored(String assetId, String reason) async {
          final restored = await _waitForAssetInfoState(
            tester,
            assetsApi,
            assetId,
            (asset) => !asset.isTrashed,
          ...
          _expectAssetInfoPreserved(restored, beforeInfos[assetId]!);
        }

        Future<void> expectSearchContains(
          String assetId,
          String fileName, {
          ...
          await _waitForServerAssetIdsByOriginalFilename(
            ...
            searchApi,
            fileName,
            (ids) => ids.contains(assetId),
            type: type,
            reason: 'Expected 076 active search to include restored $fileName',
          );
        }

        Future<void> expectSearchExcludes(
          String assetId,
          String fileName, {
          api.AssetTypeEnum type = api.AssetTypeEnum.IMAGE,
        }) async {
          await _waitForServerAssetIdsByOriginalFilename(
            ...
            (ids) => !ids.contains(assetId),
            type: type,
            reason: 'Expected 076 active search to filter trashed $fileName',
          );
        }

        Future<void> assertActiveAfterRestore(String phase) async {
          await _resetAndSyncRemoteState(tester, container);
          for (final assetId in logicalAssetIds) {
            await expectServerRestored(assetId, 'Expected 076 $phase asset $assetId to be active on the server');
            await _waitForRemoteAssetState(
              tester,
              container,
              assetId,
              (asset) => asset.hasRemote && !asset.isTrashed && asset.visibility == AssetVisibility.timeline,
              reason: 'Expected 076 $phase asset $assetId to be active locally',
            );
            expect(await _remoteAssetRowCountById(drift, assetId), 1);
          }
          await expectServerRestored(motionId, 'Expected 076 $phase motion asset to be active on the server');
          await _waitForRemoteAssetState(
            tester,
            container,
            motionId,
            (asset) => asset.isVideo && asset.visibility == AssetVisibility.hidden && !asset.isTrashed,
            reason: 'Expected 076 $phase motion asset to remain hidden and active locally',
          );
          expect(await _remoteAssetRowCountById(drift, motionId), 1);
          await _expectTimelineAssetSet(
            tester,
            ...
            includes: logicalAssetIds,
            excludes: {motionId},
            reason: 'Expected 076 $phase restored assets in main timeline',
          );
          await _expectTimelineAssetSet(
            tester,
            videoTimeline,
            includes: {videoId},
            excludes: {motionId},
            reason: 'Expected 076 $phase restored standalone video in video collection',
          );
          await _expectTimelineAssetSet(
            tester,
            trashTimeline,
            includes: const {},
            excludes: allKnownIds,
            reason: 'Expected 076 $phase restored assets to leave trash',
          );
        }

        await assetService.trash(logicalAssetIds.toList());
        await _resetAndSyncRemoteState(tester, container);
        for (final assetId in allKnownIds) {
          await _waitForAssetInfoState(
            tester,
            assetsApi,
            assetId,
            (asset) => asset.isTrashed,
            reason: 'Expected 076 asset $assetId to enter trash',
          );
          await _waitForRemoteAssetState(
            tester,
            container,
            assetId,
            (asset) => asset.hasRemote && asset.isTrashed,
            reason: 'Expected 076 local asset $assetId to enter trash',
          );
        }
        await _expectTimelineAssetSet(
          tester,
          ...
          includes: const {},
          excludes: allKnownIds,
          reason: 'Expected 076 trashed assets to leave main timeline',
        );
        await _expectTimelineAssetSet(
          tester,
          videoTimeline,
          includes: const {},
          excludes: {videoId, motionId},
          reason: 'Expected 076 trashed videos to leave video collection',
        );
        await _expectTimelineAssetSet(
          tester,
          trashTimeline,
          includes: logicalAssetIds,
          excludes: {motionId},
          reason: 'Expected 076 trash to show logical assets without hidden motion',
        );
        await expectSearchExcludes(photoId, photoName);
        await expectSearchExcludes(videoId, videoName, type: api.AssetTypeEnum.VIDEO);
        await expectSearchExcludes(liveId, liveName);

        await assetService.restoreTrash([photoId]);
        await expectServerRestored(photoId, 'Expected 076 photo to restore individually');
        await _waitForRemoteAssetState(
          tester,
          container,
          photoId,
          (asset) => asset.hasRemote && !asset.isTrashed,
          reason: 'Expected 076 photo to restore locally',
        );
        await _expectTimelineAssetSet(
          tester,
          trashTimeline,
          includes: {videoId, liveId},
          excludes: {photoId, motionId},
          reason: 'Expected 076 trash after photo-only restore',
        );
        await expectSearchContains(photoId, photoName);

        await assetService.restoreTrash([videoId]);
        await expectServerRestored(videoId, 'Expected 076 video to restore individually');
        await _waitForRemoteAssetState(
          tester,
          container,
          videoId,
          (asset) => asset.hasRemote && asset.isVideo && !asset.isTrashed,
          reason: 'Expected 076 video to restore locally',
        );
        await _expectTimelineAssetSet(
          tester,
          videoTimeline,
          includes: {videoId},
          excludes: {motionId},
          reason: 'Expected 076 video collection after video restore',
        );
        await _expectTimelineAssetSet(
          tester,
          trashTimeline,
          includes: {liveId},
          excludes: {photoId, videoId, motionId},
          reason: 'Expected 076 trash after photo and video restore',
        );
        await expectSearchContains(videoId, videoName, type: api.AssetTypeEnum.VIDEO);

        final liveRestore = await trashApi.restoreAssets(api.BulkIdsDto(ids: [liveId]));
        expect(liveRestore, isNotNull);
        expect(liveRestore!.count, 1);
        await assertActiveAfterRestore('final');
        await expectSearchContains(liveId, liveName);

        for (final assetId in allKnownIds) {
          final download = assetId == motionId
              ? await _waitForSuccessfulResponse(
                  tester,
                  () => assetsApi.playAssetVideoWithHttpInfo(assetId),
                  acceptedStatusCodes: const {200, 206},
                  timeout: const Duration(minutes: 3),
                )
              : await _waitForSuccessfulResponse(
          ...
          expect(download.bodyBytes.length, beforeBytes[assetId]);
          expect(
            base64Encode(md5.convert(download.bodyBytes).bytes),
            beforeDownloads[assetId],
            reason: 'Restore must preserve bytes for 076 asset $assetId',
          );
        }

        const markerCaseId = 'MOB-MEDIA-076-$_caseSuffix';
        debugPrint(
          '$markerCaseId:RESTORED ${jsonEncode({
            'photoAsset': {'assetId': photoId, 'fileName': photoName, 'checksum': beforeDownloads[photoId], 'originalPath': beforeInfos[photoId]!.originalPath, 'bytes': beforeBytes[photoId]},
            'videoAsset': {'assetId': videoId, 'fileName': videoName, 'checksum': beforeDownloads[videoId], 'originalPath': beforeInfos[videoId]!.originalPath, 'bytes': beforeBytes[videoId], 'playbackBytes': videoPlayback.bodyBytes.length},
            'liveAsset': {'assetId': liveId, 'motionAssetId': motionId, 'fileName': liveName, 'motionName': motionName, 'stillChecksum': beforeDownloads[liveId], 'motionChecksum': beforeDownloads[motionId], 'originalPath': beforeInfos[liveId]!.originalPath, 'motionOriginalPath': beforeInfos[motionId]!.originalPath, 'stillBytes': beforeBytes[liveId], 'motionBytes': beforeBytes[motionId]},
            'restoredIds': logicalAssetIds.toList()..sort(),
          })}',
        );
        await _pumpFor(tester, const Duration(seconds: 12));
        expect(tester.takeException(), isNull);
    ...
    _realStackSessionTest(
      'MOB-MEDIA-077-$_caseSuffix',
      'empties trash permanently after cancellation leaves media unchanged',
      (tester) async {
        await _loadAuthenticatedApp(
          tester,
          overrideCancellation: true,
        ...
        final runToken = DateTime.now().toUtc().microsecondsSinceEpoch.toString();
        final seed = runToken.hashCode;
        final photoName = 'immich-e2e-empty-077-photo-$runToken.jpg';
        final videoName = 'immich-e2e-empty-077-video-$runToken.mp4';
        final liveName = 'immich-e2e-empty-077-live-$runToken.jpg';
        final motionName = liveName.replaceFirst(RegExp(r'\.[^.]+$'), '.mp4');

        expect(
          await _serverAssetIdsByOriginalFilename(searchApi, photoName),
          isEmpty,
          reason: 'The 077 photo filename must be unique before upload',
        );
        expect(
          await _serverAssetIdsByOriginalFilename(searchApi, videoName, type: api.AssetTypeEnum.VIDEO),
          isEmpty,
          reason: 'The 077 video filename must be unique before upload',
        );
        expect(
          await _serverAssetIdsByOriginalFilename(searchApi, liveName),
          isEmpty,
          reason: 'The 077 Live Photo filename must be unique before upload',
        );

        final createdPhoto = await container
            .read(fileMediaRepositoryProvider)
            .saveLocalAsset(_generatedJpegBytes(seed + 770), title: photoName, relativePath: 'Pictures/ImmichE2E077');
        expect(createdPhoto, isNotNull, reason: 'Expected PhotoManager to save the 077 photo fixture');
        final photoEntity = createdPhoto!;
        createdLocalAssetIds.add(photoEntity.id);
        final videoEntity = await _saveLocalTestVideo(
          container,
          createdLocalAssetIds,
          title: videoName,
          relativePath: Platform.isAndroid ? 'Movies/ImmichE2E077' : null,
          bytes: _generatedMp4Bytes('MOB-MEDIA-077-$_caseSuffix-video-$runToken'),
        );
        final liveEntity = await _saveLocalTestLivePhoto(
          container,
          createdLocalAssetIds,
          title: liveName,
          relativePath: Platform.isAndroid ? 'Pictures/ImmichE2E077' : null,
          imageBytes: _generatedJpegBytes(seed + 771),
          videoBytes: _generatedMp4Bytes('MOB-MEDIA-077-$_caseSuffix-live-$runToken'),
        );
        ...
        final localPhoto = await _waitForLocalAssetByIdState(
          ...
          (asset) =>
              ...
              asset.contentSize != null &&
              asset.contentSize! > 0,
          reason: 'Expected 077 photo fixture to sync locally',
        );
        final localVideo = await _waitForLocalAssetByIdState(
          container,
          videoEntity.id,
          tester,
          (asset) => asset.isVideo && asset.isLocalOnly && asset.contentSize != null && asset.contentSize! > 0,
          reason: 'Expected 077 video fixture to sync locally',
        );
        final localLive = await _waitForLocalAssetByIdState(
          container,
          liveEntity.id,
          tester,
          (asset) => asset.isImage && asset.isMotionPhoto && asset.isLocalOnly,
          reason: 'Expected 077 Live Photo fixture to sync locally',
        );
        ...
        final photoUpload = await uploadFixture(localPhoto, '077 photo');
        final photoId = photoUpload.remoteId;
        createdRemoteAssetIds.add(photoId);
        final videoUpload = await uploadFixture(localVideo, '077 video');
        final videoId = videoUpload.remoteId;
        createdRemoteAssetIds.add(videoId);
        final liveUpload = await uploadFixture(localLive, '077 Live Photo');
        final liveId = liveUpload.remoteId;
        createdRemoteAssetIds.add(liveId);
        _expectLivePhotoProgressNames(liveUpload.progress.keys, stillName: liveName, motionName: motionName);
        ...
        await _waitForServerAssetIdsByOriginalFilename(
          ...
          photoName,
          (ids) => ids.length == 1 && ids.contains(photoId),
          reason: 'Expected 077 photo to be discoverable before trash',
        );
        await _waitForServerAssetIdsByOriginalFilename(
          tester,
          searchApi,
          ...
          (ids) => ids.length == 1 && ids.contains(videoId),
          type: api.AssetTypeEnum.VIDEO,
          reason: 'Expected 077 video to be discoverable before trash',
        );
        await _waitForServerAssetIdsByOriginalFilename(
          tester,
          searchApi,
          liveName,
          (ids) => ids.length == 1 && ids.contains(liveId),
          reason: 'Expected 077 Live Photo to be discoverable before trash',
        );
        ...
        final photoInfo = await _waitForAssetInfoState(
          ...
          photoId,
          (asset) => asset.type == api.AssetTypeEnum.IMAGE && asset.originalFileName == photoName && !asset.isTrashed,
          reason: 'Expected 077 photo to be active before trash',
        );
        beforeInfos[photoId] = photoInfo;
        await rememberDownload(photoId);
        expect(beforeDownloads[photoId], photoInfo.checksum);
        ...
        final videoInfo = await _waitForAssetInfoState(
          ...
          (asset) =>
              ...
              asset.duration != null &&
              asset.duration! > 0,
          reason: 'Expected 077 video to be active before trash',
        );
        beforeInfos[videoId] = videoInfo;
        await rememberDownload(videoId);
        expect(beforeDownloads[videoId], videoInfo.checksum);
        ...
        final liveInfo = await _waitForAssetInfoState(
          ...
          (asset) =>
              ...
              !asset.isTrashed &&
              asset.livePhotoVideoId.orElse(null) != null,
          reason: 'Expected 077 Live Photo still to be active before trash',
        );
        beforeInfos[liveId] = liveInfo;
        await rememberDownload(liveId);
        expect(beforeDownloads[liveId], liveInfo.checksum);
        ...
        final motionInfo = await _waitForAssetInfoState(
          ...
          motionId,
          (asset) => asset.type == api.AssetTypeEnum.VIDEO && !asset.isTrashed,
          reason: 'Expected 077 linked motion asset before trash',
        );
        beforeInfos[motionId] = motionInfo;
        final motionPlayback = await _waitForSuccessfulResponse(
          tester,
        ...
        await _waitForRemoteAssetState(
          ...
          (asset) =>
              ...
              !asset.isTrashed &&
              asset.checksum == beforeDownloads[photoId],
          reason: 'Expected 077 photo to sync before trash',
        );
        await _waitForRemoteAssetState(
          tester,
          container,
          ...
          (asset) =>
              ...
              asset.checksum == beforeDownloads[videoId] &&
              asset.duration > Duration.zero,
          reason: 'Expected 077 video to sync before trash',
        );
        await _waitForRemoteAssetState(
          tester,
          container,
          ...
          (asset) =>
              ...
              asset.checksum == beforeDownloads[liveId] &&
              asset.livePhotoVideoId == motionId,
          reason: 'Expected 077 Live Photo still to sync before trash',
        );
        await _waitForRemoteAssetState(
          tester,
          container,
          motionId,
          (asset) => asset.isVideo && asset.visibility == AssetVisibility.hidden && !asset.isTrashed,
          reason: 'Expected 077 motion asset to sync hidden before trash',
        );
        for (final assetId in [photoId, videoId, liveId, motionId]) {
          expect(await _remoteAssetRowCountById(drift, assetId), 1);
        }
        ...
        final logicalAssetIds = {photoId, videoId, liveId};
        final sortedLogicalAssetIds = logicalAssetIds.toList()..sort();
        final allKnownIds = {...logicalAssetIds, motionId};
        final timelineFactory = container.read(timelineFactoryProvider);
        final mainTimeline = timelineFactory.main([ownerId]);
        final videoTimeline = timelineFactory.video(ownerId);
        ...
        await _expectTimelineAssetSet(
          ...
          includes: logicalAssetIds,
          excludes: {motionId},
          reason: 'Expected 077 uploaded assets to be active before trash',
        );
        await _expectTimelineAssetSet(
          tester,
          videoTimeline,
          includes: {videoId},
          excludes: {motionId},
          reason: 'Expected 077 video collection to expose only the standalone video before trash',
        );
        await _expectTimelineAssetSet(
          tester,
          trashTimeline,
          includes: const {},
          excludes: allKnownIds,
          reason: 'Expected 077 fixtures to be absent from trash initially',
        );
        ...
        Future<void> expectSearchExcludes(
          ...
          await _waitForServerAssetIdsByOriginalFilename(
            ...
            (ids) => !ids.contains(assetId),
            type: type,
            reason: 'Expected 077 active search to filter $fileName',
          );
        }

        await assetService.trash(sortedLogicalAssetIds);
        await _resetAndSyncRemoteState(tester, container);
        for (final assetId in allKnownIds) {
          await _waitForAssetInfoState(
            tester,
            ...
            assetId,
            (asset) => asset.isTrashed,
            reason: 'Expected 077 asset $assetId to enter trash',
          );
          await _waitForRemoteAssetState(
            tester,
            container,
            assetId,
            (asset) => asset.hasRemote && asset.isTrashed,
            reason: 'Expected 077 local asset $assetId to enter trash',
          );
          expect(await _remoteAssetRowCountById(drift, assetId), 1);
        }
        await _expectTimelineAssetSet(
          ...
          includes: const {},
          excludes: allKnownIds,
          reason: 'Expected 077 trashed assets to leave main timeline',
        );
        await _expectTimelineAssetSet(
          tester,
          videoTimeline,
          includes: const {},
          excludes: {videoId, motionId},
          reason: 'Expected 077 trashed videos to leave video collection',
        );
        await _expectTimelineAssetSet(
          tester,
          trashTimeline,
          includes: logicalAssetIds,
          excludes: {motionId},
          reason: 'Expected 077 trash to show logical assets without hidden motion',
        );
        await expectSearchExcludes(photoId, photoName);
        await expectSearchExcludes(videoId, videoName, type: api.AssetTypeEnum.VIDEO);
        await expectSearchExcludes(liveId, liveName);
        ...
        await _expectTimelineAssetSet(
          ...
          includes: logicalAssetIds,
          excludes: {motionId},
          reason: 'Cancelling empty trash must leave 077 assets in trash',
        );
        for (final assetId in allKnownIds) {
          await _waitForAssetInfoState(
            tester,
          ...
          await _waitForRemoteAssetState(
            ...
            assetId,
            (asset) => asset.hasRemote && asset.isTrashed,
            reason: 'Cancelling empty trash must keep local trash state for $assetId',
          );
          expect(await _remoteAssetRowCountById(drift, assetId), 1);
        }

        final emptyResult = await container.read(actionProvider.notifier).emptyTrash(ownerId);
        expect(emptyResult.success, isTrue, reason: 'Expected 077 confirmed empty trash action to succeed');
        expect(
          emptyResult.count,
          logicalAssetIds.length,
          reason: 'Empty trash should permanently delete only the 077 logical assets',
        );
        await _pumpFor(tester, const Duration(seconds: 2));

        createdRemoteAssetIds.clear();
        for (final assetId in allKnownIds) {
          await _waitForAssetInfoUnavailable(
            tester,
            assetsApi,
            assetId,
            reason: 'Confirmed empty trash should permanently delete 077 asset $assetId',
          );
          await _waitForRemoteAssetDeleted(
            tester,
            container,
            assetId,
            reason: 'Confirmed empty trash should remove local 077 asset row $assetId',
          );
          expect(await _remoteAssetRowCountById(drift, assetId), 0);
        }
        await _waitForRejectedResponse(
          tester,
          () => _authenticatedApiGet('/assets/$photoId/original?edited=false&e2eNoCache=$runToken'),
          reason: '077 deleted photo original must no longer download',
        );
        await _waitForRejectedResponse(
          tester,
          () => _authenticatedApiGet('/assets/$liveId/original?edited=false&e2eNoCache=$runToken'),
          reason: '077 deleted Live Photo still must no longer download',
        );
        await _waitForRejectedResponse(
          tester,
          () => _authenticatedApiGet('/assets/$videoId/original?edited=false&e2eNoCache=$runToken'),
          reason: '077 deleted video original must no longer download',
        );
        await _waitForRejectedResponse(
          tester,
          () => _authenticatedApiGet('/assets/$videoId/video/playback?e2eNoCache=$runToken'),
          reason: '077 deleted video playback must no longer stream',
        );
        await _waitForRejectedResponse(
          tester,
          () => _authenticatedApiGet('/assets/$motionId/video/playback?e2eNoCache=$runToken'),
          reason: '077 deleted Live Photo motion must no longer stream',
        );
        await _expectTimelineAssetSet(
          tester,
          mainTimeline,
          includes: const {},
          excludes: allKnownIds,
          reason: 'Expected 077 assets to stay absent from main timeline',
        );
        await _expectTimelineAssetSet(
          tester,
          videoTimeline,
          includes: const {},
          excludes: {videoId, motionId},
          reason: 'Expected 077 videos to stay absent from video collection',
        );
        await _expectTimelineAssetSet(
          tester,
          trashTimeline,
          includes: const {},
          excludes: allKnownIds,
          reason: 'Trash timeline should be empty for 077 after confirmation',
        );
        await expectSearchExcludes(photoId, photoName);
        await expectSearchExcludes(videoId, videoName, type: api.AssetTypeEnum.VIDEO);
        await expectSearchExcludes(liveId, liveName);

        const markerCaseId = 'MOB-MEDIA-077-$_caseSuffix';
        debugPrint(
          '$markerCaseId:EMPTIED ${jsonEncode({
            'photoAsset': {'assetId': photoId, 'fileName': photoName, 'checksum': beforeDownloads[photoId], 'originalPath': beforeInfos[photoId]!.originalPath, 'bytes': beforeBytes[photoId]},
            'videoAsset': {'assetId': videoId, 'fileName': videoName, 'checksum': beforeDownloads[videoId], 'originalPath': beforeInfos[videoId]!.originalPath, 'bytes': beforeBytes[videoId], 'playbackBytes': videoPlayback.bodyBytes.length},
            'liveAsset': {'assetId': liveId, 'motionAssetId': motionId, 'fileName': liveName, 'motionName': motionName, 'stillChecksum': beforeDownloads[liveId], 'motionChecksum': beforeDownloads[motionId], 'originalPath': beforeInfos[liveId]!.originalPath, 'motionOriginalPath': beforeInfos[motionId]!.originalPath, 'stillBytes': beforeBytes[liveId], 'motionBytes': beforeBytes[motionId]},
            'emptyCount': emptyResult.count,
            'purgedIds': sortedLogicalAssetIds,
          })}',
        );
        await _pumpFor(tester, const Duration(seconds: 12));
        expect(tester.takeException(), isNull);
    ...
    _realStackSessionTest(
      'MOB-MEDIA-078-$_caseSuffix',
      'closes the upload trash restore purge and same-content reupload loop',
      (tester) async {
        await _loadAuthenticatedApp(
          tester,
          overrideCancellation: true,
        ...
        final runToken = DateTime.now().toUtc().microsecondsSinceEpoch.toString();
        final seed = runToken.hashCode;
        final photoName = 'immich-e2e-loop-078-photo-$runToken.jpg';
        final videoName = 'immich-e2e-loop-078-video-$runToken.mp4';
        final liveName = 'immich-e2e-loop-078-live-$runToken.jpg';
        final motionName = liveName.replaceFirst(RegExp(r'\.[^.]+$'), '.mp4');

        expect(
          await _serverAssetIdsByOriginalFilename(searchApi, photoName),
          isEmpty,
          reason: 'The 078 photo filename must be unique before upload',
        );
        expect(
          await _serverAssetIdsByOriginalFilename(searchApi, videoName, type: api.AssetTypeEnum.VIDEO),
          isEmpty,
          reason: 'The 078 video filename must be unique before upload',
        );
        expect(
          await _serverAssetIdsByOriginalFilename(searchApi, liveName),
          isEmpty,
          reason: 'The 078 Live Photo filename must be unique before upload',
        );

        final createdPhoto = await container
            .read(fileMediaRepositoryProvider)
            .saveLocalAsset(_generatedJpegBytes(seed + 780), title: photoName, relativePath: 'Pictures/ImmichE2E078');
        expect(createdPhoto, isNotNull, reason: 'Expected PhotoManager to save the 078 photo fixture');
        final photoEntity = createdPhoto!;
        createdLocalAssetIds.add(photoEntity.id);
        final videoEntity = await _saveLocalTestVideo(
          container,
          createdLocalAssetIds,
          title: videoName,
          relativePath: Platform.isAndroid ? 'Movies/ImmichE2E078' : null,
          bytes: _generatedMp4Bytes('MOB-MEDIA-078-$_caseSuffix-video-$runToken'),
        );
        final liveEntity = await _saveLocalTestLivePhoto(
          container,
          createdLocalAssetIds,
          title: liveName,
          relativePath: Platform.isAndroid ? 'Pictures/ImmichE2E078' : null,
          imageBytes: _generatedJpegBytes(seed + 781),
          videoBytes: _generatedMp4Bytes('MOB-MEDIA-078-$_caseSuffix-live-$runToken'),
        );
        ...
        final localPhoto = await _waitForLocalAssetByIdState(
          ...
          (asset) =>
              ...
              asset.contentSize != null &&
              asset.contentSize! > 0,
          reason: 'Expected 078 photo fixture to sync locally',
        );
        final localVideo = await _waitForLocalAssetByIdState(
          container,
          videoEntity.id,
          tester,
          (asset) => asset.isVideo && asset.isLocalOnly && asset.contentSize != null && asset.contentSize! > 0,
          reason: 'Expected 078 video fixture to sync locally',
        );
        final localLive = await _waitForLocalAssetByIdState(
          container,
          liveEntity.id,
          tester,
          (asset) => asset.isImage && asset.isMotionPhoto && asset.isLocalOnly,
          reason: 'Expected 078 Live Photo fixture to sync locally',
        );
        ...
        final photoUpload = await uploadFixture(localPhoto, '078 photo');
        final photoId = photoUpload.remoteId;
        createdRemoteAssetIds.add(photoId);
        final videoUpload = await uploadFixture(localVideo, '078 video');
        final videoId = videoUpload.remoteId;
        createdRemoteAssetIds.add(videoId);
        final liveUpload = await uploadFixture(localLive, '078 Live Photo');
        final liveId = liveUpload.remoteId;
        createdRemoteAssetIds.add(liveId);
        _expectLivePhotoProgressNames(liveUpload.progress.keys, stillName: liveName, motionName: motionName);
        ...
        await _waitForServerAssetIdsByOriginalFilename(
          ...
          photoName,
          (ids) => ids.length == 1 && ids.contains(photoId),
          reason: 'Expected 078 photo to be discoverable after upload',
        );
        await _waitForServerAssetIdsByOriginalFilename(
          tester,
          searchApi,
          ...
          (ids) => ids.length == 1 && ids.contains(videoId),
          type: api.AssetTypeEnum.VIDEO,
          reason: 'Expected 078 video to be discoverable after upload',
        );
        await _waitForServerAssetIdsByOriginalFilename(
          tester,
          searchApi,
          liveName,
          (ids) => ids.length == 1 && ids.contains(liveId),
          reason: 'Expected 078 Live Photo to be discoverable after upload',
        );
        ...
        final photoInfo = await _waitForAssetInfoState(
          ...
          photoId,
          (asset) => asset.type == api.AssetTypeEnum.IMAGE && asset.originalFileName == photoName && !asset.isTrashed,
          reason: 'Expected 078 photo to be active after upload',
        );
        beforeInfos[photoId] = photoInfo;
        await rememberDownload(photoId);
        expect(beforeDownloads[photoId], photoInfo.checksum);
        ...
        final videoInfo = await _waitForAssetInfoState(
          ...
          (asset) =>
              ...
              asset.duration != null &&
              asset.duration! > 0,
          reason: 'Expected 078 video to be active after upload',
        );
        beforeInfos[videoId] = videoInfo;
        await rememberDownload(videoId);
        expect(beforeDownloads[videoId], videoInfo.checksum);
        ...
        final liveInfo = await _waitForAssetInfoState(
          ...
          (asset) =>
              ...
              !asset.isTrashed &&
              asset.livePhotoVideoId.orElse(null) != null,
          reason: 'Expected 078 Live Photo still to be active after upload',
        );
        beforeInfos[liveId] = liveInfo;
        await rememberDownload(liveId);
        expect(beforeDownloads[liveId], liveInfo.checksum);
        ...
        final motionInfo = await _waitForAssetInfoState(
          ...
          motionId,
          (asset) => asset.type == api.AssetTypeEnum.VIDEO && !asset.isTrashed,
          reason: 'Expected 078 linked motion asset after upload',
        );
        beforeInfos[motionId] = motionInfo;
        final motionPlayback = await _waitForSuccessfulResponse(
          tester,
        ...
        final syncedPhoto = await _waitForRemoteAssetState(
          ...
          (asset) =>
              ...
              !asset.isTrashed &&
              asset.checksum == beforeDownloads[photoId],
          reason: 'Expected 078 photo to sync locally after upload',
        );
        await _waitForRemoteAssetState(
          tester,
          container,
          ...
          (asset) =>
              ...
              asset.checksum == beforeDownloads[videoId] &&
              asset.duration > Duration.zero,
          reason: 'Expected 078 video to sync locally after upload',
        );
        await _waitForRemoteAssetState(
          tester,
          container,
          ...
          (asset) =>
              ...
              asset.checksum == beforeDownloads[liveId] &&
              asset.livePhotoVideoId == motionId,
          reason: 'Expected 078 Live Photo still to sync locally after upload',
        );
        await _waitForRemoteAssetState(
          tester,
          container,
          motionId,
          (asset) => asset.isVideo && asset.visibility == AssetVisibility.hidden && !asset.isTrashed,
          reason: 'Expected 078 motion asset to sync hidden after upload',
        );
        for (final assetId in [photoId, videoId, liveId, motionId]) {
          expect(await _remoteAssetRowCountById(drift, assetId), 1);
        }
        ...
        final logicalAssetIds = {photoId, videoId, liveId};
        final sortedLogicalAssetIds = logicalAssetIds.toList()..sort();
        final purgedAssetIds = {videoId, liveId};
        final sortedPurgedAssetIds = purgedAssetIds.toList()..sort();
        final allOriginalIds = {...logicalAssetIds, motionId};
        final timelineFactory = container.read(timelineFactoryProvider);
        final mainTimeline = timelineFactory.main([ownerId]);
        final videoTimeline = timelineFactory.video(ownerId);
        final trashTimeline = timelineFactory.trash(ownerId);
        ...
        await _expectTimelineAssetSet(
          ...
          includes: logicalAssetIds,
          excludes: {motionId},
          reason: 'Expected 078 uploaded assets to enter the main timeline',
        );
        await _expectTimelineAssetSet(
          tester,
          videoTimeline,
          includes: {videoId},
          excludes: {motionId},
          reason: 'Expected 078 standalone video before trash',
        );
        await _expectTimelineAssetSet(
          tester,
          trashTimeline,
          includes: const {},
          excludes: allOriginalIds,
          reason: 'Expected 078 fixtures to be absent from trash initially',
        );

        await _openTimelineAsset(tester, syncedPhoto);
        expect(
          container.read(assetViewerProvider).currentAsset?.remoteId,
          photoId,
          reason: 'Expected 078 synced photo to open from the timeline',
        );
        await tester.binding.handlePopRoute();
        await _pumpUntil(
          tester,
        ...
        Future<void> expectSearchContains(
          ...
          await _waitForServerAssetIdsByOriginalFilename(
            ...
            (ids) => ids.contains(assetId),
            type: type,
            reason: 'Expected 078 active search to include $fileName',
          );
        }
        ...
        Future<void> expectSearchExcludes(
          ...
          await _waitForServerAssetIdsByOriginalFilename(
            ...
            (ids) => !ids.contains(assetId),
            type: type,
            reason: 'Expected 078 active search to filter $fileName',
          );
        }

        await assetService.trash(sortedLogicalAssetIds);
        await _resetAndSyncRemoteState(tester, container);
        for (final assetId in allOriginalIds) {
          await _waitForAssetInfoState(
            tester,
            assetsApi,
            assetId,
            (asset) => asset.isTrashed,
            reason: 'Expected 078 asset $assetId to enter trash',
          );
          await _waitForRemoteAssetState(
            tester,
            container,
            assetId,
            (asset) => asset.hasRemote && asset.isTrashed,
            reason: 'Expected 078 local asset $assetId to enter trash',
          );
          expect(await _remoteAssetRowCountById(drift, assetId), 1);
        }
        await _expectTimelineAssetSet(
          ...
          includes: const {},
          excludes: allOriginalIds,
          reason: 'Expected 078 trashed assets to leave main timeline',
        );
        await _expectTimelineAssetSet(
          tester,
          videoTimeline,
          includes: const {},
          excludes: {videoId, motionId},
          reason: 'Expected 078 trashed videos to leave video collection',
        );
        await _expectTimelineAssetSet(
          tester,
          trashTimeline,
          includes: logicalAssetIds,
          excludes: {motionId},
          reason: 'Expected 078 trash to show logical assets before restore',
        );
        await expectSearchExcludes(photoId, photoName);
        await expectSearchExcludes(videoId, videoName, type: api.AssetTypeEnum.VIDEO);
        await expectSearchExcludes(liveId, liveName);
        ...
        await assetService.restoreTrash([photoId]);
        final restoredPhotoInfo = await _waitForAssetInfoState(
          tester,
          assetsApi,
          photoId,
          (asset) => !asset.isTrashed,
          reason: 'Expected 078 photo to restore before emptying trash',
        );
        _expectAssetInfoPreserved(restoredPhotoInfo, beforeInfos[photoId]!);
        await _resetAndSyncRemoteState(tester, container);
        await _waitForRemoteAssetState(
          tester,
          container,
          photoId,
          (asset) => asset.hasRemote && !asset.isTrashed && asset.checksum == beforeDownloads[photoId],
          reason: 'Expected 078 restored photo to sync locally',
        );
        await _expectTimelineAssetSet(
          tester,
          mainTimeline,
          includes: {photoId},
          excludes: {videoId, liveId, motionId},
          reason: 'Expected 078 restored photo in main timeline',
        );
        await _expectTimelineAssetSet(
          tester,
          trashTimeline,
          includes: purgedAssetIds,
          excludes: {photoId, motionId},
          reason: 'Expected 078 trash to retain only purge candidates',
        );
        await expectSearchContains(photoId, photoName);

        final emptyResult = await container.read(actionProvider.notifier).emptyTrash(ownerId);
        expect(emptyResult.success, isTrue, reason: 'Expected 078 empty trash action to succeed');
        expect(
          emptyResult.count,
          purgedAssetIds.length,
          reason: '078 empty trash should permanently delete two logical assets',
        );
        await _pumpFor(tester, const Duration(seconds: 2));

        createdRemoteAssetIds.removeAll([videoId, liveId, motionId]);
        for (final assetId in [videoId, liveId, motionId]) {
          await _waitForAssetInfoUnavailable(
            tester,
            assetsApi,
            assetId,
            reason: 'Expected 078 purged asset $assetId to disappear',
          );
          await _waitForRemoteAssetDeleted(
            tester,
            container,
            assetId,
            reason: 'Expected 078 purged asset $assetId to leave local DB',
          );
          expect(await _remoteAssetRowCountById(drift, assetId), 0);
        }
        await _waitForRejectedResponse(
          tester,
          () => _authenticatedApiGet('/assets/$videoId/original?edited=false&e2eNoCache=$runToken'),
          reason: '078 purged video original must no longer download',
        );
        await _waitForRejectedResponse(
          tester,
          () => _authenticatedApiGet('/assets/$videoId/video/playback?e2eNoCache=$runToken'),
          reason: '078 purged video playback must no longer stream',
        );
        await _waitForRejectedResponse(
          tester,
          () => _authenticatedApiGet('/assets/$liveId/original?edited=false&e2eNoCache=$runToken'),
          reason: '078 purged Live Photo still must no longer download',
        );
        await _waitForRejectedResponse(
          tester,
          () => _authenticatedApiGet('/assets/$motionId/video/playback?e2eNoCache=$runToken'),
          reason: '078 purged Live Photo motion must no longer stream',
        );
        await _expectTimelineAssetSet(
          tester,
          mainTimeline,
          includes: {photoId},
          excludes: {videoId, liveId, motionId},
          reason: 'Expected 078 only restored photo after purge',
        );
        await _expectTimelineAssetSet(
          tester,
          videoTimeline,
          includes: const {},
          excludes: {videoId, motionId},
          reason: 'Expected 078 purged videos to be absent before reupload',
        );
        await _expectTimelineAssetSet(
          tester,
          trashTimeline,
          includes: const {},
          excludes: allOriginalIds,
          reason: 'Expected 078 trash to be empty after purge',
        );
        await expectSearchExcludes(videoId, videoName, type: api.AssetTypeEnum.VIDEO);
        await expectSearchExcludes(liveId, liveName);

        final reupload = await uploadFixture(localVideo, '078 video reupload');
        final reuploadId = reupload.remoteId;
        createdRemoteAssetIds.add(reuploadId);
        expect(reuploadId, isNot(videoId), reason: '078 same-content reupload must not reuse the purged URL id');
        final reuploadInfo = await _waitForAssetInfoState(
          tester,
          assetsApi,
          reuploadId,
          (asset) =>
              ...
              !asset.isTrashed &&
              asset.checksum == beforeDownloads[videoId],
          reason: 'Expected 078 same-content reupload to become an active video',
        );
        await _waitForServerAssetIdsByOriginalFilename(
          tester,
          searchApi,
          videoName,
          (ids) => ids.length == 1 && ids.contains(reuploadId) && !ids.contains(videoId),
          type: api.AssetTypeEnum.VIDEO,
          reason: 'Expected 078 reupload to be the only active server video match',
        );
        final reuploadOriginal = await _waitForSuccessfulResponse(
          tester,
          () => container.read(assetApiRepositoryProvider).downloadAsset(reuploadId, edited: false),
          timeout: const Duration(minutes: 3),
        );
        expect(reuploadOriginal.bodyBytes.length, beforeBytes[videoId]);
        expect(
          base64Encode(md5.convert(reuploadOriginal.bodyBytes).bytes),
          beforeDownloads[videoId],
          reason: '078 reupload must preserve the original video bytes',
        );
        final reuploadPlayback = await _waitForSuccessfulResponse(
          tester,
          () => assetsApi.playAssetVideoWithHttpInfo(reuploadId),
          acceptedStatusCodes: const {200, 206},
          timeout: const Duration(minutes: 3),
        );
        expect(reuploadPlayback.bodyBytes, isNotEmpty);
        ...
        await _waitForRemoteAssetState(
          ...
          (asset) =>
              ...
              asset.checksum == beforeDownloads[videoId] &&
              asset.duration > Duration.zero,
          reason: 'Expected 078 reuploaded video to sync locally',
        );
        expect(await _remoteAssetRowCountById(drift, videoId), 0);
        expect(await _remoteAssetRowCountById(drift, reuploadId), 1);
        await _expectTimelineAssetSet(
          tester,
          mainTimeline,
          includes: {photoId, reuploadId},
          excludes: {videoId, liveId, motionId},
          reason: 'Expected 078 final timeline to contain restored and reupload',
        );
        await _expectTimelineAssetSet(
          tester,
          videoTimeline,
          includes: {reuploadId},
          excludes: {videoId, motionId},
          reason: 'Expected 078 video collection to contain only reupload',
        );
        await _expectTimelineAssetSet(
          tester,
          trashTimeline,
          includes: const {},
          excludes: {photoId, videoId, liveId, motionId, reuploadId},
          reason: 'Expected 078 final trash to be empty',
        );
        await expectSearchContains(photoId, photoName);
        await expectSearchContains(reuploadId, videoName, type: api.AssetTypeEnum.VIDEO);
        await expectSearchExcludes(liveId, liveName);

        const markerCaseId = 'MOB-MEDIA-078-$_caseSuffix';
        debugPrint(
          '$markerCaseId:LOOP_CLOSED ${jsonEncode({
            'restoredPhoto': {'assetId': photoId, 'fileName': photoName, 'checksum': beforeDownloads[photoId], 'originalPath': beforeInfos[photoId]!.originalPath, 'bytes': beforeBytes[photoId]},
            'purgedVideo': {'assetId': videoId, 'fileName': videoName, 'checksum': beforeDownloads[videoId], 'originalPath': beforeInfos[videoId]!.originalPath, 'bytes': beforeBytes[videoId], 'playbackBytes': videoPlayback.bodyBytes.length},
            'purgedLive': {'assetId': liveId, 'motionAssetId': motionId, 'fileName': liveName, 'motionName': motionName, 'stillChecksum': beforeDownloads[liveId], 'motionChecksum': beforeDownloads[motionId], 'originalPath': beforeInfos[liveId]!.originalPath, 'motionOriginalPath': beforeInfos[motionId]!.originalPath, 'stillBytes': beforeBytes[liveId], 'motionBytes': beforeBytes[motionId]},
            'reuploadedVideo': {'assetId': reuploadId, 'fileName': videoName, 'checksum': reuploadInfo.checksum, 'originalPath': reuploadInfo.originalPath, 'bytes': reuploadOriginal.bodyBytes.length, 'playbackBytes': reuploadPlayback.bodyBytes.length},
            'emptyCount': emptyResult.count,
            'purgedIds': sortedPurgedAssetIds,
          })}',
        );
        await _pumpFor(tester, const Duration(seconds: 12));
        expect(tester.takeException(), isNull);
...
Future<void> _openRemoteAlbumMenu(WidgetTester tester) async {
  ...
  final tappable = menuButton.hitTestable();
  if (tappable.evaluate().isNotEmpty) {
    await tester.tap(tappable.last, warnIfMissed: false);
  } else {
    final iconButton = tester.widget<IconButton>(menuButton.last);
    expect(iconButton.onPressed, isNotNull, reason: 'Expected the remote album menu button to be enabled');
...
Future<void> _tapHitTestableFinder(
  ...
  await _pumpUntilFoundWithReason(tester, finder, reason: reason ?? 'Expected finder to be tappable', timeout: timeout);
...
Future<List<DriftMemory>> _waitForMemoryProvider(
  WidgetTester tester,
  ProviderContainer container,
  bool Function(List<DriftMemory>) matches, {
  required String reason,
  ...
  while (DateTime.now().isBefore(end)) {
    container.invalidate(driftMemoryFutureProvider);
    latest = await container.read(driftMemoryFutureProvider.future);
    if (matches(latest)) {
      return latest;
    }
    await _pumpFor(tester, const Duration(milliseconds: 500));
...
Future<void> _pauseAndResumeApp(WidgetTester tester) async {
  tester.binding.handleAppLifecycleStateChanged(AppLifecycleState.paused);
...
Future<void> _loadAppPreservingStore(
  ...
  if (_selectedCaseId.isNotEmpty && _seedSelectedSessionStore && Store.tryGet(StoreKey.accessToken) == null) {
...
Future<void> _loadAuthenticatedApp(
  ...
  await _seedAuthenticatedStore(email: email, password: password).timeout(const Duration(seconds: 30));
  if (resetSyncAcksBeforeStart) {
    await _deleteSyncAckForStoredSession().timeout(const Duration(seconds: 30));
  }
...
Future<void> _resetAndSyncRemoteState(WidgetTester tester, ProviderContainer container) async {
  await container
      ...
      .timeout(const Duration(seconds: 30));
  await Store.delete(StoreKey.syncMigrationStatus).timeout(const Duration(seconds: 30));
  await container.read(syncStreamRepositoryProvider).reset().timeout(const Duration(seconds: 30));
  final syncSuccess = await container.read(syncStreamServiceProvider).sync().timeout(const Duration(seconds: 60));
  expect(syncSuccess, isTrue);
  await pumpUntilFound(tester, find.byType(Timeline), timeout: const Duration(seconds: 60));
}
...
Future<LocalAsset> _waitForLocalAssetByName(ProviderContainer container, String name, WidgetTester tester) async {
  final expectedId = _savedLocalFixtureIdsByRequestedName[name];
  var lastSeen = const <String>{};
  for (var attempt = 0; attempt < 12; attempt++) {
    await container.read(backgroundSyncProvider).syncLocal(full: true);
    final assets = await _localAssets(container);
    for (final asset in assets) {
      if (asset.name == name || (expectedId != null && asset.id == expectedId)) {
        return asset;
      }
    }
  ...
  fail('Local asset $name was not discovered; expectedId=$expectedId; saw ${sorted.join(', ')}');
}

Future<LocalAsset> _waitForLocalAssetById(
  ProviderContainer container,
  String id,
  WidgetTester tester, {
  required String reason,
  ...
  for (var attempt = 0; attempt < 12; attempt++) {
    await container.read(backgroundSyncProvider).syncLocal(full: true);
    final assets = await _localAssets(container);
    latest = assets.where((asset) => asset.id == id).firstOrNull;
    if (latest != null) {
      return latest;
    }
    await _pumpFor(tester, const Duration(seconds: 2));
...
Future<LocalAsset> _waitForNewLocalAsset(
  ProviderContainer container,
  Set<String> previousIds,
  WidgetTester tester, {
  required String reason,
}) async {
  var latest = const <LocalAsset>[];
  for (var attempt = 0; attempt < 12; attempt++) {
    await container.read(backgroundSyncProvider).syncLocal(full: true);
    latest = await _localAssets(container);
    final additions = latest.where((asset) => !previousIds.contains(asset.id)).toList()
  ...
  fail('$reason; saw ${latest.length} local assets');
}

Future<LocalAsset> _waitForLocalAssetWhere(
  ProviderContainer container,
  WidgetTester tester,
  ...
  final sample = latest
      .take(16)
      .map((asset) => '${asset.name}:${asset.width}x${asset.height}:o${asset.orientation}:gps=${asset.hasCoordinates}')
      .join(', ');
  fail('$reason; saw ${latest.length} local assets, sample=$sample');
}

Future<LocalAsset> _waitForLocalAssetByIdState(
  ProviderContainer container,
  String id,
  WidgetTester tester,
  bool Function(LocalAsset asset) matches, {
...
Future<List<LocalAsset>> _waitForLocalAssetsByIds(
  ProviderContainer container,
  Set<String> ids,
  WidgetTester tester, {
  required String reason,
  ...
  for (var attempt = 0; attempt < 12; attempt++) {
    await container.read(backgroundSyncProvider).syncLocal(full: true);
    final assets = await _localAssets(container);
    final latestById = <String, LocalAsset>{};
    for (final asset in assets.where((asset) => ids.contains(asset.id))) {
      latestById.putIfAbsent(asset.id, () => asset);
    }
    latest = latestById.values.toList();
    if (latestById.length == ids.length) {
      return latestById.values.toList();
    }
    await _pumpFor(tester, const Duration(seconds: 2));
  }

  final latestIds = latest.map((asset) => asset.id).toSet();
  fail('$reason; missing ids=${ids.difference(latestIds)}');
}

void _expectLivePhotoProgressNames(
  Iterable<String> fileNames, {
  required String stillName,
  required String motionName,
}) {
  final names = fileNames.toSet();
  expect(names, hasLength(2));
  if (!Platform.isIOS) {
    expect(names, containsAll([stillName, motionName]));
  }
  expect(names.any((name) => name.toLowerCase().endsWith('.mp4') || name.toLowerCase().endsWith('.mov')), isTrue);
  expect(
    names.any(
      (name) =>
          name.toLowerCase().endsWith('.jpg') ||
...
Future<LocalAsset> _saveLocalTestImage(
  ...
  return _saveLocalTestImageBytes(
    container,
    createdLocalAssetIds,
    title: title,
    relativePath: relativePath,
    bytes: _generatedJpegBytes(seed),
  );
}

Future<LocalAsset> _saveLocalTestImageBytes(
  ProviderContainer container,
  Set<String> createdLocalAssetIds, {
  required String title,
  required String relativePath,
  ...
  final created = await container
      .read(fileMediaRepositoryProvider)
      .saveLocalAsset(Uint8List.fromList(bytes), title: title, relativePath: relativePath);
  expect(created, isNotNull, reason: 'Expected PhotoManager to save local fixture $title');
  createdLocalAssetIds.add(created!.id);
  final copied = await _copyIosFixtureAssetToRelativePath(created.id, title, relativePath, createdLocalAssetIds);
  if (copied == null) {
    _savedLocalFixtureIdsByRequestedName[title] = created.id;
    return created;
  }
  _savedLocalFixtureIdsByRequestedName[title] = copied.id;
  return LocalAsset(
    id: copied.id,
    name: title,
    type: AssetType.image,
    createdAt: copied.createDateTime,
    updatedAt: copied.modifiedDateTime,
    width: copied.width,
    height: copied.height,
    orientation: copied.orientation,
...
Future<AssetEntity> _saveLocalTestVideo(
  ...
  try {
    ...
    expect(created, isNotNull, reason: 'Expected PhotoManager to save local video fixture $title');
    createdLocalAssetIds.add(created!.id);
    final copied = await _copyIosFixtureAssetToRelativePath(created.id, title, relativePath, createdLocalAssetIds);
    final result = copied ?? created;
    _savedLocalFixtureIdsByRequestedName[title] = result.id;
    return result;
  } finally {
    if (file.existsSync()) {
      file.deleteSync();
...
Future<AssetEntity> _saveLocalTestLivePhoto(
  ...
  final video = File('${directory.path}/${title.replaceFirst(RegExp(r'\.[^.]+$'), '.mp4')}');
  ...
  try {
    ...
    final created = Platform.isAndroid
        ? await fileMediaRepository.saveImageWithFile(image.path, title: title, relativePath: relativePath)
        : await fileMediaRepository.saveLivePhoto(image: image, video: video, title: title);
    expect(created, isNotNull, reason: 'Expected PhotoManager to save local Live Photo/Motion Photo fixture $title');
    createdLocalAssetIds.add(created!.id);
    final copied = await _copyIosFixtureAssetToRelativePath(created.id, title, relativePath, createdLocalAssetIds);
    final result = copied ?? created;
    _savedLocalFixtureIdsByRequestedName[title] = result.id;
...
Future<AssetEntity?> _copyIosFixtureAssetToRelativePath(
  String assetId,
  String title,
  String? relativePath,
  Set<String> createdLocalAssetIds,
) async {
  if (!Platform.isIOS || relativePath == null || relativePath.trim().isEmpty) {
    return null;
  }
  ...
  final album = await _iosFixtureAlbumForRelativePath(relativePath);
  if (album == null) {
    return null;
  }
  final copied = await PhotoManager.editor.copyAssetToPath(asset: entity, pathEntity: album);
  createdLocalAssetIds.add(copied.id);
  _savedLocalFixtureIdsByRequestedName[title] = copied.id;
  return copied;
}

Future<AssetPathEntity?> _iosFixtureAlbumForRelativePath(String relativePath) async {
  final normalized = relativePath.trim();
  if (normalized.isEmpty) {
    return null;
  }
  final cached = _iosFixtureAlbumsByRelativePath[normalized];
  if (cached != null) {
    final refreshed = await _findIosFixtureAlbumByName(cached.name);
    if (refreshed != null) {
      _iosFixtureAlbumsByRelativePath[normalized] = refreshed;
      return refreshed;
    }
    _iosFixtureAlbumsByRelativePath.remove(normalized);
  }
  ...
  if (segments.first.toLowerCase() == 'pictures' || segments.first.toLowerCase() == 'dcim') {
    segments.removeAt(0);
  }
  if (segments.isEmpty) {
    return null;
  ...
  final albumName = segments.last;
  AssetPathEntity? album;
  try {
    album = await PhotoManager.editor.darwin.createAlbum(albumName);
  } on StateError {
    album = await _findIosFixtureAlbumByName(albumName);
  }
  album ??= await _findIosFixtureAlbumByName(albumName);
  if (album == null) {
    return null;
  ...
  _iosFixtureAlbumsByRelativePath[normalized] = album;
  return album;
}

Future<AssetPathEntity?> _findIosFixtureAlbumByName(String albumName) async {
  final paths = await PhotoManager.getAssetPathList(hasAll: false, type: RequestType.common);
  for (final path in paths) {
    if (!path.isAll && path.albumType != 2 && path.name == albumName) {
      return path;
    }
  }
  return null;
...
List<int> _generatedAndroidMotionPhotoBytes(List<int> stillBytes, List<int> motionBytes) {
...
Future<void> _waitForLocalAssetGoneById(ProviderContainer container, String id, WidgetTester tester) async {
  LocalAsset? latest;
  for (var attempt = 0; attempt < 12; attempt++) {
    await container.read(backgroundSyncProvider).syncLocal(full: true);
    final assets = await _localAssets(container);
  ...
  fail('Local asset $id was still discovered after deletion; latest=$latest');
}
...
Future<List<LocalAlbum>> _waitForSelectableLocalAlbums(
  WidgetTester tester,
  ProviderContainer container, {
  ...
  for (var attempt = 0; attempt < 12; attempt++) {
    ...
    visibleAlbums = albums.where((album) => album.assetCount > 0).toList()
      ..sort((a, b) {
        final countCompare = b.assetCount.compareTo(a.assetCount);
        if (countCompare != 0) {
          return countCompare;
        }
    ...
    if (visibleAlbums.length >= minCount) {
      return visibleAlbums;
    }
    await _pumpFor(tester, const Duration(seconds: 2));
  }

  final seenNames = visibleAlbums.map((album) => '${album.name}(${album.assetCount})').toList()..sort();
  fail('Expected at least $minCount selectable local albums; saw ${seenNames.join(', ')}');
}
...
LocalAlbum _singleAlbumById(List<LocalAlbum> albums, String id) {
  final matches = albums.where((album) => album.id == id).toList();
  expect(matches, hasLength(1), reason: 'Expected exactly one local album with id $id');
  return matches.single;
}

List<LocalAlbum> _chooseBackupSelectionAlbums(List<LocalAlbum> albums) {
  final nonRecentsAlbums = albums.where((album) => album.name.toLowerCase() != 'recents');
  final ordered = [...nonRecentsAlbums, ...albums.where((album) => album.name.toLowerCase() == 'recents')];
  final byId = <String, LocalAlbum>{};
  for (final album in ordered) {
    byId.putIfAbsent(album.id, () => album);
  }
  return byId.values.take(3).toList();
}

List<LocalAlbum> _filterLocalAlbumsByQuery(List<LocalAlbum> albums, String query) {
  final normalizedQuery = query.toLowerCase();
  return albums.where((album) => album.name.toLowerCase().contains(normalizedQuery)).toList();
}

Map<String, BackupSelection> _backupSelectionById(List<LocalAlbum> albums, Set<String> albumIds) {
  return {for (final album in albums.where((album) => albumIds.contains(album.id))) album.id: album.backupSelection};
}

Map<BackupSelection, int> _backupSelectionCountsById(List<LocalAlbum> albums, Set<String> albumIds) {
  final counts = {for (final selection in BackupSelection.values) selection: 0};
  for (final album in albums.where((album) => albumIds.contains(album.id))) {
    counts[album.backupSelection] = counts[album.backupSelection]! + 1;
  }
  return counts;
}
...
Future<void> _waitForAppConfigSetting<T>(WidgetTester tester, SettingsKey<T> settingKey, T expected) async {
  await _pumpUntil(
    tester,
...
Future<void> _openSettingsRoot(WidgetTester tester, ProviderContainer container) async {
  final router = container.read(appRouterProvider);
...
Future<void> _openSettingsSection(WidgetTester tester, ProviderContainer container, SettingSection section) async {
...
Future<void> _scrollSettingsRootUntilHitTestable(WidgetTester tester, Finder target) async {
  final scrollable = find.descendant(of: find.byType(SettingsPage), matching: find.byType(Scrollable));
  expect(scrollable, findsWidgets);
...
Future<void> _setSettingsSwitchValue(WidgetTester tester, Key tileKey, bool expected) async {
...
Future<void> _setSettingsSliderFraction(WidgetTester tester, Key sliderKey, double fraction) async {
...
Future<void> _selectSettingsRadioValue(WidgetTester tester, String keyPrefix, Object value) async {
...
Future<void> _scrollSettingsListUntilHitTestable(WidgetTester tester, Finder target) async {
  final scrollable = find.descendant(of: find.byType(SettingsSubPage), matching: find.byType(Scrollable));
  expect(scrollable, findsWidgets);
...
Offset? _hitTestableElementCenterOrNull(
  WidgetTester tester,
  Finder finder, {
  double minCenterY = 0,
  double bottomPadding = 0,
...
Future<bool> _tapTimelineAssetTileById(WidgetTester tester, String assetId) async {
  final tile = _timelineAssetTileForAssetId(assetId);
  await _pumpFor(tester, const Duration(milliseconds: 200));
  if (tile.evaluate().isEmpty) {
    return false;
  }
  var center = _hitTestableElementCenterOrNull(tester, tile, minCenterY: 80, bottomPadding: 96);
  if (center == null) {
    try {
      await tester.ensureVisible(tile.first);
    } catch (_) {
    ...
    await _pumpFor(tester, const Duration(milliseconds: 300));
    center = _hitTestableElementCenterOrNull(tester, tile, minCenterY: 80, bottomPadding: 96);
  }
  if (center == null) {
    return false;
  }
...
Future<bool> _longPressTimelineAssetTileById(WidgetTester tester, String assetId) async {
  final tile = _timelineAssetTileForAssetId(assetId);
  await _pumpFor(tester, const Duration(milliseconds: 200));
  if (tile.evaluate().isEmpty) {
    return false;
...
BaseAsset? _visibleTimelineAssetById(WidgetTester tester, String assetId) {
  for (final element in _timelineAssetTileForAssetId(assetId).evaluate()) {
    final widget = element.widget;
    if (widget is ThumbnailTile && widget.asset != null) {
      return widget.asset;
    }
  }
  return null;
...
Future<BaseAsset?> _timelineServiceAssetById(ProviderContainer container, String assetId) async {
  final timelineService = container.read(timelineServiceProvider);
  final totalAssets = timelineService.totalAssets;
  if (totalAssets <= 0) {
    return null;
  }
  final assets = timelineService.hasRange(0, totalAssets)
      ? timelineService.getAssets(0, totalAssets)
      : await timelineService.loadAssets(0, totalAssets);
  return assets.where((asset) => _timelineAssetId(asset) == assetId).firstOrNull;
}

Future<BaseAsset> _selectableTimelineAssetById(WidgetTester tester, ProviderContainer container, String assetId) async {
  final selectedAsset = container
      .read(multiSelectProvider)
      .selectedAssets
      .where((asset) => _timelineAssetId(asset) == assetId)
      .firstOrNull;
  final asset =
      selectedAsset ??
      _visibleTimelineAssetById(tester, assetId) ??
      await _timelineServiceAssetById(container, assetId);
  expect(asset, isNotNull, reason: 'Expected timeline asset $assetId to be available for selection');
  return asset!;
}

Future<void> _setTimelineAssetSelectedById(
  WidgetTester tester,
  ProviderContainer container,
  String assetId, {
  ...
  if (longPress) {
    await _longPressTimelineAssetTileById(tester, assetId);
  } else {
    await _tapTimelineAssetTileById(tester, assetId);
  }

  final selectedIds = _timelineAssetIds(container.read(multiSelectProvider).selectedAssets);
  if (selectedIds.contains(assetId) == selected) {
    return;
  }

  final asset = await _selectableTimelineAssetById(tester, container, assetId);
  final notifier = container.read(multiSelectProvider.notifier);
  if (selected) {
    notifier.selectAsset(asset);
  } else {
    notifier.deselectAsset(asset);
  }
  await _pumpFor(tester, const Duration(milliseconds: 300));
}

Future<void> _setTimelineSelectionByIds(
  WidgetTester tester,
  ProviderContainer container,
  Set<String> expectedAssetIds,
) async {
  final currentIds = _timelineAssetIds(container.read(multiSelectProvider).selectedAssets);
  if (currentIds.length == expectedAssetIds.length && currentIds.containsAll(expectedAssetIds)) {
    return;
  }

  final notifier = container.read(multiSelectProvider.notifier);
  for (final asset in container.read(multiSelectProvider).selectedAssets.toList()) {
    if (!expectedAssetIds.contains(_timelineAssetId(asset))) {
      notifier.deselectAsset(asset);
    }
  }
  for (final assetId in expectedAssetIds) {
    if (!currentIds.contains(assetId)) {
      notifier.selectAsset(await _selectableTimelineAssetById(tester, container, assetId));
    }
  }
  await _pumpFor(tester, const Duration(milliseconds: 300));
}
...
Future<void> _waitForTimelineAssetsAvailableById(
  WidgetTester tester,
  ProviderContainer container,
  Set<String> expectedAssetIds,
) async {
  ...
  while (DateTime.now().isBefore(end)) {
    ...
    if (timelineService.totalAssets > 0) {
      final assets = await timelineService.loadAssets(0, timelineService.totalAssets);
      latestIds = _timelineAssetIds(assets);
      if (latestIds.containsAll(expectedAssetIds)) {
        return;
      }
    }
    await _pumpFor(tester, const Duration(milliseconds: 500));
  ...
  fail('Expected timeline assets ${expectedAssetIds.join(', ')} to be loaded; saw ${latestIds.join(', ')}');
}
...
Future<void> _selectTimelineAssetsById(WidgetTester tester, ProviderContainer container, List<String> assetIds) async {
  ...
  await _waitForTimelineAssetsAvailableById(tester, container, assetIds.toSet());

  container.read(multiSelectProvider.notifier).reset();
  await _pumpFor(tester, const Duration(milliseconds: 200));

  await _setTimelineAssetSelectedById(tester, container, assetIds.first, selected: true, longPress: true);
  await _setTimelineSelectionByIds(tester, container, {assetIds.first});
  await _waitForSelectedTimelineAssetIds(tester, container, {assetIds.first});

  for (var index = 1; index < assetIds.length; index++) {
    await _setTimelineAssetSelectedById(tester, container, assetIds[index], selected: true);
    final expectedIds = assetIds.take(index + 1).toSet();
    await _setTimelineSelectionByIds(tester, container, expectedIds);
    await _waitForSelectedTimelineAssetIds(tester, container, expectedIds);
  }
}
...
Future<void> _tapTrashMenuAction(WidgetTester tester, String labelKey) async {
  ...
  final tappableMenuButton = menuButton.hitTestable();
  await pumpUntilFound(tester, tappableMenuButton, timeout: const Duration(seconds: 30));
  await tester.tap(tappableMenuButton.first, warnIfMissed: false);
  await _pumpFor(tester, const Duration(milliseconds: 300));
  final menuAction = find.text(labelKey.tr()).hitTestable();
  await pumpUntilFound(tester, menuAction, timeout: const Duration(seconds: 30));
  await tester.tap(menuAction.last, warnIfMissed: false);
  await _pumpFor(tester, const Duration(milliseconds: 300));
}
...
const _iosUrlLauncherPigeonCodec = _IosUrlLauncherPigeonCodec();
const _iosLaunchSuccess = _IosUrlLauncherPigeonEnumValue(129, 0);
const _iosInAppLoadSuccess = _IosUrlLauncherPigeonEnumValue(130, 0);

BasicMessageChannel<Object?> _urlLauncherPigeonChannel(
  String platform,
  String method, {
  MessageCodec<Object?> codec = const StandardMessageCodec(),
}) => BasicMessageChannel<Object?>('dev.flutter.pigeon.url_launcher_$platform.UrlLauncherApi.$method', codec);

class _IosUrlLauncherPigeonEnumValue {
  final int type;
  final int index;

  const _IosUrlLauncherPigeonEnumValue(this.type, this.index);
}

class _IosUrlLauncherPigeonCodec extends StandardMessageCodec {
  const _IosUrlLauncherPigeonCodec();

  @override
  void writeValue(WriteBuffer buffer, Object? value) {
    if (value is _IosUrlLauncherPigeonEnumValue) {
...
void _recordUrlLauncherInvocations(List<String> launchedUrls) {
  ...
  messenger.setMockDecodedMessageHandler<Object?>(_urlLauncherPigeonChannel('android', 'canLaunchUrl'), (_) async {
    return <Object?>[true];
  });
  messenger.setMockDecodedMessageHandler<Object?>(_urlLauncherPigeonChannel('android', 'launchUrl'), (message) async {
    final arguments = message! as List<Object?>;
    launchedUrls.add(arguments.first! as String);
    return <Object?>[true];
  });
  messenger.setMockDecodedMessageHandler<Object?>(
    _urlLauncherPigeonChannel('ios', 'canLaunchUrl', codec: _iosUrlLauncherPigeonCodec),
    (_) async => <Object?>[_iosLaunchSuccess],
  );
  messenger.setMockDecodedMessageHandler<Object?>(
    _urlLauncherPigeonChannel('ios', 'launchUrl', codec: _iosUrlLauncherPigeonCodec),
    (message) async {
      final arguments = message! as List<Object?>;
      launchedUrls.add(arguments.first! as String);
      return <Object?>[_iosLaunchSuccess];
    },
  );
  messenger.setMockDecodedMessageHandler<Object?>(
    _urlLauncherPigeonChannel('ios', 'openUrlInSafariViewController', codec: _iosUrlLauncherPigeonCodec),
    (message) async {
      final arguments = message! as List<Object?>;
      launchedUrls.add(arguments.first! as String);
      return <Object?>[_iosInAppLoadSuccess];
    },
  );
...
void _clearUrlLauncherInvocationRecorder() {
  final messenger = TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger;
  messenger.setMockDecodedMessageHandler<Object?>(_urlLauncherPigeonChannel('android', 'canLaunchUrl'), null);
  messenger.setMockDecodedMessageHandler<Object?>(_urlLauncherPigeonChannel('android', 'launchUrl'), null);
  messenger.setMockDecodedMessageHandler<Object?>(
    _urlLauncherPigeonChannel('ios', 'canLaunchUrl', codec: _iosUrlLauncherPigeonCodec),
    null,
  );
...
void _expectSharedDisplayNames(_ShareInvocation invocation, Set<String> displayNames, {int? expectedCount}) {
  final count = expectedCount ?? displayNames.length;
  expect(invocation.paths, hasLength(count));
  expect(invocation.mimeTypes, hasLength(count));
  expect(invocation.existedWhenShared, everyElement(isTrue));
  for (final name in displayNames) {
    expect(
      invocation.paths.any((path) {
        final basename = path.split('/').last;
        return basename == name || basename.endsWith('-$name') || basename.contains(name);
      }),
      isTrue,
...
Future<Directory> _createShareIntentFixtureDirectory(String prefix) async {
  final directory = await getApplicationDocumentsDirectory();
  final token = DateTime.now().toUtc().microsecondsSinceEpoch;
  return Directory('${directory.path}/$prefix$token').create(recursive: true);
}
...
Future<void> _createAlbumFromBottomSheet(WidgetTester tester, String albumName) async {
  ...
  final createIcon = find.descendant(of: find.byType(AddToAlbumHeader), matching: find.byIcon(Icons.add));
  await pumpUntilFound(tester, createIcon, timeout: const Duration(seconds: 30));
  final createButton = find.ancestor(of: createIcon, matching: find.byType(TextButton)).hitTestable();
  for (var attempt = 0; attempt < 4 && createButton.evaluate().isEmpty; attempt++) {
    await _expandBottomSheet(tester, GeneralBottomSheet);
  }
  expect(createButton, findsWidgets, reason: 'Expected the create-new-album button to be tappable in the bottom sheet');
...
Future<void> _writeUpgradeBackupSettings() async {
  await _writeBackupSettings(_upgradeExpectedBackupSettings);
}

Future<void> _writeBackupSettingsWithSyncAlbums(Map<String, Object> settings) async {
  await _writeBackupSettings(settings);
  await SettingsRepository.instance.write(SettingsKey.backupSyncAlbums, settings['backupSyncAlbums']! as bool);
}

Future<void> _writeBackupSettings(Map<String, Object> settings) async {
  await SettingsRepository.instance.write(SettingsKey.backupEnabled, settings['backupEnabled']! as bool);
  await SettingsRepository.instance.write(
    SettingsKey.backupUseCellularForPhotos,
    settings['backupUseCellularForPhotos']! as bool,
  );
  await SettingsRepository.instance.write(
    SettingsKey.backupUseCellularForVideos,
    settings['backupUseCellularForVideos']! as bool,
  );
  await SettingsRepository.instance.write(
    SettingsKey.backupRequireCharging,
    settings['backupRequireCharging']! as bool,
  );
  await SettingsRepository.instance.write(SettingsKey.backupTriggerDelay, settings['backupTriggerDelay']! as int);
}

Future<void> _expectBackupSettings(WidgetTester tester, Map<String, Object> expected) async {
  await _waitForAppConfigSetting(tester, SettingsKey.backupEnabled, expected['backupEnabled']! as bool);
  await _waitForAppConfigSetting(
    tester,
    SettingsKey.backupUseCellularForPhotos,
    expected['backupUseCellularForPhotos']! as bool,
  );
  await _waitForAppConfigSetting(
    tester,
    SettingsKey.backupUseCellularForVideos,
    expected['backupUseCellularForVideos']! as bool,
  );
  await _waitForAppConfigSetting(tester, SettingsKey.backupRequireCharging, expected['backupRequireCharging']! as bool);
  await _waitForAppConfigSetting(tester, SettingsKey.backupTriggerDelay, expected['backupTriggerDelay']! as int);
  await _waitForAppConfigSetting(tester, SettingsKey.backupSyncAlbums, expected['backupSyncAlbums']! as bool);
}

Map<String, Object> _readBackupSettings() {
...
Map<String, Object> _readBackupSettingsWithSyncAlbums() {
  final config = SettingsRepository.instance.appConfig;
  return {..._readBackupSettings(), 'backupSyncAlbums': config.read(SettingsKey.backupSyncAlbums)};
}

Map<String, Object> _readUpgradeBackupSettings() => _readBackupSettings();
...
Future<void> _openSearchFilterSheet(WidgetTester tester, Key chipKey) async {
  ...
  final chipScrollable = find.descendant(of: chipList, matching: find.byType(Scrollable));
  await pumpUntilFound(tester, chipScrollable, timeout: const Duration(seconds: 30));
  ...
  await _pumpFor(tester, const Duration(milliseconds: 200));
  await _tapHitTestableFinder(tester, chip, reason: 'Expected search filter chip $chipKey to be tappable');
  await pumpUntilFound(tester, find.byKey(const Key('search_filter_apply')), timeout: const Duration(seconds: 30));
}

Future<void> _applySearchFilterSheet(WidgetTester tester) async {
  await _tapHitTestableFinder(tester, find.byKey(const Key('search_filter_apply')));
  await _pumpFor(tester, const Duration(milliseconds: 500));
}

Future<void> _clearSearchFilterSheet(WidgetTester tester) async {
  await _tapHitTestableFinder(tester, find.byKey(const Key('search_filter_clear')));
  await _pumpFor(tester, const Duration(milliseconds: 500));
}
...
Future<void> _expectDuplicateUploadReusesRemoteId(
  WidgetTester tester,
  ProviderContainer container,
  api.SearchApi searchApi,
  LocalAsset asset, {
  ...
  expect(duplicateError, isNull, reason: 'Expected $label duplicate pass to be non-error');
  expect(duplicateRemoteId, expectedRemoteId, reason: 'Expected $label duplicate pass to reuse the same asset');
  await _waitForServerAssetIdsByOriginalFilename(
    tester,
    searchApi,
    serverFilename,
    (ids) => ids.length == 1 && ids.contains(expectedRemoteId),
    type: serverType,
...
Future<String> _uploadGeneratedJpegAsSecondClient(
  ...
  final bytes = _generatedJpegBytes(bytesSeed ?? createdAt.microsecondsSinceEpoch);
...
Future<({List<String> imageIds, List<String> videoIds})> _uploadTimelineSeedAssets(
  String caseNumber, {
  required int imageCount,
  int videoCount = 0,
  bool includeSharedTimestamp = false,
  bool includePreviousMonth = false,
}) async {
  final imageIds = <String>[];
  final videoIds = <String>[];
  final runToken = DateTime.now().toUtc().microsecondsSinceEpoch;
  final baseCreatedAt = DateTime.now().toUtc().add(const Duration(minutes: 5));
  final sharedCreatedAt = baseCreatedAt.subtract(const Duration(milliseconds: 1));

  for (var index = 0; index < imageCount; index++) {
    final createdAt = includeSharedTimestamp && index < 2
        ? sharedCreatedAt
        : includePreviousMonth && index.isOdd
        ? baseCreatedAt.subtract(Duration(days: 40, seconds: index))
        : baseCreatedAt.subtract(Duration(seconds: index));
    final id = await _uploadGeneratedJpegAsSecondClient(
      'immich-e2e-timeline-$caseNumber-$runToken-${index.toString().padLeft(2, '0')}.jpg',
      createdAt,
      bytesSeed: runToken + index,
    );
    imageIds.add(id);
  }

  for (var index = 0; index < videoCount; index++) {
    final id = await _uploadGeneratedMp4AsSecondClient(
      'immich-e2e-timeline-$caseNumber-$runToken-video-${index.toString().padLeft(2, '0')}.mp4',
      baseCreatedAt.add(Duration(seconds: index + 1)),
    );
    videoIds.add(id);
  }
...
SearchFilter _searchFilter({
  ...
  return SearchFilter(
    ...
    rating: rating == null ? SearchRatingFilter() : SearchRatingFilter(rating: Option.some(rating)),
    mediaType: mediaType,
    tagIds: tagIds,
  );
}
...
api.MetadataSearchDto _metadataSearchDto({
  ...
  return api.MetadataSearchDto(
    ...
    personIds: personIds == null ? const api.Optional.absent() : api.Optional.present(personIds),
    country: country == null ? const api.Optional.absent() : api.Optional.present(country),
    state: state == null ? const api.Optional.absent() : api.Optional.present(state),
    city: city == null ? const api.Optional.absent() : api.Optional.present(city),
    isNotInAlbum: isNotInAlbum == null ? const api.Optional.absent() : api.Optional.present(isNotInAlbum),
...
Future<List<api.PersonResponseDto>> _waitForServerPeopleState(
  ...
  while (DateTime.now().isBefore(end)) {
    try {
      latest =
          (await peopleApi.getAllPeople(withHidden: withHidden, size: 1000))?.people ?? const <api.PersonResponseDto>[];
      if (matches(latest)) {
        return latest;
      }
    } catch (error) {
...
Future<Set<String>> _waitForMapMarkerAssetIds(
  ...
  final end = DateTime.now().add(const Duration(seconds: 60));
  while (DateTime.now().isBefore(end)) {
    final markers = await container.read(mapFactoryProvider).remote([userId], options).getMarkers(bounds);
...
Future<List<BaseAsset>> _expectTimelineAssetSet(
  ...
  final missingIds = includes.difference(latestIds.toSet()).toList()..sort();
  final unexpectedIds = latestIds.toSet().intersection(excludes).toList()..sort();
  fail(
    '$reason; missing=${missingIds.join(', ')} unexpected=${unexpectedIds.join(', ')} '
    'latest timeline ids=${latestIds.join(', ')}',
  );
}
...
Future<void> _waitForAuthenticatedUserEndpoint(
  WidgetTester tester,
  String apiEndpoint, {
  required Duration timeout,
}) async {
  await _waitForSuccessfulResponse(
    tester,
    () => http.get(
      Uri.parse('$apiEndpoint/users/me'),
      headers: {...ApiService.getRequestHeaders(), 'Authorization': 'Bearer ${Store.get(StoreKey.accessToken)}'},
    ),
    timeout: timeout,
  );
