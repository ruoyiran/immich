    ...
    _realStackSessionTest(
      ...
      (tester) async {
        ...
        final photoFixtures = [
          for (var index = 0; index < 3; index++)
            (
              name: 'immich-e2e-trash-075-photo-${index + 1}-$runToken.jpg',
              bytes: _generatedJpegBytes(seed + index),
            ),
        ];
        final videoFixtures = [
          for (var index = 0; index < 2; index++)
            (
              ...
              bytes: _generatedMp4Bytes(
                'MOB-MEDIA-075-$_caseSuffix-video-${index + 1}-$runToken',
              ),
            ),
        ];
        ...
        final liveMotionBytes = _generatedMp4Bytes(
          'MOB-MEDIA-075-$_caseSuffix-live-$runToken',
        );
        ...
        for (final fixture in photoFixtures) {
          expect(
            ...
            isEmpty,
            reason:
                'The 075 photo filename ${fixture.name} must be unique before upload',
          );
          final created = await container
              ...
              .saveLocalAsset(
                fixture.bytes,
                title: fixture.name,
                relativePath: 'Pictures/ImmichE2E075',
              );
          expect(
            created,
            isNotNull,
            reason: 'Expected PhotoManager to save 075 photo ${fixture.name}',
          );
          createdLocalAssetIds.add(created!.id);
        }
        for (final fixture in videoFixtures) {
          expect(
            ...
            isEmpty,
            reason:
                'The 075 video filename ${fixture.name} must be unique before upload',
          );
          await _saveLocalTestVideo(
            container,
            createdLocalAssetIds,
            title: fixture.name,
        ...
        for (final fixture in photoFixtures) {
          photoLocalAssets[fixture.name] = await _waitForLocalAssetByNameState(
            container,
            fixture.name,
            tester,
            (asset) =>
        ...
        for (final fixture in videoFixtures) {
          videoLocalAssets[fixture.name] = await _waitForLocalAssetByNameState(
            container,
            fixture.name,
            tester,
            (asset) =>
        ...
        final liveLocal = await _waitForLocalAssetByNameState(
          container,
          liveName,
          tester,
        ...
        Future<({String remoteId, Map<String, List<double>> progress})>
        uploadFixture(LocalAsset asset, String label) async {
          final progressByName = <String, List<double>>{};
          String? remoteAssetId;
          String? uploadError;
          await container
        ...
        for (final fixture in photoFixtures) {
          final upload = await uploadFixture(
            photoLocalAssets[fixture.name]!,
            '075 photo ${fixture.name}',
          );
        ...
        for (final fixture in videoFixtures) {
          final upload = await uploadFixture(
            videoLocalAssets[fixture.name]!,
            '075 video ${fixture.name}',
          );
        ...
        createdRemoteAssetIds.add(liveId);
        expect(liveUpload.progress.keys, containsAll([liveName, motionName]));
        ...
        final liveStillChecksum = base64Encode(
          md5.convert(originalStill.bodyBytes).bytes,
        );
        ...
        final liveMotionChecksum = base64Encode(
          md5.convert(motionPlayback.bodyBytes).bytes,
        );
        ...
        await _expectTimelineAssetSet(
          ...
          excludes: {motionId},
          reason:
              'Expected 075 mixed batch assets to be visible before trash',
        );
        await _expectTimelineAssetSet(
          tester,
          videoTimeline,
          ...
          excludes: {motionId},
          reason:
              'Expected 075 uploaded videos to be visible before trash without the hidden motion asset',
        );
        await _expectTimelineAssetSet(
          tester,
          trashTimeline,
        ...
        for (final metadata in photoMetadata) {
          await _waitForAssetInfoState(
            ...
            reason:
                'Expected 075 photo ${metadata['fileName']} to be logically trashed',
          );
        }
        for (final metadata in videoMetadata) {
          await _waitForAssetInfoState(
            ...
            (asset) =>
                ...
                asset.checksum == metadata['checksum'],
            reason:
                'Expected 075 video ${metadata['fileName']} to be logically trashed',
          );
        }
        await _waitForAssetInfoState(
          tester,
        ...
        final trashAssets = await _expectTimelineAssetSet(
          ...
          excludes: {motionId},
          reason:
              'Expected 075 trash to contain six logical assets without the hidden motion asset',
        );
        expect(
          _timelineAssetIds(trashAssets).intersection(allKnownIds),
          expectedTrashIds,
          reason:
              'Expected 075 trash timeline to expose exactly the six logical assets from this batch',
        );
        ...
        for (final entry in photoIdsByName.entries) {
          await _waitForServerAssetIdsByOriginalFilename(
            ...
            reason:
                'Expected 075 active search to filter trashed photo ${entry.key}',
          );
        }
        for (final entry in videoIdsByName.entries) {
          await _waitForServerAssetIdsByOriginalFilename(
            ...
            type: api.AssetTypeEnum.VIDEO,
            reason:
                'Expected 075 active search to filter trashed video ${entry.key}',
          );
        }
        await _waitForServerAssetIdsByOriginalFilename(
          tester,
        ...
        debugPrint(
          '$markerCaseId:TRASHED ${jsonEncode({
            ...
            'liveAsset': {
              ...
              'fileName': liveName,
              'motionName': motionName,
              'stillChecksum': liveStillChecksum,
              'motionChecksum': liveMotionChecksum,
              'originalPath': liveInfo.originalPath,
              'stillBytes': originalStill.bodyBytes.length,
              'motionBytes': motionPlayback.bodyBytes.length,
            },
    ...
    _realStackSessionTest(
      ...
      (tester) async {
        ...
        final createdPhoto = await container
            .read(fileMediaRepositoryProvider)
            .saveLocalAsset(
              _generatedJpegBytes(seed + 760),
              title: photoName,
              relativePath: 'Pictures/ImmichE2E076',
            );
        expect(
          createdPhoto,
          isNotNull,
          reason: 'Expected PhotoManager to save the 076 photo fixture',
        );
        createdLocalAssetIds.add(createdPhoto!.id);
        await _saveLocalTestVideo(
          container,
          createdLocalAssetIds,
          title: videoName,
          ...
          bytes: _generatedMp4Bytes(
            'MOB-MEDIA-076-$_caseSuffix-video-$runToken',
          ),
        );
        await _saveLocalTestLivePhoto(
          container,
          ...
          videoBytes: _generatedMp4Bytes(
            'MOB-MEDIA-076-$_caseSuffix-live-$runToken',
          ),
        );

        await container.read(backgroundSyncProvider).syncLocal(full: true);
        final localPhoto = await _waitForLocalAssetByNameState(
          container,
          photoName,
          tester,
          (asset) =>
        ...
        final localVideo = await _waitForLocalAssetByNameState(
          container,
          videoName,
          tester,
          (asset) =>
        ...
        beforeDownloads[motionId] = base64Encode(
          md5.convert(motionPlayback.bodyBytes).bytes,
        );
        ...
        await _expectTimelineAssetSet(
          ...
          excludes: {motionId},
          reason:
              'Expected 076 video collection to expose only the standalone video before trash',
        );
        await _expectTimelineAssetSet(
          tester,
          trashTimeline,
        ...
        Future<void> assertActiveAfterRestore(String phase) async {
          ...
          for (final assetId in logicalAssetIds) {
            await expectServerRestored(
              assetId,
              'Expected 076 $phase asset $assetId to be active on the server',
            );
            await _waitForRemoteAssetState(
              tester,
              container,
              ...
              (asset) =>
                  ...
                  asset.visibility == AssetVisibility.timeline,
              reason:
                  'Expected 076 $phase asset $assetId to be active locally',
            );
            expect(await _remoteAssetRowCountById(drift, assetId), 1);
          }
          await expectServerRestored(
            motionId,
            'Expected 076 $phase motion asset to be active on the server',
          );
          await _waitForRemoteAssetState(
            tester,
            container,
            ...
            (asset) =>
                ...
                !asset.isTrashed,
            reason:
                'Expected 076 $phase motion asset to remain hidden and active locally',
          );
          expect(await _remoteAssetRowCountById(drift, motionId), 1);
          await _expectTimelineAssetSet(
            tester,
          ...
          await _expectTimelineAssetSet(
            ...
            excludes: {motionId},
            reason:
                'Expected 076 $phase restored standalone video in video collection',
          );
          await _expectTimelineAssetSet(
            tester,
            trashTimeline,
        ...
        await _expectTimelineAssetSet(
          ...
          excludes: {motionId},
          reason:
              'Expected 076 trash to show logical assets without hidden motion',
        );
        ...
        await expectServerRestored(
          photoId,
          'Expected 076 photo to restore individually',
        );
        await _waitForRemoteAssetState(
          tester,
          container,
        ...
        await expectServerRestored(
          videoId,
          'Expected 076 video to restore individually',
        );
        await _waitForRemoteAssetState(
          tester,
          container,
        ...
        final liveRestore = await trashApi.restoreAssets(
          api.BulkIdsDto(ids: [liveId]),
        );
        ...
        debugPrint(
          '$markerCaseId:RESTORED ${jsonEncode({
            'photoAsset': {
              'assetId': photoId,
              'fileName': photoName,
              'checksum': beforeDownloads[photoId],
              'originalPath': beforeInfos[photoId]!.originalPath,
              'bytes': beforeBytes[photoId],
            },
            'videoAsset': {
              'assetId': videoId,
              'fileName': videoName,
              'checksum': beforeDownloads[videoId],
              'originalPath': beforeInfos[videoId]!.originalPath,
              'bytes': beforeBytes[videoId],
              'playbackBytes': videoPlayback.bodyBytes.length,
            },
            'liveAsset': {
              'assetId': liveId,
              'motionAssetId': motionId,
              'fileName': liveName,
              'motionName': motionName,
              'stillChecksum': beforeDownloads[liveId],
              'motionChecksum': beforeDownloads[motionId],
              'originalPath': beforeInfos[liveId]!.originalPath,
              'motionOriginalPath': beforeInfos[motionId]!.originalPath,
              'stillBytes': beforeBytes[liveId],
              'motionBytes': beforeBytes[motionId],
            },
    ...
    _realStackSessionTest(
      ...
      (tester) async {
        ...
        final createdPhoto = await container
            .read(fileMediaRepositoryProvider)
            .saveLocalAsset(
              _generatedJpegBytes(seed + 770),
              title: photoName,
              relativePath: 'Pictures/ImmichE2E077',
            );
        expect(
          createdPhoto,
          isNotNull,
          reason: 'Expected PhotoManager to save the 077 photo fixture',
        );
        createdLocalAssetIds.add(createdPhoto!.id);
        await _saveLocalTestVideo(
          container,
          ...
          bytes: _generatedMp4Bytes(
            'MOB-MEDIA-077-$_caseSuffix-video-$runToken',
          ),
        );
        await _saveLocalTestLivePhoto(
          container,
          ...
          videoBytes: _generatedMp4Bytes(
            'MOB-MEDIA-077-$_caseSuffix-live-$runToken',
          ),
        );
        ...
        await _expectTimelineAssetSet(
          ...
          excludes: {motionId},
          reason:
              'Expected 077 video collection to expose only the standalone video before trash',
        );
        await _expectTimelineAssetSet(
          tester,
          trashTimeline,
        ...
        await _expectTimelineAssetSet(
          ...
          excludes: {motionId},
          reason:
              'Expected 077 trash to show logical assets without hidden motion',
        );
        await expectSearchExcludes(photoId, photoName);
        await expectSearchExcludes(
          videoId,
        ...
        for (final assetId in allKnownIds) {
          ...
          await _waitForRemoteAssetState(
            ...
            (asset) => asset.hasRemote && asset.isTrashed,
            reason:
                'Cancelling empty trash must keep local trash state for $assetId',
          );
          expect(await _remoteAssetRowCountById(drift, assetId), 1);
        }

        final emptyResult = await container
            .read(actionProvider.notifier)
            .emptyTrash(ownerId);
        expect(
          emptyResult.success,
          isTrue,
          reason: 'Expected 077 confirmed empty trash action to succeed',
        );
        expect(
          emptyResult.count,
          logicalAssetIds.length,
          reason:
              'Empty trash should permanently delete only the 077 logical assets',
        );
        await _pumpFor(tester, const Duration(seconds: 2));
        ...
        for (final assetId in allKnownIds) {
          await _waitForAssetInfoUnavailable(
            ...
            assetId,
            reason:
                'Confirmed empty trash should permanently delete 077 asset $assetId',
          );
          await _waitForRemoteAssetDeleted(
            tester,
            container,
            assetId,
            reason:
                'Confirmed empty trash should remove local 077 asset row $assetId',
          );
          expect(await _remoteAssetRowCountById(drift, assetId), 0);
        }
        await _waitForRejectedResponse(
          tester,
          () => _authenticatedApiGet(
            '/assets/$photoId/original?edited=false&e2eNoCache=$runToken',
          ),
        ...
        await _waitForRejectedResponse(
          tester,
          () => _authenticatedApiGet(
            '/assets/$liveId/original?edited=false&e2eNoCache=$runToken',
          ),
        ...
        await _waitForRejectedResponse(
          tester,
          () => _authenticatedApiGet(
            '/assets/$videoId/original?edited=false&e2eNoCache=$runToken',
          ),
        ...
        await _waitForRejectedResponse(
          tester,
          () => _authenticatedApiGet(
            '/assets/$videoId/video/playback?e2eNoCache=$runToken',
          ),
        ...
        await _waitForRejectedResponse(
          tester,
          () => _authenticatedApiGet(
            '/assets/$motionId/video/playback?e2eNoCache=$runToken',
          ),
    ...
    _realStackSessionTest(
      ...
      (tester) async {
        ...
        final createdPhoto = await container
            .read(fileMediaRepositoryProvider)
            .saveLocalAsset(
              _generatedJpegBytes(seed + 780),
              title: photoName,
              relativePath: 'Pictures/ImmichE2E078',
            );
        expect(
          createdPhoto,
          isNotNull,
          reason: 'Expected PhotoManager to save the 078 photo fixture',
        );
        createdLocalAssetIds.add(createdPhoto!.id);
        await _saveLocalTestVideo(
          container,
          ...
          bytes: _generatedMp4Bytes(
            'MOB-MEDIA-078-$_caseSuffix-video-$runToken',
          ),
        );
        await _saveLocalTestLivePhoto(
          container,
          ...
          videoBytes: _generatedMp4Bytes(
            'MOB-MEDIA-078-$_caseSuffix-live-$runToken',
          ),
        );
        ...
        uploadFixture(LocalAsset asset, String label) async {
          ...
          expect(
            uploadError,
            isNull,
            reason: 'Expected $label upload to succeed',
          );
          expect(
            remoteAssetId,
            isNotNull,
        ...
        expect(
          emptyResult.success,
          isTrue,
          reason: 'Expected 078 empty trash action to succeed',
        );
        expect(
          emptyResult.count,
          purgedAssetIds.length,
          reason:
              '078 empty trash should permanently delete two logical assets',
        );
        await _pumpFor(tester, const Duration(seconds: 2));
        ...
        expect(
          reuploadId,
          isNot(videoId),
          reason: '078 same-content reupload must not reuse the purged URL id',
        );
        final reuploadInfo = await _waitForAssetInfoState(
          ...
          (asset) =>
              ...
              asset.checksum == beforeDownloads[videoId],
          reason:
              'Expected 078 same-content reupload to become an active video',
        );
        await _waitForServerAssetIdsByOriginalFilename(
          tester,
          searchApi,
          ...
          (ids) =>
              ids.length == 1 &&
              ids.contains(reuploadId) &&
              !ids.contains(videoId),
          type: api.AssetTypeEnum.VIDEO,
          reason:
              'Expected 078 reupload to be the only active server video match',
        );
        final reuploadOriginal = await _waitForSuccessfulResponse(
          tester,
          () => container
              .read(assetApiRepositoryProvider)
              .downloadAsset(reuploadId, edited: false),
          timeout: const Duration(minutes: 3),
        );
        ...
        await _expectTimelineAssetSet(
          ...
          excludes: {videoId, liveId, motionId},
          reason:
              'Expected 078 final timeline to contain restored and reupload',
        );
        await _expectTimelineAssetSet(
          tester,
          videoTimeline,
