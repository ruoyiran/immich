...
const _backgroundWorkerTriggerMarker =
    'MOB-REAL-031-A:TRIGGER_ANDROID_BACKGROUND_WORKER';
...
void main() async {
  ...
  group('real stack auth', () {
    ...
    _realStackSessionTest(
      ...
      'runs scheduled background backup and tears down cleanly',
      (tester) async {
        final (container, drift) = await _loadAuthenticatedSyncContainer();
        final backgroundWorker = container.read(
          backgroundWorkerFgServiceProvider,
        );
        final backgroundWorkerLock = container.read(
          backgroundWorkerLockServiceProvider,
        );
        final apiService = container.read(apiServiceProvider);
        final user = Store.tryGet(StoreKey.currentUser);
        expect(user, isNotNull);
        ...
        await SettingsRepository.instance.write(
          SettingsKey.backupTriggerDelay,
          1,
        );
        ...
        expect(
          beforeServerIds,
          isEmpty,
          reason: 'Background backup fixture name must be unique for this run',
        );
        ...
        final initialCounts = await _waitForBackupCounts(
          ...
          reason:
              'Expected selected album to expose exactly one background backup candidate',
        );
        ...
        await container
            ...
            .configure(minimumDelaySeconds: 1, requireCharging: false);
        ...
        final firstServerIds = await _waitForServerAssetIdsByOriginalFilename(
          ...
          reason:
              'Expected forced Android background worker to upload the selected local asset',
          timeout: const Duration(minutes: 3),
        );
        final uploadedId = firstServerIds.difference(beforeServerIds).single;
        uploadedRemoteIds.add(uploadedId);

        final firstSyncSuccess = await container
            .read(syncStreamServiceProvider)
            .sync();
        ...
        await _waitForRemoteAssetState(
          ...
          (asset) =>
              asset.visibility == AssetVisibility.timeline && !asset.isTrashed,
          reason:
              'Expected background-uploaded asset to sync into the local remote timeline',
        );
        expect(await _remoteAssetRowCountById(drift, uploadedId), 1);
        ...
        final afterFirstCounts = await _waitForBackupCounts(
          ...
          reason:
              'Uploaded background asset should no longer be a backup candidate',
        );
        expect(afterFirstCounts.processing, 0);
        expect(
          await container
              .read(backgroundUploadServiceProvider)
              .getActiveTasks(kBackupGroup),
          isEmpty,
        );
        ...
        final secondServerIds = await _serverAssetIdsByOriginalFilename(
          apiService.searchApi,
          _backgroundBackupAssetName,
        );
        expect(
          ...
          firstServerIds,
          reason:
              'A repeated background schedule must not upload the same selected asset twice',
        );
        final secondSyncSuccess = await container
            .read(syncStreamServiceProvider)
            .sync();
    ...
    _realStackSessionTest(
      ...
      (tester) async {
        ...
        final originalBackupEnabled =
            SettingsRepository.instance.appConfig.backup.enabled;
        final originalSyncAlbums =
            SettingsRepository.instance.appConfig.backup.syncAlbums;
        addTearDown(() async {
          await _restoreAlbumBackupSelections(
            albumRepository,
            originalSelections,
          );
          await SettingsRepository.instance.write(
        ...
        final cameraAlbum = _singleAlbumNamed(
          albums,
          _backupAlbumSelectionCameraAlbum,
        );
        final screenshotsAlbum = _singleAlbumNamed(
          albums,
          _backupAlbumSelectionScreenshotsAlbum,
        );
        final downloadAlbum = _singleAlbumNamed(
          albums,
          _backupAlbumSelectionDownloadAlbum,
        );
        final duplicateAlbums = _albumsNamed(
          albums,
          _backupAlbumSelectionDuplicateAlbum,
        );
        expect(
          duplicateAlbums,
          hasLength(2),
          reason:
              'Expected two same-name fixture albums for duplicate handling',
        );
        ...
        await _waitForVisibleBackupAlbumTile(
          tester,
          _backupAlbumSelectionCameraAlbum,
        );
        ...
        expect(
          cameraSearchResults,
          isNot(contains(_backupAlbumSelectionScreenshotsAlbum)),
        );

        await _tapBackupAlbumTile(tester, _backupAlbumSelectionCameraAlbum);
        await _waitForAlbumSelection(
          container,
          cameraAlbum.id,
        ...
        expect(
          _visibleBackupAlbumNames(
            tester,
          ).where((name) => name == _backupAlbumSelectionDuplicateAlbum),
          hasLength(2),
        );
        await tester.tap(
          find.widgetWithText(ElevatedButton, 'select_all'.tr()),
        );
        await _pumpFor(tester, const Duration(milliseconds: 800));
        ...
        await _doubleTapBackupAlbumTile(
          tester,
          _backupAlbumSelectionDownloadAlbum,
        );
        await _waitForAlbumSelection(
        ...
        expect(
          ...
          reason:
              'An excluded album must be mutually exclusive with selected backup albums',
        );
        ...
        final selectedAlbumIds = {
          screenshotsAlbum.id,
          ...duplicateAlbums.map((album) => album.id),
        };
        ...
        await _expectAlbumSelections(container, {
          ...
          for (final album in duplicateAlbums)
            album.id: BackupSelection.selected,
          downloadAlbum.id: BackupSelection.excluded,
        });
        final expectedTotal = await _expectedBackupAssetTotal(
          container,
          selectedAlbumIds,
          excludedAlbumIds,
        );
    ...
    _realStackSessionTest(
      ...
      (tester) async {
        ...
        final originalBackupEnabled =
            SettingsRepository.instance.appConfig.backup.enabled;
        final originalCellularPhotos =
            SettingsRepository.instance.appConfig.backup.useCellularForPhotos;
        final originalCellularVideos =
            SettingsRepository.instance.appConfig.backup.useCellularForVideos;
        final originalRequireCharging =
            SettingsRepository.instance.appConfig.backup.requireCharging;
        final originalTriggerDelay =
            SettingsRepository.instance.appConfig.backup.triggerDelay;
        final originalSyncAlbums =
            SettingsRepository.instance.appConfig.backup.syncAlbums;
        final beforeServerIds = await _serverAssetIdsByOriginalFilename(
          apiService.searchApi,
        ...
        addTearDown(() async {
          try {
            container
                ...
                .stopForegroundBackup(reason: 'MOB-UI-036 cleanup');
          } catch (_) {
            // The ProviderScope may already have disposed the notifier.
          }
          await SettingsRepository.instance.write(
          ...
          await SettingsRepository.instance.write(
            SettingsKey.backupUseCellularForPhotos,
            originalCellularPhotos,
          );
          await SettingsRepository.instance.write(
            SettingsKey.backupUseCellularForVideos,
            originalCellularVideos,
          );
          await SettingsRepository.instance.write(
            SettingsKey.backupRequireCharging,
            originalRequireCharging,
          );
          await SettingsRepository.instance.write(
            SettingsKey.backupTriggerDelay,
            originalTriggerDelay,
          );
          await SettingsRepository.instance.write(
            SettingsKey.backupSyncAlbums,
            originalSyncAlbums,
          ...
          final afterServerAlbumIds = await _serverAlbumIdsByName(
            apiService.albumsApi,
            _backupSettingsAlbumName,
          );
          for (final albumId in afterServerAlbumIds.difference(
            beforeServerAlbumIds,
          )) {
        ...
        final albums = await _waitForBackupAlbumFixtures(
          tester,
          container,
          requiredNames: {_backupSettingsAlbumName},
        );
        final settingsAlbum = _singleAlbumNamed(
          albums,
          _backupSettingsAlbumName,
        );
        await albumRepository.upsert(
          settingsAlbum.copyWith(backupSelection: BackupSelection.selected),
        );
        await container.read(backupAlbumProvider.notifier).getAll();

        final initialCounts = await container
            .read(foregroundUploadServiceProvider)
            .getBackupCounts(user!.id);
        expect(
          initialCounts.total,
        ...
        await pumpUntilFound(
          tester,
          find.byType(BackupToggleButton),
          timeout: const Duration(seconds: 30),
        );
        ...
        await _expectBackupOptionSwitch(
          tester,
          titleKey: 'videos',
          expected: false,
        );
        ...
        expect(
          find.text('network_requirement_videos_upload'.tr()),
          findsWidgets,
        );
        expect(
          find.text('network_requirement_photos_upload'.tr()),
          findsWidgets,
        );
        expect(
          find.text('charging_requirement_mobile_backup'.tr()),
          findsWidgets,
        );
      ...
      (tester) async {
        ...
        addTearDown(() async {
          try {
            container
                .read(driftBackupProvider.notifier)
                .stopForegroundBackup(reason: 'MOB-UI-037 cleanup');
          } catch (_) {
            // The ProviderScope may already have disposed the notifier.
          }
        ...
        final albums = await _waitForBackupAlbumFixtures(
          tester,
          container,
          requiredNames: {_uploadQueueAlbumName},
        );
        final uploadQueueAlbum = _singleAlbumNamed(
          albums,
          _uploadQueueAlbumName,
        );
        await albumRepository.upsert(
          uploadQueueAlbum.copyWith(backupSelection: BackupSelection.selected),
        );
        await container.read(backupAlbumProvider.notifier).getAll();

        final successAsset = await _waitForLocalAssetByName(
          container,
          _uploadQueueSuccessAssetName,
          tester,
        );
        final retryAsset = await _waitForLocalAssetByName(
          container,
          _uploadQueueRetryAssetName,
          tester,
        );
        ...
        await _ensureUploadQueueTextVisible(
          tester,
          _uploadQueueSuccessAssetName,
          pageType: DriftBackupAssetDetailPage,
        );
        await _ensureUploadQueueTextVisible(
          tester,
          _uploadQueueRetryAssetName,
        ...
        final progressItems = await _waitForUploadItems(
          ...
          reason:
              'Expected the large retry asset to appear as an active upload item before cancellation',
          timeout: const Duration(seconds: 60),
        );
        ...
        container
            .read(driftBackupProvider.notifier)
            .stopForegroundBackup(reason: 'MOB-UI-037 cancel progress probe');
        await _waitForUploadItems(
          tester,
          container,
          (items) => items.isEmpty,
          reason:
              'Expected cancelling the foreground upload to clear upload detail items',
        );
        ...
        final failedItems = await _waitForUploadItems(
          ...
          reason:
              'Expected an upload failure when the server endpoint is unreachable',
          timeout: const Duration(seconds: 60),
        );
        ...
        container
            .read(driftBackupProvider.notifier)
            .stopForegroundBackup(reason: 'MOB-UI-037 retry after failure');
        await Store.put(StoreKey.serverEndpoint, originalEndpoint);
        await Store.put(StoreKey.serverUrl, originalServerUrl);
        apiService.setEndpoint(originalEndpoint);
        await apiService.updateHeaders();
        ...
        final successIds = await _waitForServerAssetIdsByOriginalFilename(
          ...
          reason:
              'Expected the success upload asset to be created exactly once',
          timeout: const Duration(seconds: 90),
        );
        ...
        expect(
          retryIds.difference(beforeRetryIds),
          contains(duplicateRemoteId),
        );
        expect(
...
Future<void> _selectOnlyBackupAlbumForAsset(
  ProviderContainer container,
  LocalAsset asset,
) async {
  final albumRepository = container.read(localAlbumRepository);
  ...
  for (final album in albums) {
    if (album.backupSelection != BackupSelection.none) {
      await albumRepository.upsert(
        album.copyWith(backupSelection: BackupSelection.none),
      );
    }
  }
  ...
  expect(
    sourceAlbums,
    isNotEmpty,
    reason: 'Expected ${asset.name} to belong to at least one local album',
  );
  await albumRepository.upsert(
    sourceAlbums.first.copyWith(backupSelection: BackupSelection.selected),
  );
}

Future<Map<String, BackupSelection>> _albumBackupSelections(
  ProviderContainer container,
) async {
  final albums = await container.read(localAlbumServiceProvider).getAll();
...
Future<void> _restoreAlbumBackupSelections(
  ...
  for (final album in albums) {
    ...
    if (album.backupSelection != originalSelection) {
      await albumRepository.upsert(
        album.copyWith(backupSelection: originalSelection),
      );
    }
  }
}
...
Future<List<LocalAlbum>> _waitForBackupAlbumFixtures(
  ...
  for (var attempt = 0; attempt < 12; attempt++) {
    ...
    final hasDuplicatedName =
        duplicatedName == null ||
        albums.where((album) => album.name == duplicatedName).length >= 2;
  ...
  fail(
    'Expected backup album fixtures $requiredNames$duplicateExpectation; saw ${seenNames.join(', ')}',
  );
}
...
Future<void> _searchBackupAlbums(WidgetTester tester, String query) async {
  ...
  await tester.enterText(
    find.descendant(of: page, matching: find.byType(TextField)).last,
    query,
  );
  await tester.testTextInput.receiveAction(TextInputAction.done);
...
Future<void> _clearBackupAlbumSearch(WidgetTester tester) async {
  final page = find.byType(DriftBackupAlbumSelectionPage);
  await tester.tap(
    find.descendant(of: page, matching: find.byIcon(Icons.close)).last,
  );
  await _pumpFor(tester, const Duration(milliseconds: 500));
}
...
Future<void> _tapBackupAlbumTile(WidgetTester tester, String albumName) async {
  ...
  await tester.tap(
    _backupAlbumTileFinder(albumName).first,
    warnIfMissed: false,
  );
  await _pumpFor(tester, const Duration(milliseconds: 800));
}

Future<void> _doubleTapBackupAlbumTile(
  WidgetTester tester,
  String albumName,
) async {
  await _ensureBackupAlbumVisible(tester, albumName);
...
Future<void> _ensureBackupAlbumVisible(
  WidgetTester tester,
  String albumName,
) async {
  ...
  final scrollable = find.descendant(
    of: find.byType(DriftBackupAlbumSelectionPage),
    matching: find.byType(Scrollable),
  );
  expect(
    scrollable,
    findsWidgets,
    reason:
        'Expected a scrollable backup album list before looking for $albumName',
  );
  await tester.scrollUntilVisible(
    tile,
    600,
...
Finder _backupAlbumTileFinder(String albumName) {
  return find.byWidgetPredicate(
    (widget) =>
        widget is DriftAlbumInfoListTile && widget.album.name == albumName,
  );
}
...
Future<void> _waitForVisibleBackupAlbumTile(
  WidgetTester tester,
  String albumName,
) async {
  ...
  fail(
    'Expected visible backup album "$albumName"; saw visible albums: ${visibleNames.join(', ')}',
  );
}
...
Future<BackupSelection> _albumSelection(
  ProviderContainer container,
  String albumId,
) async {
  final albums = await container.read(localAlbumServiceProvider).getAll();
...
Future<void> _expectAlbumSelections(
  ProviderContainer container,
  Map<String, BackupSelection> expectedById,
) async {
  ...
  for (final entry in expectedById.entries) {
    expect(
      actual[entry.key],
      entry.value,
      reason: 'Unexpected backup selection for local album ${entry.key}',
    );
  }
}
...
Future<int> _expectedBackupAssetTotal(
  ...
  for (final albumId in selectedAlbumIds) {
    selectedAssetIds.addAll(
      (await albumRepository.getAssets(albumId)).map((asset) => asset.id),
    );
  }

  final excludedAssetIds = <String>{};
  for (final albumId in excludedAlbumIds) {
    excludedAssetIds.addAll(
      (await albumRepository.getAssets(albumId)).map((asset) => asset.id),
    );
  }
...
Finder _backupPageSwitchFinder() {
  return find.descendant(
    of: find.byType(BackupToggleButton),
...
Future<void> _ensureBackupOptionTextVisible(
  WidgetTester tester,
  String titleKey,
) async {
  ...
  final scrollable = find.descendant(
    of: find.byType(DriftBackupOptionsPage),
    matching: find.byType(Scrollable),
  );
  expect(
    scrollable,
    findsWidgets,
    reason:
        'Expected backup options to be scrollable before looking for $titleKey',
  );
  try {
    await tester.scrollUntilVisible(
      text,
...
Finder _backupOptionSwitchFinder(String titleKey) {
  final tile = find.ancestor(
    of: find.text(titleKey.tr()),
    matching: find.byType(SettingListTile),
  );
  return find.descendant(of: tile, matching: find.byType(Switch));
}

Future<void> _expectBackupOptionSwitch(
  WidgetTester tester, {
  ...
  expect(
    ...
    expected,
    reason: 'Unexpected value for ${titleKey.tr()}',
  );
}
...
Future<void> _setBackupDelaySlider(
  WidgetTester tester,
  int expectedSliderValue,
) async {
  ...
  await tester.tapAt(
    Offset(rect.left + rect.width * (expectedSliderValue / 3), rect.center.dy),
  );
  await _pumpFor(tester, const Duration(milliseconds: 500));
}

Future<void> _waitForBackupSetting<T>(
  WidgetTester tester,
...
Future<void> _ensureUploadQueueTextVisible(
  ...
  expect(
    scrollable,
    findsWidgets,
    reason: 'Expected $pageType to be scrollable before looking for "$text"',
  );
  try {
    await tester.scrollUntilVisible(
      textFinder,
...
Future<Map<String, DriftUploadStatus>> _waitForUploadItems(
  ...
  final end = DateTime.now().add(timeout);
  while (DateTime.now().isBefore(end)) {
    latest = Map<String, DriftUploadStatus>.from(
      container.read(driftBackupProvider).uploadItems,
    );
    if (matches(latest)) {
      return latest;
    }
